...
// pane's shell does NOT inherit Termifier.app's own HOME override at all --
...
// scripts read `~/Library/Application Support/Termifier/agent-endpoint.json`
...
// WORSE mismatch. `TERMIFIER_UITEST_SESSION_DIR` / `TERMIFIER_UITEST_DEFAULTS_SUITE`
...
// `uiTestSuite`, keyed by `TERMIFIER_UITEST_DEFAULTS_SUITE`, never
// `.standard`).
//
...
// own header on why: the TermifierUITests runner is itself App-Sandboxed and
...
final class CockpitApprovalE2ETests: TermifierUITestCase {

    // MARK: - Accessibility identifiers

    /// Literal mirrors of `AccessibilityID.ApprovalBanner.*`
    /// (Termifier/Helpers/AccessibilityID.swift) -- the `TermifierUITests`
    /// target drives the app-under-test as a separate OS process via
    /// `XCUIApplication`, with no `@testable import Termifier` linkage, so
    ...
    /// own `"termifier.commandPalette.searchField"`) already hardcodes the
    ...
    private static let approvalBannerAllowButtonID = "termifier.approvalBanner.allowButton"
    private static let approvalBannerPayloadID = "termifier.approvalBanner.payload"
    private static let approvalBannerPayloadExpandedID = "termifier.approvalBanner.payloadExpanded"
    private static let approvalBannerNextButtonID = "termifier.approvalBanner.nextButton"
    private static let approvalBannerPreviousButtonID = "termifier.approvalBanner.previousButton"
    private static let approvalBannerQueueMenuID = "termifier.approvalBanner.queueMenu"
    private static let approvalBannerContainerID = "termifier.approvalBanner.container"
    private static let approvalBannerOptionsMenuID = "termifier.approvalBanner.optionsMenu"
    private static let approvalBannerDismissButtonID = "termifier.approvalBanner.dismissButton"
    private static let approvalBannerTooltipID = "termifier.approvalBanner.tooltip"
    ...
    func test_cockpitTools_endToEnd() throws {
        ...
        let surfaceID = paneExec("echo $TERMIFIER_SURFACE_ID", counter: &counter)
        XCTAssertNotEqual(surfaceID, "(no output)", "could not read $TERMIFIER_SURFACE_ID from the pane")
        XCTAssertFalse(surfaceID.isEmpty, "$TERMIFIER_SURFACE_ID must be set for every ghostty-spawned pane")
        ...
        let allowOutFile = "/tmp/termifier-e2e-cockpit-run-allow-\(ProcessInfo.processInfo.processIdentifier).json"
        ...
        let denyOutFile = "/tmp/termifier-e2e-cockpit-run-deny-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        let navAOutFile = "/tmp/termifier-e2e-cockpit-run-nav-a-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        let navBOutFile = "/tmp/termifier-e2e-cockpit-run-nav-b-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    /// `Termifier/Features/ApprovalInbox/`) rather than inline inside the
    ...
    func test_approvalBanner_isPositionedAtVisibleFrameTopRightCorner() throws {
        ...
        let outFile = "/tmp/termifier-e2e-cockpit-panel-position-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    func test_paletteExecute_headerShowsDesignatedHostTabTitle_notThisWindow() throws {
        ...
        let outFile = "/tmp/termifier-e2e-cockpit-palette-header-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
    ...
    func test_payloadTap_togglesExpandedPayload() throws {
        ...
        let outFile = "/tmp/termifier-e2e-cockpit-payload-expand-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    /// deny, since Termifier never answers the question at all. The button
    ...
    func test_dismissButton_returnsStatusDismissed_removesBanner() throws {
        ...
        let outFile = "/tmp/termifier-e2e-cockpit-dismiss-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    /// Hovering the payload body must surface Termifier's own drawn tooltip
    ...
    func test_hoverPayload_showsTooltip_panelNeverMoves() throws {
        ...
        let outFile = "/tmp/termifier-e2e-cockpit-hover-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        // Termifier-drawn tooltip must still appear.
        let allowButton = app.buttons[Self.approvalBannerAllowButtonID]
    ...
    func test_twoWindows_onePendingRequestEach_singleAppWidePanel_pagesBetweenBoth() throws {
        ...
        let firstSurfaceID = paneExec("echo $TERMIFIER_SURFACE_ID", counter: &counter)
        XCTAssertFalse(firstSurfaceID.isEmpty, "$TERMIFIER_SURFACE_ID must be set for the first window's own pane")
        ...
        let secondSurfaceID = paneExec("echo $TERMIFIER_SURFACE_ID", counter: &counter)
        XCTAssertFalse(secondSurfaceID.isEmpty, "$TERMIFIER_SURFACE_ID must be set for the second window's own pane")
        ...
        let firstOutFile = "/tmp/termifier-e2e-cockpit-twowin-first-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        let secondOutFile = "/tmp/termifier-e2e-cockpit-twowin-second-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    private func enableAIAgentIPCViaCommandPalette() {
        ...
        XCTAssertTrue(alert.waitForExistence(timeout: 10),
                     "the IPC enable/error alert (TermifierWindowController.showIPCAlert) did not appear")
    ...
    private func toolCallSync(name: String, argumentsJSON: String, counter: inout Int, timeoutAttempts: Int = 20) -> [String: Any] {
        ...
        counter += 1
        let scriptPath = "/tmp/termifier-e2e-cockpit-sync-\(counter).py"
    ...
    private func toolCallBackgrounded(name: String, argumentsJSON: String, outFile: String, counter: inout Int) {
        ...
        let encoded = Data(script.utf8).base64EncodedString()
        counter += 1
        let scriptPath = "/tmp/termifier-e2e-cockpit-bg-\(counter).py"
    ...
    private func toolCallScript(name: String, argumentsJSON: String, maxTimeSeconds: Int) -> String {
        ...
        def main():
            endpoint_path = os.path.expanduser(
                "~/Library/Application Support/Termifier/agent-endpoint.json"
            )
