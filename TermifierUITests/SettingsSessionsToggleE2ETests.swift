...
//
// WHY NO "-termifier.session.persistentSessionsEnabled" LAUNCH ARGUMENT
...
//
// WHY A PER-TEST `TERMIFIER_UITEST_DEFAULTS_SUITE`, NOT THE REAL
// `com.termifier.terminal.e2e` DOMAIN (investigated, not assumed -- an
...
// developer's REAL `com.termifier.terminal.e2e` defaults domain regardless
// of `homeDir`). `SessionSettings` now has its own dedicated seam for
// this (`TERMIFIER_UITEST_DEFAULTS_SUITE`, mirroring
// `SessionPersistenceActor`'s existing `TERMIFIER_UITEST_SESSION_DIR`
// convention): this suite launches Termifier with that variable set to a
...
// isolate everything else this journey touches (`TERMIFIER_UITEST_SESSION_DIR`
...
final class SettingsSessionsToggleE2ETests: TermifierUITestCase {

    private var homeDir: String!
    private var sessionDir: String!
    /// Per-test-unique `UserDefaults` suite name (see this file's header,
    /// "WHY A PER-TEST `TERMIFIER_UITEST_DEFAULTS_SUITE`"): the app-under-test
    /// reads/writes `SessionSettings.persistentSessionsEnabled` here
    /// instead of the developer's real `com.termifier.terminal.e2e` defaults
    ...
    override func setUp() {
        ...
        defaultsSuiteName = "com.termifier.tests.e2e.SettingsSessionsToggleE2ETests-\(UUID().uuidString)"
    ...
    override func tearDown() {
        ...
        if let defaultsSuiteName {
            // Best-effort only (mirrors TermifierUITestCase.saveScreenshot's
            ...
            // `com.termifier.terminal.e2e` domain, which this suite never
    ...
    /// file's header): no `-termifier.session.*` overrides at all.
    ...
    /// `AccessibilityID` enum type itself: TermifierUITests never imports
    /// the Termifier module (this whole target only `import XCTest`,
    ...
    /// (Termifier/Helpers/AccessibilityID.swift, a new case this cycle's
    /// fix proposes) once it's wired onto the real switch via
    /// `.setAccessibilityIdentifier(...)` in SettingsWindowController.swift.
    private static let persistentSessionsSwitchIdentifier = "termifier.settings.sessions.persistentSessionsSwitch"
    ...
    func test_togglingPersistentSessionsSwitch_actuallyPersistsAcrossRelaunch() {
        ...
        quitAppViaMenu()
        XCTAssertTrue(app.wait(for: .notRunning, timeout: 10), "Termifier did not fully quit after \"Quit Termifier\".")
