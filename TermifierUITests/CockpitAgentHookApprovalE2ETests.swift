...
// exercise `TermifierMCPServer.routeApprovalRequest` at all.
//
...
//
// `-termifier.cockpit.agentHookApprovalEnabled YES` -- the same
...
// agentHookApprovalEnabledKey`, Termifier/Features/ApprovalInbox/
// CockpitSettings.swift; this target has no `@testable import Termifier`
...
final class CockpitAgentHookApprovalE2ETests: TermifierUITestCase {
    ...
    private static let allowButtonID = "termifier.approvalBanner.allowButton"
    private static let optionsMenuID = "termifier.approvalBanner.optionsMenu"
    private static let questionTextID = "termifier.approvalBanner.questionText"
    private static let answerButtonID = "termifier.approvalBanner.answerButton"
    private static let otherTextFieldID = "termifier.approvalBanner.otherTextField"
    private static let dismissButtonID = "termifier.approvalBanner.dismissButton"
    private static let containerID = "termifier.approvalBanner.container"
    ...
    func test_agentHookWithSuggestions_optionsMenuListsOfferAndNo_denyProducesDenyBehavior() throws {
        ...
        """
        let outFile = "/tmp/termifier-e2e-agenthook-suggestions-\(ProcessInfo.processInfo.processIdentifier).json"
        ...
        XCTAssertFalse(alwaysAllowItem.exists,
                       "no Termifier-side \"Always Allow ... in This Pane\" item may appear when the CLI already sent its own permission_suggestions")
    ...
    /// menu must carry Termifier's own pane-scoped "Always Allow Bash in
    ...
    func test_agentHookWithoutSuggestions_optionsMenuListsAlwaysAllow_yesProducesAllowBehavior() throws {
        ...
        """
        let outFile = "/tmp/termifier-e2e-agenthook-nosuggestions-\(ProcessInfo.processInfo.processIdentifier).json"
        postApprovalRequestBackgrounded(bodyJSON: bodyJSON, surfaceID: surfaceID, outFile: outFile, counter: &counter)
        ...
        XCTAssertTrue(waitFor(alwaysAllowItem, timeout: 10),
                     "the Options menu must list Termifier's own pane-scoped Always-Allow item when the CLI sent no permission_suggestions of its own")
    ...
    func test_agentQuestion_chooseListedOption_answersEchoesChosenLabel() throws {
        ...
        """
        let outFile = "/tmp/termifier-e2e-agentquestion-option-\(ProcessInfo.processInfo.processIdentifier).json"
        postApprovalRequestBackgrounded(bodyJSON: bodyJSON, surfaceID: surfaceID, outFile: outFile, counter: &counter)
    ...
    func test_agentQuestion_chooseOther_freeTextEchoesIntoAnswers() throws {
        ...
        {"hook_event_name":"PermissionRequest","tool_name":"AskUserQuestion","tool_input":{"questions":[{"question":"Pick a color","header":"Color","options":[{"label":"Red","description":"warm"},{"label":"Blue","description":"cool"}],"multiSelect":false}]}}
        """
        let outFile = "/tmp/termifier-e2e-agentquestion-other-\(ProcessInfo.processInfo.processIdentifier).json"
        postApprovalRequestBackgrounded(bodyJSON: bodyJSON, surfaceID: surfaceID, outFile: outFile, counter: &counter)
    ...
    /// "Termifier does not answer this" -- `AgentHookPermissionResponse.body`
    ...
    func test_agentHook_dismiss_producesEmptyResponseBody_removesBanner() throws {
        ...
        """
        let outFile = "/tmp/termifier-e2e-agenthook-dismiss-\(ProcessInfo.processInfo.processIdentifier).json"
    ...
    /// `$TERMIFIER_SURFACE_ID` shell variable, so the value handed to the
    /// raw `/approval-request` POST below is exactly what
    /// `TermifierMCPServer.approvalSurfaceExists(_:)` will look up.
    ...
    private func toolCallSync(name: String, argumentsJSON: String, counter: inout Int) -> [String: Any] {
        ...
        counter += 1
        let scriptPath = "/tmp/termifier-e2e-agenthook-sync-\(counter).py"
    ...
    /// `X-Termifier-Agent-Kind: claude-code` and `X-Termifier-Surface-ID:
    ...
    private func postApprovalRequestBackgrounded(
        ...
        def main():
            ...
            proc = subprocess.run(
                [
                    ...
                    "-X", "POST",
                    "-H", "Authorization: Bearer " + token,
                    "-H", "X-Termifier-Agent-Kind: claude-code",
                    "-H", "X-Termifier-Surface-ID: \(surfaceID)",
                    "-H", "Content-Type: application/json",
                    "--data-binary", body,
        ...
        let encoded = Data(script.utf8).base64EncodedString()
        counter += 1
        let scriptPath = "/tmp/termifier-e2e-agenthook-bg-\(counter).py"
