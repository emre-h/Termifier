...
// switcher (Termifier/Features/Settings/SettingsWindowController.swift,
...
// is scoped to TermifierUITests/ only, so:
...
// re-focuses the main Termifier window afterward, so it stays the frontmost/
// key window (and XCUITest's `windows` query orders front-to-back) for
// this test's whole duration.

import XCTest

final class SettingsWindowE2ETests: TermifierUITestCase {
    ...
    /// to `TermifierUITestCase.uiShotDir` for manual review -- including of
    ...
    func test_settingsWindow_toolbarTabsHaveImagesAndSwitchContent() {
        ...
        XCTAssertTrue(
            ...
            "No window appeared after invoking Preferences… from the Termifier application menu."
        )
