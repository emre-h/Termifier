...
// (`Termifier/App/AppDelegate.swift:1928-1952`) calls
...
// file (`$HOME/.termifier/state/history/<id>.raw`, see
...
//      `create_session` (termifier-session/crates/daemon/src/conn.rs:466-
...
// (Termifier.app is quit and relaunched, but the daemon-held session
...
// `termifier-session history on` (via a pane, see `PaneCLIExec`'s header
...
final class AgentResumeOfferE2ETests: TermifierUITestCase {
    ...
    private func launchApp(autoExecute: Bool) {
        ...
        app.launchArguments = [
            ...
            "-termifier.session.agentResumeEnabled", "YES",
            "-termifier.session.historyPersistenceEnabled", "YES",
            "-termifier.session.agentResumeAutoExecute", autoExecute ? "YES" : "NO",
        ]
    ...
    private func setUpResumableSessionAcrossRestart(autoExecute: Bool) -> String {
        ...
        // header) are REQUIRED on every pane-executed termifier-session
        ...
        let historyOnOutput = paneExec(
            "\(TermifierUITestCase.builtSessionBinaryPath) \(termifierSessionRootFlags(homeDir: homeDir)) history on",
            counter: &execCounter
        )
        XCTAssertEqual(
            historyOnOutput, "on",
            "`termifier-session history on` did not reply \"on\" (got \(historyOnOutput.debugDescription)); " +
        ...
        _ = paneExec(
            "\(TermifierUITestCase.builtSessionBinaryPath) \(termifierSessionRootFlags(homeDir: homeDir)) " +
        ...
        XCTAssertEqual(
            ...
            "within ~10s of `termifier-session meta set`. Last ledger contents: \(metaPoll.raw)"
        )
    ...
    func test_agentResumeOffer_autoExecutesResumeCommand_whenAutoExecuteSettingIsOn() {
        ...
        panePasteAndReturn(
            "\(TermifierUITestCase.builtSessionBinaryPath) \(termifierSessionRootFlags(homeDir: homeDir)) kill \(sessionID)"
        )
        _ = ledger.poll(
            timeoutAttempts: 10, sleepInterval: 1,
            transform: { sessions in self.ledger.session(withID: sessionID, in: sessions) },
