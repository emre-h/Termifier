...
final class TabReorderUITests: TermifierUITestCase {

    // MARK: - Helpers

    // Position-ordered tab lookup.
    //
    // The tab rows expose their `termifier.*.tab.<UUID>` identifier (via
    ...
    /// Tab-bar tab elements (identifier `termifier.tabBar.tab.<UUID>`, no
