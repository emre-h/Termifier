...
//  (ShellIntegrationInstaller/TermifierShellIntegrationEnvironment,
...
//  (`PaneCLIExec.termifierSessionRootFlags`) -- there is no equivalent
//  escape hatch here: `termifier.zsh`/the fish integration both read
//  `"$HOME/Library/Application Support/Termifier/agent-endpoint.json"` as a
...
//  verified) that a pane's shell does NOT inherit Termifier.app's own HOME
...
//  `~/Library/Application Support/Termifier/` tree. This is acceptable
...
//  removal of anything pre-existing): `TERMIFIER_UITEST_SESSION_DIR` /
//  `TERMIFIER_UITEST_DEFAULTS_SUITE` (the base class's own isolation) still
...
//  otherwise `TermifierWindowController.enableIPC` stops the MCP server
...
//  `SessionPersistenceE2ETests`'s own header on why: the `TermifierUITests`
...
final class CommandLogE2ETests: TermifierUITestCase {
    ...
    func test_trackedCommands_areQueryableViaTerminalMCPTools() throws {
        ...
        panePasteAndReturn("echo TERMIFIER_CMDLOG_MARKER_A1; false")
        panePasteAndReturn("echo done")

        let encodedScript = Data(Self.queryScript.utf8).base64EncodedString()
        let queryCommand = "printf '%s' '\(encodedScript)' | base64 -d > /tmp/termifier-e2e-cmdlog-query.py && " +
            "python3 /tmp/termifier-e2e-cmdlog-query.py"
        // Generous timeoutAttempts: the script itself retries
        // terminal_list_commands for up to ~20s internally (the curl
        // POSTs termifier.zsh's hooks fire are backgrounded+disowned, an
        ...
        let markerIndex = commands.firstIndex { ($0["command"] as? String)?.contains("TERMIFIER_CMDLOG_MARKER_A1") == true }
        let doneIndex = commands.firstIndex { ($0["command"] as? String)?.contains("echo done") == true }
        let markerCommandIndex = try XCTUnwrap(markerIndex, "no tracked command's text contains TERMIFIER_CMDLOG_MARKER_A1")
        ...
        XCTAssertEqual(markerCommand["exit_code"] as? Int, 1,
                       "`echo TERMIFIER_CMDLOG_MARKER_A1; false` must report exit_code 1 -- the LAST command in the " +
        ...
        XCTAssertTrue(outputText.contains("TERMIFIER_CMDLOG_MARKER_A1"),
    ...
    /// real `TermifierWindowController.enableIPC()` -- starts a real
    /// `TermifierMCPServer`, writes a real `agent-endpoint.json`, installs
    ...
    def main():
        ...
        surface_id = os.environ.get("TERMIFIER_SURFACE_ID", "")
        if not surface_id:
            surface_id = os.environ.get("TERMIFIER_SESSION_ID", "")
        if not surface_id:
            print(json.dumps({"error": "TERMIFIER_SURFACE_ID and TERMIFIER_SESSION_ID both unset in the pane"}))
            return
        ...
        for _ in range(20):
            ...
            marker_command = next(
                (c for c in list_result.get("commands", []) if "TERMIFIER_CMDLOG_MARKER_A1" in c.get("command", "")),
                None,
            )
