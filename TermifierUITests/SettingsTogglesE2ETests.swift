...
// identifiers, still prefixed `termifier.settings.sessions.*`) plus
...
// `SessionSettings.store` (resolves `TERMIFIER_UITEST_DEFAULTS_SUITE` before
...
final class SettingsTogglesE2ETests: TermifierUITestCase {

    /// The real, standing UserDefaults domain every `--uitesting` launch
    /// of Termifier.app resolves `UserDefaults.standard` to (see this file's
    /// header). Read directly from the TEST process to save/restore the
    /// three raw keys that bypass `TERMIFIER_UITEST_DEFAULTS_SUITE`.
    ...
    private nonisolated(unsafe) static let e2eStandardDefaults = UserDefaults(suiteName: "com.termifier.terminal.e2e")!

    private static let smoothScrollKey = "smoothScrollEnabled"
    private static let lspAutoInstallKey = "termifier.lsp.autoInstallEnabled"
    private static let lspRequireConfirmationKey = "termifier.lsp.requireInstallConfirmation"
    ...
    override func setUp() {
        ...
        defaultsSuiteName = "com.termifier.tests.e2e.SettingsTogglesE2ETests-\(UUID().uuidString)"
    ...
    override func tearDown() {
        ...
        // Restore the three raw `com.termifier.terminal.e2e` keys to exactly
    ...
    /// SettingsSessionsToggleE2ETests exactly: no `-termifier.session.*` /
    /// `-termifier.lsp.*` NSArgumentDomain overrides, which would shadow
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["TERMIFIER_UITEST_DEFAULTS_SUITE"] = defaultsSuiteName
    ...
    private func assertTogglePersistsAcrossRelaunch(
        ...
        XCTAssertTrue(
            ...
            "Termifier did not fully quit after \"Quit Termifier\".",
            file: file, line: line
        )
    ...
    func test_historyPersistenceSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.sessions.historyPersistenceSwitch",
            pane: "Sessions"
        )
    }
    ...
    func test_agentResumeSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.sessions.agentResumeSwitch",
            pane: "Agents"
        )
    }
    ...
    func test_agentResumeAutoExecuteSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.sessions.agentResumeAutoExecuteSwitch",
            pane: "Agents"
        )
    }
    ...
    func test_smoothScrollingSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.appearance.smoothScrollingSwitch",
            pane: "Appearance"
        )
    }
    ...
    func test_lspAutoInstallSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.lsp.lspAutoInstallSwitch",
            pane: "LSP"
        )
    }
    ...
    func test_lspRequireConfirmationSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "termifier.settings.lsp.lspRequireConfirmationSwitch",
            pane: "LSP"
        )
    }
    ...
    func test_agentHookApprovalSwitch_isReachableAndTogglesOnAgentsPane() {
        ...
        let toggleElement = toggle(identifier: "termifier.settings.sessions.agentHookApprovalSwitch")
