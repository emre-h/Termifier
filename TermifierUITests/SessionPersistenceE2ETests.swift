...
// restart (termifier-session's daemon keeps the child process running
// independent of Termifier.app's lifetime), and the pane restored after
...
// `termifier-session ls --all --json` as an out-of-process `Process` from
// inside the test runner. That approach is unusable here: the
// `TermifierUITests` runner is itself App-Sandboxed (`codesign -d
...
// `termifier-session ls` from the runner therefore always returns empty
...
// `$HOME/.termifier/state/sessions.json` (see
// `termifier-session/crates/daemon/src/ledger.rs`), the same file the
// daemon writes atomically (temp file + rename) on every registry
// change, and the same array `termifier-session ls` would otherwise print,
...
// same child process alive across Termifier.app's termination, and that
...
//
// Isolation:
// - Window/tab session persistence: `TermifierUITestCase`'s existing
//   `TERMIFIER_UITEST_SESSION_DIR` (per-test temp dir) — reused here via
//   its own launchEnvironment key, unchanged.
// - termifier-session daemon state (`$HOME/.termifier/run`,
//   `$HOME/.termifier/state` — see
//   `termifier-session/crates/cli/src/commands/mod.rs`'s
//   `resolve_runtime_dir`): the daemon consults `$HOME` directly and
//   has no independent `TERMIFIER_SESSION_*` runtime-dir override wired
//   through the bundled `Termifier.app` launch path, so `HOME` itself is
//   overridden to a per-test temp directory instead.
// - `TERMIFIER_SESSION_BIN` points `SessionBinaryResolver` at a
//   specific, already-built `termifier-session` binary rather than the
...
// not run yet): `Termifier.app`'s ghostty-spawned `termifier-session attach`
// child process inherits its environment from `Termifier.app`'s own
// process, which is what makes overriding `HOME` on `app
// .launchEnvironment` (rather than some Termifier-internal setting) enough
...
final class SessionPersistenceE2ETests: TermifierUITestCase {
    ...
    override func setUp() {
        ...
        // $HOME/.termifier/run/sessiond.sock (see
        // `termifier-session/crates/cli/src/commands/mod.rs`'s
        ...
        // past that limit once `.termifier/run/sessiond.sock` is appended.
        ...
        // "/.termifier/run/sessiond.sock" (25 bytes) = 46 bytes total,
    ...
    /// Path to a pre-built `termifier-session` binary this test points
    /// `TERMIFIER_SESSION_BIN` at. RED-phase placeholder: the actual
    ...
    /// simulating the user quitting and reopening Termifier, not a fresh
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["TERMIFIER_UITEST_SESSION_DIR"] = sessionDir
        app.launchEnvironment["HOME"] = homeDir
        app.launchEnvironment["TERMIFIER_SESSION_BIN"] = Self.builtSessionBinaryPath()
    ...
    /// Quits the app through its own application menu ("Termifier" > "Quit
    /// Termifier"), instead of `app.terminate()`. `app.terminate()` sends
    ...
    private func quitAppViaMenu() {
        menuAction("Termifier", item: "Quit Termifier")
    }
    ...
    private func sidebarTabTitles() -> Set<String> {
        let rows = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@ AND NOT identifier ENDSWITH %@", "termifier.sidebar.tab.", ".closeButton"))
    ...
    // `termifier-session ls` process, because the sandboxed test runner
    // cannot open the unix domain socket that would require (see file
    // header).

    /// Reads and parses the daemon's on-disk session ledger at
    /// `$HOME/.termifier/state/sessions.json` (see
    /// `termifier-session/crates/daemon/src/ledger.rs`), the same array
    /// `termifier-session ls` would otherwise print. Each entry mirrors
    /// `SessionInfo` (`termifier-session/crates/proto/src/control.rs`):
    ...
    /// `termifier-session/crates/daemon/src/conn.rs`), and `"state"`,
    ...
    private func daemonSessions() -> (sessions: [[String: Any]], raw: String) {
        let ledgerURL = URL(fileURLWithPath: "\(homeDir!)/.termifier/state/sessions.json")
    ...
    func test_persistentSession_survivesRestart_sameShellStaysAlive() {
        ...
        // `termifier-session attach --create`, which registers a session
        ...
        guard sawRunningSession.value else {
            XCTFail(
                ...
                "`termifier-session attach --create` ever established a live daemon session. " +
        ...
        for (id, snap) in preRestart {
            XCTAssertGreaterThanOrEqual(
                ...
                "tab appearing, meaning the pane's `termifier-session attach --create` " +
        ...
        XCTAssertEqual(
            ...
            "The number of Termifier main windows changed after restarting (before: " +
        ...
        for (id, before) in preRestart {
            guard let after = postRestart[id] else {
                XCTFail(
                    ...
                    "keep it alive across Termifier.app's termination. Last ledger contents: " +
                    "\(postRestartPoll.raw)"
                )
                continue
            }
            XCTAssertTrue(
                ...
                "the daemon-held child process died instead of surviving Termifier.app's " +
                "termination."
            )
            XCTAssertEqual(
