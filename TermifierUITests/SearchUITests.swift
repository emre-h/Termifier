...
final class SearchUITests: TermifierUITestCase {
    ...
    /// types a query into `termifier.search.searchField` that matches it
    /// multiple times, and asserts `termifier.search.matchCount`'s text
    ...
    func test_searchFindsSeededContent_andNextMovesSelectedMatch() {
        ...
        let matchCount = app.descendants(matching: .any)
            .matching(identifier: "termifier.search.matchCount")
            .firstMatch
        ...
        // "Find Next" (termifier.search.nextButton, the search bar's own
