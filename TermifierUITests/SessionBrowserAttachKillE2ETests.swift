...
// (`Termifier/Features/SessionBrowser/SessionBrowserWindowController.swift`,
...
// `SUN_LEN`, `TERMIFIER_UITEST_SESSION_DIR` for window/tab persistence,
// `TERMIFIER_SESSION_BIN` pinning a specific built `termifier-session` binary)
...
// (Termifier/Features/SessionBrowser/SessionBrowserView.swift) each carry
// per-row `.accessibilityIdentifier`s from `AccessibilityID.SessionBrowser`
// (Termifier/Helpers/AccessibilityID.swift), the same pattern
...
// up by its row-scoped identifier (`termifier.sessionBrowser.row.<id>
// .attachButton` / `.killButton`) rather than by label.

import XCTest

final class SessionBrowserAttachKillE2ETests: TermifierUITestCase {
    ...
    /// itself -- not some Termifier-internal setting -- is what redirects
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["TERMIFIER_SESSION_BIN"] = TermifierUITestCase.builtSessionBinaryPath
    ...
    /// suite's `homeDir` launches Termifier.app with), so it would silently
    ...
    /// the count of actual Termifier main (tab-hosting) windows, the thing
    ...
    /// header comment).
    ///
    /// Deliberately sidebar, NOT `TermifierUITestCase.countTabBarTabs()`'s
    ...
    /// `Group` elements expose NEITHER their `termifier.tabBar.tab.<UUID>`
    /// identifier NOR their `termifier.tabBar.tab.index.N` value under any
    ...
    /// SIDEBAR's equivalent rows (`termifier.sidebar.tab.<UUID>`), by
    ...
    /// termifier-session CLI, routed through A's own pane -- see
    /// `PaneCLIExec.paneExec`'s header for why a sandboxed XCUITest
    /// runner cannot spawn `termifier-session` itself). Opens the browser,
    ...
    func test_sessionBrowser_attachDetachedSession_thenKillAnotherSession() {
        ...
        // header for why a bare `termifier-session`/`$TERMIFIER_SESSION_BIN`
        // inside a pane silently resolves against the real HOME instead
        // of this test's isolated one). If `TERMIFIER_SESSION_BIN` isn't set
        ...
        let sessionBinaryPath = TermifierUITestCase.builtSessionBinaryPath
        XCTAssertFalse(
            sessionBinaryPath.isEmpty,
            "TERMIFIER_SESSION_BIN is empty in this test runner's own process environment, " +
            "so every termifier-session command this suite pastes into a pane would be " +
            ...
            "header). Set TERMIFIER_SESSION_BIN as a build setting/env var when invoking " +
            "xcodebuild test, e.g. `xcodebuild test ... TERMIFIER_SESSION_BIN=/path/to/" +
            "termifier-session`."
        )
        ...
        // Session B: created Detached (via `termifier-session new`, which
        // never attaches -- see termifier-session/crates/cli/src/commands/
        ...
        // REAL `~/.termifier`, not this test's isolated homeDir.
        //
        ...
        // process context `termifier-session new`'s spawned child actually
        ...
        // resolved runner-side above (never a `$TERMIFIER_SESSION_BIN`
        ...
        let sessionBID = paneExec(
            "'\(sessionBinaryPath)' \(termifierSessionRootFlags(homeDir: homeDir)) new --name sessionB --cwd \(sessionBCwd)",
            counter: &execCounter
        )
        XCTAssertFalse(
            sessionBID.isEmpty || sessionBID == "(no output)",
            "`termifier-session new` produced no output when run through session A's pane, " +
            "meaning the CLI call routed through the pane never reached the daemon (or " +
            "TERMIFIER_SESSION_BIN isn't inherited into the pane's environment -- see " +
        ...
        XCTAssertFalse(
            ...
            "`termifier-session new --cwd \(sessionBCwd)` returned an error instead of a " +
        ...
        guard let sessionBRow = sessionBPoll.value else {
            XCTFail(
                "Session B (id \(sessionBID), printed by `termifier-session new`) never " +
        ...
        XCTAssertEqual(
            ...
            "Session B should have 0 attached_clients right after `termifier-session new` " +
        ...
        // Termifier itself, not this suite), so it renders its raw id; B
        ...
        // anywhere in this Termifier process) but not A (isAttachedHere,
        ...
        let attachButtonForB = app.buttons["termifier.sessionBrowser.row.\(sessionBID).attachButton"]
        ...
        XCTAssertEqual(
            ...
            "The number of Termifier main windows changed after attaching session B " +
        ...
        let killButtonForA = app.buttons["termifier.sessionBrowser.row.\(sessionAID).killButton"]
