...
// TermifierTests/RecoveryBarModelTests.swift for the view-model-level RED
...
// existence check against `$TERMIFIER_UITEST_SESSION_DIR/sessions.recovery.json`
...
// termifier-session daemon-backed persistent session or an ordinary
...
// that method's own doc comment) -- no termifier-session binary, no daemon
// socket, no `TERMIFIER_SESSION_BIN` involved at all. So instead of
// standing up an isolated daemon (short-HOME trick, TERMIFIER_SESSION_BIN,
...
// `sessionRefs` key) directly into a fresh `TERMIFIER_UITEST_SESSION_DIR`
...
// (TermifierTests/, root) -- this suite adds the missing "the user can
...
//
// Isolation: `TERMIFIER_UITEST_SESSION_DIR` only (mirrors the base
// TermifierUITestCase.setUp()'s own key) -- no HOME override, no
// TERMIFIER_SESSION_BIN, no termifier-session daemon involved at all (see
...
/// the preserved file".
final class SessionRecoveryBarE2ETests: TermifierUITestCase {
    ...
    override func setUp() {
        ...
        sessionDir = NSTemporaryDirectory() + "TermifierUITests-recoverybar-\(UUID().uuidString)"
    ...
    /// to under a `TERMIFIER_UITEST_SESSION_DIR` override (see
    /// SessionPersistenceActor.swift's init): the directory itself IS
    /// `termifierDir` in that branch (unlike a real launch, which appends
    /// `.termifier` under `HOME` -- the test override skips that extra
    /// path component), so the recovery file lives directly at
    /// `<sessionDir>/sessions.recovery.json`, not
    /// `<sessionDir>/.termifier/sessions.recovery.json`.
    ...
    /// shell with zero termifier-session/daemon involvement (see file
    ...
    //
    // Literal `"termifier.recoveryBar.*"` strings, not `AccessibilityID.RecoveryBar.*`:
    // TermifierUITests runs the app-under-test as a SEPARATE process (no
    // `@testable import Termifier` anywhere in this target -- confirmed by
    ...
    // `app.buttons["termifier.sessionBrowser.row.\(id).attachButton"]`
    // already does, cross-referenced back to
    // Termifier/Helpers/AccessibilityID.swift by name in each comment.

    /// AccessibilityID.RecoveryBar.restoreButton
    private var restoreButton: XCUIElement {
        app.buttons["termifier.recoveryBar.restoreButton"]
    }

    /// AccessibilityID.RecoveryBar.dismissButton
    private var dismissButton: XCUIElement {
        app.buttons["termifier.recoveryBar.dismissButton"]
    }
    ...
    private func pollForRecoveryBarContainer(exists: Bool, timeout: TimeInterval = 10) -> Bool {
        ...
        repeat {
            let found = app.descendants(matching: .any)
                .matching(identifier: "termifier.recoveryBar.container")
                .firstMatch.exists
            if found == exists {
                return true
            }
    ...
    /// `TermifierWindowController.setupCommandRegistry()`). Unlike the bar,
    ...
    func test_recoveryBar_paletteRecoverPreviousSession_restoresWindowClearsFileAndDisablesCommand() {
        ...
        let resultsTable = app.descendants(matching: .any)
            .matching(identifier: "termifier.commandPalette.resultsTable")
            .firstMatch
        XCTAssertEqual(
...
/// -- the base `TermifierUITestCase.setUp()`'s own default session dir is
...
final class SessionRecoveryBarAbsentE2ETests: TermifierUITestCase {
    ...
    func test_recoveryBar_doesNotAppear_onNormalLaunchWithNoPreservedSnapshot() {
        ...
        let staysAbsent = XCTNSPredicateExpectation(
            ...
            object: app.otherElements["termifier.recoveryBar.container"]
        )
