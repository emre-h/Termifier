...
// `TermifierUITests/AuxiliaryWindowCloseE2ETests.swift`'s header for why.

import XCTest

final class MenuShortcutsUITests: TermifierUITestCase {
    ...
    /// switch groups using the existing `termifier.sidebar.group.<UUID>` identifier
    /// scheme.
    private func currentGroupCount() -> Int {
        countElements(matching: "termifier.sidebar.group.", excludingSuffix: "Button")
    }
    ...
    /// "GitHub issue #45" comment and `NSWindow+TermifierClose.swift`'s
    ...
    /// `TermifierUITests/AuxiliaryWindowCloseE2ETests.swift`'s job -- see
    ...
    func test_findViaMenu_opensSearchBar() {
        ...
        let searchField = app.descendants(matching: .any)
            .matching(identifier: "termifier.search.searchField")
            .firstMatch
        XCTAssertFalse(
            searchField.exists,
        ...
        XCTAssertTrue(
            ...
            "Edit > Find > Find… should reveal the in-terminal search bar (identifier: termifier.search.searchField)"
        )
    }
    ...
    func test_nextGroupViaMenu_switchesGroup() {
        ...
        // (`TermifierUITestCase.currentTabBarTabIdentifiers()`'s own doc
    ...
    func test_splitRight_isDisabled_inBrowserTab() {
        ...
        let toolbar = app.descendants(matching: .any)
            .matching(identifier: "termifier.browser.toolbar")
            .firstMatch
        XCTAssertTrue(
