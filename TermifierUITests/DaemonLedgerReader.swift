...
// unix-domain-socket connection to the daemon, so `termifier-session ls`
...
/// Reads `$HOME/.termifier/state/sessions.json` (see
/// `termifier-session/crates/daemon/src/ledger.rs`) directly from disk and
...
struct DaemonLedgerReader {
    ...
    /// serde derive in `termifier-session/crates/proto/src/control.rs`).
    ...
    /// change, so a `termifier-session meta set` reaches this file without
