...
// records real Claude Code agents running in Termifier panes (including a
...
//
// Skipped by default (see the TERMIFIER_DEMO_RECORDING guard, first line of
// the test method) so it never runs as part of a normal TermifierUITests
// pass -- only `scripts/record-demo.sh` sets that env var (propagated to
// the test runner process via project.yml's own
// `schemes.TermifierUITests.test.environmentVariables` entry, the same
// propagation `TERMIFIER_SESSION_BIN` already needs there: a plain shell
// prefix on `xcodebuild test` does not otherwise reach the runner).
//
// LAUNCH CONFIGURATION, and why this does NOT use TermifierUITestCase's own
...
// method, after the TERMIFIER_DEMO_RECORDING skip guard, so `setUp()`/
...
// argument (`-termifier.cockpit.agentHookApprovalEnabled YES`) rather than a
// runner-side `UserDefaults(suiteName:).set(...)` write: the
// TermifierUITests runner is itself App-Sandboxed (see
...
// per-run UUID (TermifierUITestCase.setUp()'s own convention), cleaned up in
// `tearDown()` below.
//
...
final class DemoRecordingScenario: TermifierUITestCase {
    ...
    /// Per-run UUID defaults suite (TermifierUITestCase.setUp()'s own
    ...
    override func tearDown() {
        ...
        // super.tearDown(): TermifierUITestCase.tearDown() force-unwraps
        ...
        // TERMIFIER_DEMO_RECORDING guard skips before `app` is ever assigned,
        ...
        if let defaultsSuiteName {
            ...
            // TermifierUITestCase.tearDown()'s own identical cleanup.
    ...
    func test_demoRecordingScenario() throws {
        guard ProcessInfo.processInfo.environment["TERMIFIER_DEMO_RECORDING"] == "1" else {
            throw XCTSkip("Set TERMIFIER_DEMO_RECORDING=1 to run the demo recording scenario")
        }

        // MARK: - Launch configuration

        // Per-run UUID suite name (TermifierUITestCase.setUp()'s own
        ...
        let suiteName = "com.termifier.tests.e2e.DemoRecordingScenario-\(UUID().uuidString)"
        defaultsSuiteName = suiteName

        let tempDir = NSTemporaryDirectory() + "TermifierDemoRecording-\(UUID().uuidString)"
        ...
        app.launchArguments = [
            ...
            // (Termifier/Features/ApprovalInbox/CockpitSettings.swift; this
            // target has no @testable import Termifier linkage). See this
            // file's header for why this must be an NSArgumentDomain
            // argument, not a suite pre-seed from the test runner.
            "-termifier.cockpit.agentHookApprovalEnabled", "YES"
        ]
        ...
        clickQuadrant(1)
        panePasteAndReturn(
            "cd /tmp/termifier-demo-workspace && claude --model sonnet " +
        ...
        clickQuadrant(4)
        panePasteAndReturn("cd /tmp/termifier-demo-workspace && clear")
        ...
        let banner = app.descendants(matching: .any)
            .matching(identifier: "termifier.approvalBanner.container")
            .firstMatch
        awaitOrContinue(banner, timeout: 60)
        Thread.sleep(forTimeInterval: 2)

        let allowButton = app.buttons["termifier.approvalBanner.allowButton"]
        ...
        let agentModeButton = app.descendants(matching: .any)
            .matching(identifier: "termifier.sidebar.agentModeButton")
            .firstMatch
        ...
        let firstAgentRow = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "termifier.sidebar.agentRow."))
            .firstMatch
        ...
        // ("touch /tmp/termifier-demo-done") is a completion SENTINEL: the
        ...
        panePasteAndReturn(
            "Respond only in English. Using the termifier-ipc tools, find the pane that is running a " +
            "shell command (not one of the agent panes), wait for that command to finish, then tell " +
            "me how many tests passed. When you have answered, run exactly: touch /tmp/termifier-demo-done"
        )

        // Hold for pane 4 to actually run the tests and pane 1 to
        // summarize the result -- a KEEPER loop that ends on the agent's
        // OWN completion sentinel (/tmp/termifier-demo-done, touched by its
        ...
        // the same "sandboxed TermifierUITests runner reading a plain /tmp
        ...
        for _ in 0..<90 {
            Thread.sleep(forTimeInterval: 2)
            approvePendingIfAny()
            if FileManager.default.fileExists(atPath: "/tmp/termifier-demo-done") {
                break
            }
        }
    ...
    /// near-duplicate rather than promoted onto TermifierUITestCase itself,
    ...
    /// EVERY agent tool call banners (TermifierMCPServer's
