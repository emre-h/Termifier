...
/// `--uitesting` app process, driven by the `TERMIFIER_UITEST_DEFAULTS_SUITE`
...
final class SettingsStore: @unchecked Sendable {
    ...
    /// full `TERMIFIER_UITEST_DEFAULTS_SUITE` rationale. A `let` resolved
    ...
    /// `TERMIFIER_UITEST_DEFAULTS_SUITE` is fixed for the process's entire
    ...
    init() {
        if let name = ProcessInfo.processInfo.environment["TERMIFIER_UITEST_DEFAULTS_SUITE"] {
