...
class QuickTerminalController: NSObject, NSWindowDelegate {
    ...
    /// `TermifierWindowController._setHasAppliedInitialSizeForTesting`'s
    ...
    /// Test seam: mirrors `TermifierWindowController
    ...
    /// GitHub issue #45 follow-on: `QuickTerminalWindow.termifierPerformClose(_:)`'s
    ...
    /// mirroring `TermifierWindowController.handleCloseSurfaceNotification`'s
    ...
    /// Not `private` (mirrors `TermifierWindowController.processChildExited`'s
    ...
    /// replicate `TermifierWindowController`'s `deferOrRun`/`isConfirmingQuit`
    ...
    /// observed only by `TermifierWindowController`, whose
    ...
    /// closed surface, matching `TermifierWindowController
