...
// (`Termifier/Helpers/AwaitBridge.swift`). See
// TermifierTests/ApprovalInbox/ApprovalInboxStoreTests.swift for the
// specced contract.
//
...
/// rendered view hierarchy (see `TermifierWindowController.refreshRecoveryBar()`'s
...
extension Notification.Name {
    static let termifierApprovalInboxChanged = Notification.Name("com.termifier.approvalInbox.changed")
}
...
final class ApprovalInboxStore {
    ...
    /// `decision`, but posts `.termifierApprovalInboxChanged` exactly ONCE
    ...
    /// `MCPCockpitBridge.gate` and `TermifierMCPServer.routeApprovalRequest`
    ...
    /// `TermifierMCPServer.routeCommandEvent`, for the opposite case: the
    ...
    /// posted `.termifierApprovalInboxChanged`, so a test can assert a
    ...
    /// `TermifierWindowController._closingTabIDsForTesting`). DO NOT use
    ...
    private func notifyChanged() {
        ...
        _testNotifyCount += 1
        #endif
        NotificationCenter.default.post(name: .termifierApprovalInboxChanged, object: self)
    }
