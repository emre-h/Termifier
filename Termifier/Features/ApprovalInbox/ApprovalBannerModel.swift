...
// instead of reaching for TermifierWindowController/NSWindow directly.
...
// TermifierTests/ApprovalInbox/ApprovalBannerModelTests.swift for the
// specced contract.
...
final class ApprovalBannerModel {
    ...
    /// `refreshHostingView()` or posts `.termifierApprovalInboxChanged`: both
    ...
    /// `.termifierApprovalInboxChanged` doc comment) exists for mutations
    ...
    /// `.termifierApprovalInboxChanged` explicitly: those two are clicked
    ...
    /// `ApprovalInboxStore.swift`'s own `.termifierApprovalInboxChanged` doc
    ...
    func select(id: UUID) {
        ...
        NotificationCenter.default.post(name: .termifierApprovalInboxChanged, object: store)
    }
    ...
    /// bar) may hold it instead. When both fire, `TermifierWindowController
    ...
    /// The panel's own top-left × button: "Termifier does not answer this
    ...
    /// Termifier (that request's own CLI, or the calling MCP agent) can still
    /// decide it once Termifier itself declines to; see that property's own
    /// doc comment. A non-dismissible request (grok/pi: Termifier's gate is
    ...
    /// request, so this posts exactly ONE `.termifierApprovalInboxChanged`
    ...
    func alwaysAllow(id: UUID) {
        ...
        case .agentHook(let toolName, let kind, _, let offers):
            ...
            // for why recording Termifier's OWN pane memory alongside that
