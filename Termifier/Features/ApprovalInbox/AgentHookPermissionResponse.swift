...
// Termifier
//
// The PermissionRequest hook stdout body Termifier writes back to a CLI
...
// `.dismissed` ("Termifier does not answer this request") produces nil for
...
// exactly what Termifier was asked to gate -- hence the expiry deny. That
// mode is also the only one a grok request is ever held for:
// `TermifierMCPServer.routeApprovalRequest` answers every other mode with no
...
// popups), so Termifier's gate is the only approval mechanism a pi tool call
...
// / `AgentEntry.grokKind` / `AgentEntry.piKind`. Termifier must never inject
...
enum AgentHookPermissionResponse {
    ...
    /// message covers it.
    static let deniedMessage = "Denied from the Termifier approval inbox."

    /// Whether `kind`'s own CLI keeps its own confirmation prompt to fall
    /// back to when Termifier answers its PermissionRequest hook with no
    ...
    /// fallback (Termifier's gate is the only approval mechanism either ever
    ...
    /// Termifier can decide a dismissed request only for a CLI that keeps its
    ...
    /// "No" choice hands back to the model, so declining from Termifier's
    ...
    private static func flatBody(for decision: ApprovalDecision, kind: String) -> Data? {
        ...
        case .expired:
            return try? JSONSerialization.data(withJSONObject: [
                "decision": "deny",
                "reason": "No approval was given in the Termifier approval inbox before the request expired.",
            ])
