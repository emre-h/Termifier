...
struct ApprovalRequest: Identifiable, Sendable {
    enum Source: Sendable, Equatable {
        ...
        /// extension gate, rather than Termifier's own MCP server -- `kind`
...
struct AgentHookOffers: Sendable, Equatable {
    ...
    /// those rows, so `ApprovalBannerModel.alwaysAllow(id:)` (Termifier's OWN
    /// pane memory) is a no-op for this request: recording a second,
    /// Termifier-side memory alongside the CLI's own would be redundant and
    /// could silently diverge from it. When `permissionUpdates` is empty,
    /// `cliOwnsPersistence` is false: Termifier's own pane-scoped memory is
    /// the only persistence available, and its row renders instead.
    let cliOwnsPersistence: Bool

    /// The no-offers constant: no permission updates, so Termifier's own
...
enum ApprovalDecision: Sendable, Equatable {
    ...
    /// The banner's own [x] dismiss action: "Termifier does not answer this
...
extension ApprovalRequest {
    ...
    /// Whether the panel's own × ("Termifier does not answer this request",
    /// `ApprovalBannerModel.dismiss(id:)`) applies to this request --
    /// true only when someone OTHER than Termifier can still decide it once
    /// Termifier itself declines to. `.mcpTool`: always true, the calling MCP
    ...
    /// such fallback -- Termifier's gate is their only prompt (same header) --
