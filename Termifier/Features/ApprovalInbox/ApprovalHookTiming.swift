...
// CLI-agents hook chain: Termifier's own long-poll timeout must resolve
...
//   Termifier is still legitimately waiting on a human decision, and the
...
// in-pane prompt -- Termifier never silently allows anything by going
// missing.
//
// See TermifierMCPServerApprovalRequestTests.test_timing_constants_orderingInvariant
...
enum ApprovalHookTiming {
    ...
    /// per-hook entry in both CLIs; Termifier's own injected approval-hook
    ...
    /// `TermifierMCPServer.approvalRequestTimeoutMs`'s default: the
    ...
    /// The `termifier-approval-hook` script's own curl `-m` deadline, in
