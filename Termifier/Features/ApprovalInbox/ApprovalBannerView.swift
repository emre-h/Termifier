...
// Termifier/Features/ApprovalInbox/) -- an independent window, not a strip
...
//
// - `.mcpTool` (Termifier's own pane_run approval): entirely inline here --
...
//   `AgentHookOffers.permissionUpdates` offer, Termifier's own pane-scoped
...
struct ApprovalBannerView: View {
    ...
    /// (already labeled by `AgentPermissionOffer.label(for:)`), Termifier's
    ...
    /// for why that flag is what decides whether Termifier's row renders.
    @ViewBuilder
    ...
    /// (Termifier/Views/Browser/BrowserContainerView.swift). The label
    ...
    /// them internally, and TermifierTests/ApprovalInbox/
