...
// Termifier
//
// Manages reading/writing ~/.codex/config.toml for the Termifier IPC MCP server.
...
struct CodexConfigManager: Sendable {
    ...
    static func enableIPC(port: Int, token: String, configPath: String? = nil) throws {
        ...
        // Remove existing termifier-ipc sections, normalize line endings
        ...
        // Inside a persistent termifier-session pane, TERMIFIER_SURFACE_ID does not
        // identify the pane (absent with a current session daemon, stale
        // with an older one still running), so `X-Termifier-Session-ID` carries
        // the identity that has to be sent instead; see
        // `TermifierMCPServer.resolveSurfaceID`'s doc comment for how the two
        // headers are resolved. Measured against real codex 0.148.0: when
        // TERMIFIER_SESSION_ID is unset (an ordinary, non-persistent pane),
        ...
        http_headers = { "Authorization" = "Bearer \(token)", "X-Termifier-Agent-Kind" = "\(AgentEntry.codexKind)" }
        env_http_headers = { "X-Termifier-Surface-ID" = "TERMIFIER_SURFACE_ID", "X-Termifier-Session-ID" = "TERMIFIER_SESSION_ID" }
        """
    ...
    /// Regex pattern for `[mcp_servers.termifier-ipc]` section header.
    ...
    /// it — including `[mcp_servers.termifier-ipc]` — regardless of whether a
    /// blank line separates them.
    private static let anyTableHeaderPattern = #"^[ \t]*\["#

    /// Regex pattern for a `[mcp_servers.termifier-ipc.*]` or
    /// `[[mcp_servers.termifier-ipc.*]]` sub-table header. Per TOML's dotted-key
    /// semantics this is still part of the termifier-ipc entry (not a boundary)
    ...
    /// Regex pattern for the BEGIN marker of a Termifier-managed block
    /// (`CodexHooksConfigManager`'s `[[hooks.*]]` block). Recognized as a
    /// section boundary even though it's an ordinary TOML comment, not a
    /// table header.
    private static let termifierManagedBlockMarkerPattern = #"^[ \t]*#\s*BEGIN TERMIFIER"#
    ...
    private static func isTermifierManagedBlockMarker(_ line: String) -> Bool {
        line.range(of: termifierManagedBlockMarkerPattern, options: .regularExpression) != nil
    }

    /// Remove all `[mcp_servers.termifier-ipc]` sections from the content.
    /// Normalizes `\r\n` to `\n`.
    ///
    /// A section's body ends at the next TOML table header (`[...]` or
    /// `[[...]]`), or at a `# BEGIN TERMIFIER` managed-block marker — never at a
    ...
    private static func removeSections(from content: String) -> String {
        ...
        for line in lines {
            if isSectionHeader(line) {
                // Start of a termifier-ipc section — skip this line
                inSection = true
                continue
            }

            if inSection {
                if isTermifierIpcSubTableHeader(line) {
                    // A [mcp_servers.termifier-ipc.*] sub-table is still part of
                    // this entry — remove it along with the rest.
                    continue
                }
                if isAnyTableHeader(line) || isTermifierManagedBlockMarker(line) {
                    // Hit the next table header, or another managed block's
                    // BEGIN marker comment — end of termifier-ipc section.
                    inSection = false
                    result.append(line)
                    continue
                }
