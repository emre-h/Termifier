...
//  Termifier
//
//  MCP (Model Context Protocol) message types and router for the Termifier IPC system.
...
struct MCPRouter: Sendable {
    ...
    /// auto-register a peer for (no `X-Termifier-Surface-ID`, e.g. an
    ...
    /// `TermifierMCPServer`'s `initialize` case). The old text told
    ...
    private static let commonSuffixParagraphs: [String] = [
        ...
        "Terminal command-history tools (terminal_*) are available when shell integration is installed (zsh and fish only, currently). Use terminal_list_commands to list the commands recorded for a surface, oldest-first, terminal_read_output to fetch a specific command's captured output, and terminal_await_command to block until a running command finishes or the timeout elapses. terminal_await_command returns {\"status\": \"timeout\"} on timeout — simply call it again to keep waiting. terminal_list_commands and terminal_await_command's surface_id argument also accepts a termifier-session ID in place of the raw surface UUID (terminal_read_output takes command_id instead and has no surface_id argument).",
        "Cockpit pane-discovery and layout tools are always available. Use pane_list to enumerate every terminal pane across all open Termifier windows (pane identity is split-tree leaf membership, so any pane it reports is guaranteed operable), pane_split to split an existing pane into two, and tab_create to open a new tab (optionally in a named group and/or at a specific working directory — this visibly changes focus in the live app window). pane_split's surface_id argument also accepts a termifier-session ID in place of the raw surface UUID.",
        "Cockpit also offers three execution tools: pane_run runs a command in a pane (paste text plus Return), pane_send_keys sends raw text verbatim with no Return appended, and palette_execute runs a command-palette entry by its id. All three also accept a termifier-session ID in surface_id, same as pane_split. Each requires in-app human approval unless auto-approve is enabled, so the call can suspend until a person acts on it; {\"status\": \"denied\"}, {\"status\": \"approval_timeout\"}, and {\"status\": \"dismissed\"} are normal, non-error results, not failures to retry — after a denied or dismissed result, do not retry the same action without new user intent.",
    ]
    ...
    /// `TermifierMCPServerLSPIntegrationTests`.
    ...
    /// for it (see `TermifierMCPServer`'s `initialize` case) — that branches
    ...
    static func buildInitializeResponse(id: JSONRPCId, peerID: UUID? = nil) -> JSONRPCResponse {
        ...
        let initResult = MCPInitializeResult(
            ...
            serverInfo: MCPServerInfo(name: "termifier-ipc", version: "1.0.0"),
