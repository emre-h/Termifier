...
// Termifier
//
// Manages reading/writing ~/.hermes/config.yaml for the Termifier IPC MCP server.
//
...
//     `termifier-ipc:` child entry is inserted under it, with comment markers
...
enum HermesConfigError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .malformedManagedBlock(let reason):
            return "Malformed Termifier-managed block in Hermes config: \(reason)"
        case .invalidScalarValue:
...
struct HermesConfigManager: Sendable {
    ...
    private static let beginLine = "# BEGIN TERMIFIER IPC (managed by Termifier, do not edit)"
    private static let endLine = "# END TERMIFIER IPC"

    /// Matches a Termifier BEGIN marker line. The line must consist of (optional
    /// indent) + `#` + (optional whitespace) + `BEGIN TERMIFIER IPC` + word boundary
    ...
    private static let beginLineRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*#\s*BEGIN TERMIFIER IPC\b[^\n]*$"#
    )

    /// Matches a Termifier END marker line.
    private static let endLineRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*#\s*END TERMIFIER IPC\s*$"#
    )

    /// Matches a `termifier-ipc:` key line at any indent (used to validate the
    /// inside of a BEGIN/END span before treating it as a real managed block).
    private static let termifierIPCKeyRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*termifier-ipc:\s*$"#
    )
    ...
    /// Enables Termifier IPC by upserting the managed block in the Hermes config.
    /// Self-heals over previously-malformed managed blocks (orphan BEGIN/END,
    /// or BEGIN/END pairs missing the `termifier-ipc:` body) by stripping anything
    ...
    static func enableIPC(port: Int, token: String, configPath: String? = nil) throws {
        ...
        let surfaceIDScalar = try yamlDoubleQuotedScalar("${TERMIFIER_SURFACE_ID}")
        let sessionIDScalar = try yamlDoubleQuotedScalar("${TERMIFIER_SESSION_ID}")
    ...
    /// Disables Termifier IPC by removing the managed block(s) from the config.
    /// Throws `.malformedManagedBlock` on orphan BEGIN, orphan END, or
    /// BEGIN/END pairs lacking the `termifier-ipc:` body, since silent removal
    ...
    /// (BEGIN line, then `termifier-ipc:` line, then END line — in that order,
    ...
    static func isIPCEnabled(configPath: String? = nil) -> Bool {
        ...
        guard normalized.contains("BEGIN TERMIFIER IPC") else { return false }
        ...
        mcp_servers:
          termifier-ipc:
            ...
            headers:
              ...
              X-Termifier-Surface-ID: \(surfaceIDScalar)
              X-Termifier-Session-ID: \(sessionIDScalar)
              X-Termifier-Agent-Kind: \(agentKindScalar)
        \(endLine)
        """
    ...
    private static func insertCaseB(
        ...
        let subBlock = [
            ...
            "\(childIndent)termifier-ipc:",
            "\(nestedIndent)url: \(urlScalar)",
            "\(nestedIndent)headers:",
            "\(deepIndent)Authorization: \(authScalar)",
            "\(deepIndent)X-Termifier-Surface-ID: \(surfaceIDScalar)",
            "\(deepIndent)X-Termifier-Session-ID: \(sessionIDScalar)",
            "\(deepIndent)X-Termifier-Agent-Kind: \(agentKindScalar)",
            "\(childIndent)\(endLine)",
        ]
    ...
    /// is BEGIN line, followed by a `termifier-ipc:` line, followed by an END
    ...
    private static func findValidManagedBlocks(in content: String) -> [Range<String.Index>] {
        ...
        for begin in beginMatches {
            ...
            // Require a `termifier-ipc:` line strictly between begin and end.
            let between = begin.upperBound..<end.lowerBound
            if firstMatch(of: termifierIPCKeyRegex, in: content, range: between) == nil {
                continue
            }
    ...
    /// `termifier-ipc:` exists in the content. Throws `.malformedManagedBlock`
    ...
    private static func validateNoMalformedBlocks(in content: String) throws {
        ...
        for (begin, end) in zip(beginMatches, endMatches) {
            ...
            let between = begin.upperBound..<end.lowerBound
            if firstMatch(of: termifierIPCKeyRegex, in: content, range: between) == nil {
                throw HermesConfigError.malformedManagedBlock("missing termifier-ipc")
            }
        }
    }
    ...
    /// `termifier-ipc:` body exists between them. Includes the trailing newline.
