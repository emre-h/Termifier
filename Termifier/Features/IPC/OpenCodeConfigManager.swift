...
// Termifier
//
// Manages reading/writing OpenCode's config files for the Termifier IPC MCP server.
//
// OpenCode uses two files under `~/.config/opencode/`:
// 1. `opencode.json`  — the MCP remote server entry (upsert `mcp.termifier-ipc`)
...
struct OpenCodeConfigManager: Sendable {
    ...
    /// Common prefix shared by all BEGIN TERMIFIER IPC markers (historical and current).
    /// Kept as a derived constant so `beginDelimiter` stays in sync with it.
    private static let beginAnchor = "<!-- BEGIN TERMIFIER IPC"

    /// BEGIN delimiter for the Termifier IPC managed block in AGENTS.md.
    /// Must match the literal the tests search for.
    private static let beginDelimiter = beginAnchor + " (managed by Termifier, do not edit) -->"

    /// Regex that matches a full BEGIN line (`<!-- BEGIN TERMIFIER IPC ... -->`), requiring
    /// the `-->` closing on the same line. This prevents `stripManagedBlocks` from
    /// anchoring on user prose that merely mentions `<!-- BEGIN TERMIFIER IPC` mid-line.
    private static let beginLinePattern = #"<!--\s*BEGIN TERMIFIER IPC[^\n]*?-->"#

    /// END delimiter for the Termifier IPC managed block in AGENTS.md.
    private static let endDelimiter = "<!-- END TERMIFIER IPC -->"
    ...
    You are connected to Termifier IPC, enabling communication with other Claude Code instances in other terminal panes.
    ...
    /// Enables the Termifier IPC MCP entry in OpenCode's `opencode.json` and appends
    ...
    /// Disables the Termifier IPC entry by removing `mcp.termifier-ipc` from `opencode.json`
    ...
    /// Returns whether the Termifier IPC MCP entry is present in `opencode.json`.
    ...
    static func isIPCEnabled(configDir: String? = nil) -> Bool {
        ...
        return mcp[termifierIPCKey] != nil
    }
    ...
    private static func upsertOpenCodeJSON(port: Int, token: String, path: String) throws {
        ...
        // Build the termifier-ipc entry fresh — no merge, guarantees stale header keys
        ...
        let termifierEntry: [String: Any] = [
            ...
            "headers": [
                ...
                "X-Termifier-Surface-ID": "{env:TERMIFIER_SURFACE_ID}",
                "X-Termifier-Session-ID": "{env:TERMIFIER_SESSION_ID}",
                "X-Termifier-Agent-Kind": AgentEntry.openCodeKind,
            ]
        ]

        var mcp = config[mcpKey] as? [String: Any] ?? [:]
        mcp[termifierIPCKey] = termifierEntry
        config[mcpKey] = mcp
    ...
    private static func removeFromOpenCodeJSON(path: String) throws {
        ...
        mcp.removeValue(forKey: termifierIPCKey)
    ...
    /// Removes all `BEGIN TERMIFIER IPC` ... `END TERMIFIER IPC` managed blocks from a string.
    ///
    ...
    /// comment terminator on the same line as `BEGIN TERMIFIER IPC`, so user prose that
    /// merely mentions `<!-- BEGIN TERMIFIER IPC` mid-line is NOT treated as a block
    ...
    private static func stripManagedBlocks(from content: String) -> String {
        ...
        // mid-line mention of `<!-- BEGIN TERMIFIER IPC` in user prose cannot
