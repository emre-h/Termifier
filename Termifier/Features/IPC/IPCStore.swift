...
actor IPCStore {
    ...
    /// current value unchanged (`TermifierMCPServer.handleRegisterPeer`
    ...
    /// when a later step fails after the fact — `TermifierMCPServer.
    ...
    /// `TermifierMCPServer` to refresh every bound peer's badge once per
