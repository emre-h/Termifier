...
// TermifierTests/IPC/MCPCockpitBridgeTests.swift for the specced contract
// this satisfies.
//
...
enum MCPCockpitBridgeError: Error, LocalizedError, Equatable {
    ...
    /// either a surface UUID or a registered termifier-session ID.
    ...
    var errorDescription: String? {
        ...
        case .appUnavailable:
            return "The Termifier app is not currently available"
        case .tabCreationFailed:
...
final class MCPCockpitBridge {
    ...
    nonisolated static let tools: [MCPTool] = [
        MCPTool(
            name: "pane_list",
            description: "List every terminal pane across all open Termifier windows. Pane identity is "
        ...
        MCPTool(
            ...
            description: "Split an existing pane, creating a new one alongside it. surface_id also "
                + "accepts a termifier-session ID in place of the raw surface UUID.",
            inputSchema: MCPRouter.schema(
                properties: [
                    "surface_id": MCPRouter.prop("string", "Surface UUID or termifier-session ID of the pane to split"),
        ...
        MCPTool(
            ...
            description: "Run a command in a pane by pasting its text and pressing Return. surface_id "
                + "also accepts a termifier-session ID in place of the raw surface UUID. Requires in-app "
            ...
            inputSchema: MCPRouter.schema(
                properties: [
                    "surface_id": MCPRouter.prop("string", "Surface UUID or termifier-session ID of the pane to run the command in"),
        ...
        MCPTool(
            ...
            description: "Send raw text to a pane verbatim, with no Return appended. surface_id also "
                + "accepts a termifier-session ID in place of the raw surface UUID. Requires in-app user "
                + "approval unless auto-approve is enabled; {\"status\":\"denied\"}, "
                + "{\"status\":\"approval_timeout\"}, and {\"status\":\"dismissed\"} are normal results "
                + "— after denied or dismissed, do not retry without new user intent.",
            inputSchema: MCPRouter.schema(
                properties: [
                    "surface_id": MCPRouter.prop("string", "Surface UUID or termifier-session ID of the pane to send text to"),
    ...
    /// as a thrown `MCPCockpitBridgeError` -- `TermifierMCPServer
    ...
    /// `title`/`cwd`/`agent_kind`/`termifier_session_id` are OMITTED
    ...
    private func paneDict(_ pane: CockpitPaneInfo) -> [String: Any] {
        ...
        if let termifierSessionID = pane.termifierSessionID { dict["termifier_session_id"] = termifierSessionID }
        return dict
    }
    ...
    private func handleTabCreate(arguments: [String: Any]) throws -> String {
        ...
        if let rawCwd = cwd {
            ...
            // matches TermifierWindowController.normalizedCwdOverride(_:)'s
    ...
    /// as a termifier-session ID registered in `sessionSurfaceMap`.
