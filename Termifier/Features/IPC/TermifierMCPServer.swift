...
@MainActor
final class TermifierMCPServer {

    static let shared = TermifierMCPServer(agentEndpointDirectory: AgentEndpointFile.defaultDirectory)
    ...
    /// `terminal_*` dispatch (not at `TermifierMCPServer` init), by which
    ...
    /// Resolves a termifier-session ID to the surface UUID currently
    /// attached to it, for `/mcp` and `/agent-event` requests whose
    /// `X-Termifier-Surface-ID` header carries a session ID rather than a
    /// raw surface UUID (persistent-session panes, see
    /// `AgentHookScript`'s `TERMIFIER_SESSION_ID` precedence). Defaults to
    ...
    /// termifier-session daemon's per-session meta map, so a later
    ...
    /// (`~/Library/Application Support/Termifier`): only
    /// `TermifierMCPServer.shared` passes that value; every test passes a
    ...
    private func routeMCP(request: HTTPRequest) async -> HTTPResponse {
        ...
        // required header below, `/mcp` predates `X-Termifier-Surface-ID`,
        ...
        // also resolves a termifier-session ID via `sessionSurfaceMap` (the
    ...
    /// Trims whitespace from `X-Termifier-Surface-ID` and parses it as a
    ...
    /// `termifier-agent-hook`'s own hook POST on the other — is parsed
    ...
    private func parseSurfaceID(from headers: [String: String]) -> UUID? {
        guard let trimmed = header(named: "X-Termifier-Surface-ID", in: headers)?
    ...
    /// Resolves the request's Termifier pane identity, shared by both
    /// `/mcp` (`routeMCP`) and `/agent-event` (`routeAgentEvent`): first
    /// a stable `X-Termifier-Session-ID` looked up in `sessionSurfaceMap`, then
    /// the legacy overloaded `X-Termifier-Surface-ID` contract: raw surface UUID
    /// first, termifier-session ID fallback. Separate headers let clients whose
    ...
    /// sends its stable termifier-session ID in `X-Termifier-Surface-ID` (see
    /// `AgentHookScript.scriptBody`'s
    /// `${TERMIFIER_SESSION_ID:-$TERMIFIER_SURFACE_ID}` fallback), so that older
    ...
    private func resolveSurfaceID(from headers: [String: String]) -> UUID? {
        if let sessionID = header(named: "X-Termifier-Session-ID", in: headers)?
        ...
        guard let raw = header(named: "X-Termifier-Surface-ID", in: headers)?
    ...
    /// Resolves `X-Termifier-Agent-Kind`, trimming whitespace and falling
    /// back to `AgentEntry.claudeCodeKind` when the header is absent OR
    /// present-but-blank (e.g. a proxy/plugin bug that sends
    /// `X-Termifier-Agent-Kind: ` with no value), rather than letting an
    ...
    private func explicitAgentKind(from headers: [String: String]) -> String? {
        guard let trimmed = header(named: "X-Termifier-Agent-Kind", in: headers)?
            .trimmingCharacters(in: .whitespaces), !trimmed.isEmpty else {
            return nil
        }
        return trimmed
    ...
    /// `routeCommandEvent` and `routeApprovalRequest`. `termifier-agent-hook`
    ...
    private func routeAgentEvent(request: HTTPRequest) async -> HTTPResponse {
        ...
        // present) into the termifier-session daemon's per-session meta so
        // a later reattach can offer to resume this conversation. A
        // no-op inside the bridge itself when `surfaceID` has no
        // tracked termifier-session (an ordinary, non-persistent pane). A
    ...
    private func routeCommandEvent(request: HTTPRequest) async -> HTTPResponse {
        ...
        guard let rawSurfaceIDHeader = header(named: "X-Termifier-Surface-ID", in: request.headers)?
        ...
        guard let surfaceID = resolveSurfaceID(from: request.headers) else {
            ...
            // unregistered termifier-session ID) is normal steady-state for a
        ...
        // and `AgentRegistry.termifierShellIntegrationReportedSurfaces`'s for
        // why a `.start` recording, not just `.end`, is needed to win a
        // same-command race against ghostty's own OSC 133 D.
        let shellReportPhase: TermifierShellReportPhase = event.phase == .end ? .commandEnd : .commandStart
        agentRegistry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: shellReportPhase)
    ...
    /// `termifier-approval-hook` script blocks on from a CLI agent's
    ...
    /// decode (413, mirrors `routeCommandEvent`) -> `X-Termifier-Agent-Kind`
    ...
    /// `X-Termifier-Agent-Kind` (anything other than
    ...
    /// session restarts, no Termifier banner shows for its tool calls, and
    ...
    private func routeApprovalRequest(request: HTTPRequest) async -> HTTPResponse {
        ...
        guard kind == AgentEntry.claudeCodeKind || kind == AgentEntry.codexKind
            ...
            // An agent CLI Termifier doesn't have a hook-response contract
        ...
        guard ApprovalHookEvent.gateIsSoleAuthority(kind: kind, permissionMode: call.permissionMode) else {
            ...
            // exactly as it would without Termifier, rather than asking the
    ...
    func startLSP() async {
        ...
        // on-disk edits outside Termifier's own writers feed back into LSP
        // synchronisation notifications.
        let fileSyncManager = FileSyncManager()
        // Default persistence store under
        // `~/Library/Application Support/Termifier/lsp/sessions.json`. Snapshots
    ...
    func start(token: String, preferredPort: Int = 41830) throws {
        ...
        for portOffset in 0..<10 {
            ...
            if resolvedPort != tryPort {
                NSLog("[TermifierMCPServer] canonical scan requested port \(tryPort) but the listener resolved to \(resolvedPort); recording the resolved port")
            }
    ...
    ///
    /// `internal` (not `private`) so `TermifierMCPServerTests` can exercise
    ...
    func bindKernelAssignedListener() -> (NWListener, Int)? {
        ...
        for attempt in 0..<5 {
            guard let probedPort = askKernelForFreeLoopbackPort() else {
                NSLog("[TermifierMCPServer] BSD fallback attempt \(attempt): kernel did not return a port")
                continue
            }
            ...
            if Int(boundPort) != probedPort {
                NSLog("[TermifierMCPServer] BSD fallback probed port \(probedPort) but the listener resolved to \(boundPort); recording the resolved port")
            }
    ...
    ///
    /// `internal` (not `private`) so `TermifierMCPServerTests` can call this
    ...
    private func startListenerAndWaitForReady(_ nl: NWListener) -> NWListener? {
        ...
        let listenerQueue = DispatchQueue(
            label: "TermifierMCPServer.listenerProbe"
        )
        nl.stateUpdateHandler = { state in
            ...
            case .ready:
                ...
                sema.signal()
            case .failed(let err):
                NSLog("[TermifierMCPServer] probe listener failed: \(err)")
    ...
    /// production wiring.
    ///
    /// Writes `agent-endpoint.json` (port/token for the termifier-agent-hook
    ...
    private func finishStart(
        ...
        do {
            ...
            NSLog("[TermifierMCPServer] failed to write agent-endpoint.json: \(error)")
        }
        ...
        self.peerRegistrationTask = Task {
            let peer = await self.store.registerPeer(name: "termifier-app", role: "review-ui")
    ...
    func stop() -> Task<Void, Never> {
        ...
        // `TermifierMCPServerCockpitToolsTests.test_serverStop_expiresPendingApprovals`.
        ...
        // existing call sites (`TermifierWindowController.disableIPC`,
    ...
    /// (more likely under load — see `TermifierMCPServerTests`'s
    ...
    ///
    /// - Parameter surfaceID: The pane's `X-Termifier-Surface-ID` header, when
    ...
    func handleJSONRPC(
        ...
        case "initialize":
            ...
            // connection (one carrying `X-Termifier-Surface-ID`). A
            ...
            //   pane's identity and inbox carry over, by design (Termifier's
    ...
    private func handleToolCall(
        ...
        // every termifier-ipc messaging tools/call request, rather than each
    ...
    /// The termifier-ipc tools whose effects can change a bound peer's unread
    ...
    /// `TERMIFIER_SURFACE_ID` env var) resolves to the SAME bound surface, so
    ...
    private func handleRegisterPeer(
        ...
        // elsewhere in this file (`parseSurfaceID`, the `X-Termifier-Agent-Kind`
