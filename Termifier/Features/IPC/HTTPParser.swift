...
struct HTTPParser {
    ...
    static func parse(_ data: Data) throws -> HTTPRequest {
        ...
        // `completeness(of:)` (`TermifierMCPServer`'s pre-parse
    ...
    /// Shared by `parse(_:)` and `TermifierMCPServer`'s pre-parse
    ...
    /// `TermifierMCPServer.receiveUntilComplete` across possibly many
