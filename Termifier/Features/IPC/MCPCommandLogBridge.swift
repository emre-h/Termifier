...
/// to `LocalizedError` so `TermifierMCPServer.handleTerminalToolCall` can
...
final class MCPCommandLogBridge {
    ...
    nonisolated static let tools: [MCPTool] = [
        MCPTool(
            ...
            description: "List commands tracked for a terminal surface, oldest-first. surface_id also "
                + "accepts a termifier-session ID in place of the raw surface UUID. Each entry carries "
            ...
            inputSchema: MCPRouter.schema(
                properties: [
                    "surface_id": MCPRouter.prop("string", "Surface UUID or termifier-session ID to list commands for"),
        ...
        MCPTool(
            ...
            description: "Block until a running command finishes (or the timeout elapses) and return its "
                + "record. surface_id also accepts a termifier-session ID. Omit command_id to wait on the "
            ...
            inputSchema: MCPRouter.schema(
                properties: [
                    "surface_id": MCPRouter.prop("string", "Surface UUID or termifier-session ID to await a command on"),
    ...
    /// `TermifierMCPServer.handleTerminalToolCall` catches it and builds the
