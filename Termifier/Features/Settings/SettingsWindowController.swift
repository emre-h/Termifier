...
class SettingsWindowController: NSWindowController {
    ...
    private func sectionHeading(for row: SettingsRow) -> SectionHeading? {
        ...
        case .lspAutoInstall:
            return SectionHeading(
                title: "Auto-Install",
                subtitle: "Termifier hosts language servers and exposes them to AI agents over MCP. When a server is missing, Termifier can install it automatically."
            )
        ...
        case .agentHookApproval:
            return SectionHeading(
                ...
                subtitle: "Routes Claude Code and Codex permission prompts, all always-approve Grok tool calls, and every pi tool call to the Termifier approval banner. Off = agents decide alone, and pi, which has no prompt of its own, just runs the call."
            )
    ...
    private func showConfigFileError(_ error: ConfigFileOpenError, opener: SystemConfigFileOpener) {
        ...
        case .isSymlink:
            alert.informativeText = "The config path is a symbolic link. For security, Termifier will not follow symlinks."
    ...
    /// (`TermifierShellIntegrationEnvironment.remove`); the `/command-event`
...
private final class GhosttyConfigHelpViewController: NSViewController {
    ...
    override func loadView() {
        ...
        let subtitle = NSTextField(wrappingLabelWithString: "Termifier reads ~/.config/ghostty/config. Most settings are hot-reloaded when you save the file.")
        ...
        let managedLabel = NSTextField(wrappingLabelWithString: "The following keys are managed by Termifier for the Glass UI effect and will be overridden:")
