...
// Termifier
//
// Queries the termifier-session daemon's state for a given session ID.
...
/// spawning a real `termifier-session` process.
protocol SessionDaemonClientProtocol: Sendable {
    ...
    /// (`termifier-session meta set <id> <key>=<value>`). Added for
    ...
    /// `termifier-session`'s own `remote-install` subcommand. Returns `nil`
...
/// (`Termifier/Helpers/AwaitBridge.swift`): this type races two independent,
...
extension SessionDaemonClientProtocol {
    ...
    /// a hung `termifier-session` daemon.
    ///
    ...
    /// hung `termifier-session` daemon could stall
    /// `SessionReconnectCoordinator.childExited`'s reconnect decision
...
/// Production client: shells out to the bundled `termifier-session` binary
/// via `LSPCommandRunner` (the same `Process`-driving abstraction
/// `LSPInstaller` uses), rather than a bespoke `Process` invocation —
/// so `termifier-session` subprocess calls inherit `SystemCommandRunner`'s
...
final class SessionDaemonClient: SessionDaemonClientProtocol, Sendable {
    ...
    /// The `["--runtime-dir", "<root>/.termifier/run"]` pair every
    ...
    /// global `--runtime-dir` flag (`termifier-session/crates/cli/src/cli.rs:15-16`,
    ...
    /// `<real $HOME>/.termifier/run`, identical to the Rust CLI's own
    ...
    init(
        ...
        self.runtimeDirArgument = ["--runtime-dir", rootResolver.resolve() + "/.termifier/run"]
    ...
    /// `termifier-session kill` subprocess, racing it against a timeout and
    ...
    /// `termifier-session` binary's own `remote-install` subcommand, with
    ...
    /// Kills a REMOTE session by shelling `ssh -- <host>
    /// "$HOME/.termifier/bin/termifier-session kill '<sessionID>'"`, with `ssh`
    ...
    /// only ever needs the `ssh` binary -- the local termifier-session
    ...
    func killRemote(host: String, sessionID: String) async {
        ...
        let remoteCommand = "$HOME/.termifier/bin/termifier-session kill \(SessionCommandSynthesizer.shSafeToken(sessionID))"
        await Task {
            _ = try? await commandRunner.run(
...
/// --json` output — see `termifier-session/crates/proto/src/control.rs`).
