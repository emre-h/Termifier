...
enum SessionCommandSynthesizer {
    ...
    /// Builds `<binaryPath> --runtime-dir <root>/.termifier/run --state-dir
    /// <root>/.termifier/state attach <sessionID> --create --cwd <cwd>`.
    /// `sessionID` is positional, matching the CLI's `AttachArgs`
    /// (`termifier-session/crates/cli/src/cli.rs`), which has no `--id` flag.
    ///
    ...
    /// `execCommand`, for the default `.shell`-variant command that Termifier
    ...
    /// (`termifier-session/crates/cli/src/cli.rs:15-20`, `global = true`),
    /// which `attach` both accepts and forwards verbatim to the daemon it
    /// auto-spawns (`termifier-session/crates/cli/src/commands/attach.rs:193-224`).
    /// With no `rootResolver` override (real production use), the
    /// composed paths equal `<real $HOME>/.termifier/{run,state}`, identical
    ...
    /// namely `termifier-session` itself, directly, with no intermediate
    /// wrapper process left surviving, so the ghostty surface's child
    /// process *is* `termifier-session attach`, not a shell wrapping it.
    ...
    static func attachCommand(
        ...
        return "\(shSafeToken(binaryPath)) --runtime-dir \(shSafeToken(root + "/.termifier/run")) --state-dir \(shSafeToken(root + "/.termifier/state")) attach \(shSafeToken(sessionID)) --create\(initialGridArguments) --cwd \(shSafeToken(cwd))"
    }

    /// Builds the attach command for re-attaching to an *existing*
    /// `sessionID` (restore, reconnect) using `resolver` to find the
    /// termifier-session binary — as opposed to `SessionSpawnPlanner.plan`,
    ...
    /// command around a hardcoded `"termifier-session"` literal that may
    ...
    /// `TermifierWindowController.performReconnect`.
    ///
    ...
    /// `remoteCommand` is the single shell-safe word `$HOME/.termifier/bin/
    /// termifier-session attach <sessionID> --create --cwd <cwd>`, with
    ...
    ///
    /// WHY `$HOME`, NOT `~`: `$HOME/.termifier/bin/termifier-session` is left as
    /// a literal, UNQUOTED bareword within `remoteCommand`, so the
    /// REMOTE shell's own parameter expansion resolves it against the
    /// REMOTE `$HOME`. A single-quoted `'~/.termifier/bin/termifier-session'`
    ...
    static func remoteAttachCommand(
        ...
        let remoteCommand = "$HOME/.termifier/bin/termifier-session attach \(shSafeToken(sessionID)) --create\(initialGridArguments) --cwd \(shSafeToken(cwd))"
