...
// kill the underlying termifier-session. Extracted as an explicit, pure
...
enum SessionCloseKillPolicy {
    ...
    /// The single shared predicate both
    ...
    /// termifier-session too, rather than leaving it running headless.
    ...
    /// (no reachable path calls detach mid-reconnect-swap),
    /// but structurally required so detach isn't
