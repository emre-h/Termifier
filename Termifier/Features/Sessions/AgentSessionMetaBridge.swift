...
// (`AgentEvent.sessionID`) into the termifier-session daemon's per-session
// meta map, so a later reattach (`SessionResumePlanner`) can look it
// up and offer to resume the same CLI conversation.
// `TermifierMCPServer.routeAgentEvent` is meant to route every hook event
...
final class AgentSessionMetaBridge {
    ...
    /// termifier-session tracking `surfaceID`, under
    /// `SessionResumePlanner.encodeMetaKey(kind: agentKind)`. A no-op
    /// when `surfaceID` has no tracked termifier-session (an ordinary,
