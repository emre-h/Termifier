...
// `termifier-session attach --create` command that survives ghostty
...
/// Supplying this to `termifier-session attach` prevents the embedded
...
struct SessionSpawnContext: Sendable, Equatable {
    /// The resolved spawn cwd handed down by `TermifierWindowController
...
enum SpawnPlan: Sendable, Equatable {
    ...
    case passthrough
    /// Launch `termifier-session attach --create` for `sessionID`, running
    ...
    /// caller applying the plan (see `TermifierWindowController
...
enum SessionSpawnPlanner {
    ...
    /// can't find the termifier-session binary at all (a `nil` resolution
    /// must not fall back to a hardcoded `"termifier-session"` literal that
    ...
    /// (`TermifierWindowController.createManagedSurface`'s
    ...
    /// lives on the remote machine, so the LOCAL termifier-session binary's
