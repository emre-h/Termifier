...
//   - Resources/session-remote/<x86_64|aarch64>/termifier-session
...
protocol SessionRemotePayloadResolverProtocol: Sendable {
    /// The bundled cross-compiled `termifier-session` payload's absolute
...
struct SessionRemotePayloadResolver: SessionRemotePayloadResolverProtocol {
    ...
    func payloadPath(forArch arch: String) -> String? {
        bundle.url(forResource: "termifier-session", withExtension: nil, subdirectory: "session-remote/\(arch)")?.path
    }
