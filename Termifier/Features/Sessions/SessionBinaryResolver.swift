...
// Termifier
//
// Resolves the bundled `termifier-session` binary's path, shared by both
...
protocol SessionBinaryResolverProtocol: Sendable {
    /// The bundled `termifier-session` binary's absolute path, or `nil` if
...
/// Production resolver: `TERMIFIER_SESSION_BIN` env override (dev workflow
/// / test injection), then the bundled `Resources/bin/termifier-session`
/// resource (see `project.yml`'s "Bundle Session Daemon"
/// postBuildScript), then `nil`. Deliberately has no third fallback to
/// a bare `"termifier-session"` literal resolved via `PATH` — a review
...
struct SessionBinaryResolver: SessionBinaryResolverProtocol {
    ...
    func resolve() -> String? {
        if let override = environment["TERMIFIER_SESSION_BIN"], !override.isEmpty {
            return override
        }
        return bundle.url(forResource: "termifier-session", withExtension: nil, subdirectory: "bin")?.path
    }
}
