...
// Termifier
//
// Reference to a termifier-session (persistent PTY session, see
// `termifier-session/`) attached to a terminal leaf surface. Stored per leaf
...
struct SessionRef: Codable, Equatable, Sendable {
    /// The termifier-session ULID (`SessionSpec.id` / `SessionInfo.id` on the
    ...
    /// (e.g. `"claude-code"`), so a resumed termifier-session can also offer
...
extension SessionRef {
    ...
    /// rather than handed to `termifier-session attach`, which would
