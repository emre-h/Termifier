...
enum SessionReconnectDecision: Sendable, Equatable {
    ...
    case closePane
    /// Re-run `termifier-session attach --create` for `sessionID`. `attempt`
    ...
    /// — never confirmed exited — so the underlying `termifier-session` may
...
final class SessionReconnectCoordinator {
    ...
    /// Resolves which termifier-session a surface belongs to. Tests inject
    ...
    /// .sessionStateBounded(id:)` queries the LOCAL termifier-session daemon
    ...
    /// lives entirely on the remote host (see `TermifierWindowController
