...
// Termifier
//
// Resolves the on-disk session root ($HOME/.termifier: run/sessiond.sock,
// state/) that the Rust termifier-session daemon/CLI derives from the
// literal HOME env var at process start (see
// termifier-session/crates/cli/src/commands/mod.rs:74's default_home_subdir).
...
// termifier-session binary path -- so every consumer must receive its
