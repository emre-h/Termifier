...
// Termifier
//
// Installs Termifier's own zsh/fish command-lifecycle shell integration
// scripts into a Termifier-owned directory tree -- never a user rc file (the
// command-log plan's explicit "the user's own rc files are never edited"
// constraint). TermifierShellIntegrationEnvironment points ZDOTDIR /
...
enum ShellIntegrationInstaller {
    ...
    /// Default install root: `~/Library/Application Support/Termifier/shell-integration`.
    ...
    /// TERMIFIER_ZSH_ZDOTDIR (existence-checked, not merely non-empty --
    /// unset means fall back to $HOME), (2) sourcing termifier.zsh (hook
    ...
    /// resumes exactly as if Termifier were never involved.
    ///
    ...
    /// for `TERMIFIER_ZSH_ZDOTDIR` and `ghostty-integration` swapped for
    /// `termifier.zsh`. Quoted throughout (`'builtin' 'export' ...`) for the
    ...
    /// integration` itself only in the `always` block; `termifier.zsh` has
    /// no such dependency on the user's `.zshenv`, so it's the `try`
    /// step here, with sourcing the user's real `.zshenv` moved to
    /// `always` -- guaranteeing that step runs even if `termifier.zsh`
    /// itself errors, so an error in the Termifier script never breaks the
    /// user's own shell startup chain.
    static let zshenvBody = """
    if [[ -n "${TERMIFIER_ZSH_ZDOTDIR+X}" ]]; then
        'builtin' 'export' ZDOTDIR="$TERMIFIER_ZSH_ZDOTDIR"
        'builtin' 'unset' 'TERMIFIER_ZSH_ZDOTDIR'
    else
...

    {
        if [[ -o 'interactive' ]]; then
            ...
            # termifier.zsh is found regardless of how this file itself was
            # reached (mirrors ghostty's own %x:A:h idiom).
            'builtin' 'typeset' _termifier_file="${${(%):-%x}:A:h}"/termifier.zsh
            [[ ! -r "$_termifier_file" ]] || 'builtin' 'source' '--' "$_termifier_file"
            'builtin' 'unset' '_termifier_file'
        fi
    } always {
        ...
        'builtin' 'typeset' _termifier_user_zshenv=${ZDOTDIR-$HOME}"/.zshenv"
        [[ ! -r "$_termifier_user_zshenv" ]] || 'builtin' 'source' '--' "$_termifier_user_zshenv"
        'builtin' 'unset' '_termifier_user_zshenv'
    }
    """

    /// Installed at `<root>/zsh/termifier.zsh`. Registers preexec/precmd
    ...
    /// POST start/end command-lifecycle events to the local Termifier IPC
    ...
    /// (`TermifierMCPServer.routeCommandEvent`), so a completed foreground curl
    ...
    /// Fails open
    /// (TERMIFIER_SURFACE_ID and TERMIFIER_SESSION_ID both unset, or
    ...
    /// `_termifier_cmd_active` is never set for precmd to find).
    ///
    /// `_termifier_post`'s agent-endpoint.json sed-extraction is the same
    ...
    static let termifierZshBody = """
    [[ -o interactive ]] || return 0
    [[ -z "${_TERMIFIER_HOOKS_LOADED:-}" ]] || return 0
    'builtin' 'typeset' '-g' '_TERMIFIER_HOOKS_LOADED=1'

    [[ -n "${TERMIFIER_SURFACE_ID:-}" || -n "${TERMIFIER_SESSION_ID:-}" ]] || return 0

    'builtin' 'zmodload' 'zsh/datetime' 2>/dev/null || return 0

    _termifier_post() {
        local endpoint_file="$HOME/Library/Application Support/Termifier/agent-endpoint.json"
    ...
    _termifier_preexec() {
        'builtin' 'typeset' '-g' '_termifier_cmd_active=1'
        'builtin' 'typeset' '-g' _termifier_cmd_id="$$-${EPOCHREALTIME/./}-$RANDOM"
        ...
        typeset -i _termifier_ts
        _termifier_ts=$(( EPOCHREALTIME * 1000 ))
        _termifier_post "{\\"phase\\":\\"start\\",\\"cmd_id\\":\\"$_termifier_cmd_id\\",\\"command_b64\\":\\"$command_b64\\",\\"cwd_b64\\":\\"$cwd_b64\\",\\"ts\\":$_termifier_ts}"
    }

    _termifier_precmd() {
        local code=$?
        [[ -n "${_termifier_cmd_active:-}" ]] || return 0
        unset _termifier_cmd_active
        typeset -i _termifier_ts
        _termifier_ts=$(( EPOCHREALTIME * 1000 ))
        _termifier_post "{\\"phase\\":\\"end\\",\\"cmd_id\\":\\"$_termifier_cmd_id\\",\\"exit_code\\":$code,\\"ts\\":$_termifier_ts}"
    }

    'builtin' 'typeset' '-ag' 'preexec_functions'
    preexec_functions+=('_termifier_preexec')
    'builtin' 'typeset' '-ag' 'precmd_functions'
    precmd_functions+=('_termifier_precmd')

    return 0
    """

    /// Installed at `<root>/fish/vendor_conf.d/termifier-integration.fish`.
    ...
    /// fish's own shipped documentation) mirroring termifier.zsh's shape:
    /// same fail-open guard, same agent-endpoint.json sed-extraction,
    /// same bounded-synchronous curl (`--connect-timeout 0.1 --max-time
    /// 0.3`; see termifierZshBody's doc for why the start POST must be
    ...
    /// on duration.
    ///
    /// A second tradeoff, in `_termifier_postexec`: unlike zsh, whose native
    ...
    /// `_termifier_postexec` therefore consults fish's own `jobs` builtin --
    ...
    /// there is no real exit code yet for `_termifier_postexec` to report.
    ...
    /// available to `_termifier_postexec` can recover a value fish itself
    /// never computed.
    static let fishIntegrationBody = """
    status --is-interactive; or return 0

    if not set -q TERMIFIER_SURFACE_ID; and not set -q TERMIFIER_SESSION_ID
        return 0
    end

    function _termifier_post --argument-names body
        set -l endpoint_file "$HOME/Library/Application Support/Termifier/agent-endpoint.json"
        ...
        set -l surface_id "$TERMIFIER_SESSION_ID"
        test -n "$surface_id"; or set surface_id "$TERMIFIER_SURFACE_ID"
        command curl -s --connect-timeout 0.1 --max-time 0.3 \\
            -X POST \\
            -H "Authorization: Bearer $token" \\
            -H "X-Termifier-Surface-ID: $surface_id" \\
            -H 'Content-Type: application/json' \\
            --data-binary "$body" \\
            "http://127.0.0.1:$port/command-event" > /dev/null 2>&1; or return 0
    end

    function _termifier_preexec --on-event fish_preexec
        ...
        set -g _termifier_cmd_active 1
        set -g _termifier_cmd_id "$fish_pid-"(date +%s000)"-"(random)
        ...
        _termifier_post "{\\"phase\\":\\"start\\",\\"cmd_id\\":\\"$_termifier_cmd_id\\",\\"command_b64\\":\\"$command_b64\\",\\"cwd_b64\\":\\"$cwd_b64\\",\\"ts\\":$ts}"
    end

    function _termifier_postexec --on-event fish_postexec
        set -l code $status
        set -q _termifier_cmd_active; or return 0
        set -e _termifier_cmd_active
        ...
        set -l ts (math (date +%s) "*" 1000)
        _termifier_post "{\\"phase\\":\\"end\\",\\"cmd_id\\":\\"$_termifier_cmd_id\\",\\"exit_code\\":$code,\\"ts\\":$ts$suspended_field}"
    end
    """
    ...
    /// `<directory>/zsh/termifier.zsh`.
    static func termifierZshPath(in directory: URL) -> URL {
        directory.appendingPathComponent("zsh").appendingPathComponent("termifier.zsh")
    }

    /// `<directory>/fish/vendor_conf.d/termifier-integration.fish`.
    static func fishIntegrationPath(in directory: URL) -> URL {
        directory.appendingPathComponent("fish")
            ...
            .appendingPathComponent("termifier-integration.fish")
    }
    ...
    /// TermifierShellIntegrationEnvironment.apply(rootDirectory:).
    static func install(toDirectory directory: URL) throws -> URL {
        ...
        try write(termifierZshBody, to: termifierZshPath(in: directory))
    ...
    static func remove(fromDirectory directory: URL) throws {
        for path in [zshenvPath(in: directory), termifierZshPath(in: directory), fishIntegrationPath(in: directory)] {
    ...
    static func isInstalled(inDirectory directory: URL) -> Bool {
        ...
        return fm.fileExists(atPath: zshenvPath(in: directory).path)
            && fm.fileExists(atPath: termifierZshPath(in: directory).path)
