...
// Termifier
//
// Coordinates installing/removing termifier-agent-hook plus each agent CLI's
...
// the tools Termifier supports, collecting results independently so one
...
struct AgentHooksResult: Sendable {
    ...
    /// unlike the other tools it reaches Termifier through this one axis
...
struct AgentHooksCoordinator: Sendable {
    ...
    /// Installs `termifier-agent-hook` and `termifier-approval-hook` once, then
    ...
    /// tool that already has Termifier's own hooks/plugin installed --
    ...
    /// scope creep: enabling IPC for one CLI must never spread Termifier's
    /// hooks to a second CLI the user only installed afterward, and a
    /// user who removed Termifier's entries from one tool by hand must never
    ...
    /// above.
    ///
    /// When none of the tools already have Termifier's hooks, returns
    /// every tool `.skipped` WITHOUT installing the shared
    /// `termifier-agent-hook`/`termifier-approval-hook` scripts at all -- a user
    ...
    private static func removeClaudeCode() -> ConfigStatus {
        ...
        // tool present"), removal must check whether Termifier's own hooks are
    ...
    private static func removeCodex() -> ConfigStatus {
        ...
        // Termifier's managed block is actually installed, not just whether
        // ~/.codex exists.
    ...
    private static func removeGrok() -> ConfigStatus {
        // See removeClaudeCode's comment: removal must check whether
        // Termifier's own hook file is actually installed, not just whether
        // ~/.grok exists.
    ...
    /// whether the shared `termifier-agent-hook` scripts installed cleanly:
    ...
    private static func removePi() -> ConfigStatus {
        // See removeClaudeCode's comment: removal must check whether
        // Termifier's own extension file is actually installed, not just
    ...
    /// Status for Claude Code / Codex / Grok when the shared `termifier-agent-hook`
    ...
    /// `.skipped` when this tool didn't already have Termifier's hooks (it
