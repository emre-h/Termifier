...
enum AgentSource: Sendable, Equatable {
    ...
    /// Termifier's own hooks/title-heuristic pipeline. Stored in
...
extension AgentSource {
    ...
    /// reports about itself, rather than evidence Termifier inferred by
...
struct AgentEntry: Identifiable, Sendable, Equatable {
    ...
    /// The Termifier surface (if any) an `.external` entry's row should
    /// focus when clicked -- an external entry's own `surfaceID` is not
    /// necessarily a Termifier `SurfaceRegistry` UUID, so this is a
    /// separate, optional pointer to a Termifier-hosted surface that
...
extension AgentEntry {
    ...
    /// and `OpenCodePluginManager`'s `X-Termifier-Agent-Kind` header for
    ...
    /// `OpenCodePluginManager.scriptBody`'s `X-Termifier-Agent-Kind` header —
    ...
    /// argument and `GrokConfigManager`'s `X-Termifier-Agent-Kind` header
    /// both resolve to this.
    static let grokKind = "grok"
    /// pi's `kind`. Keep in sync with the `"pi"` literal in
    /// `PiExtensionManager.scriptBody`'s `X-Termifier-Agent-Kind` header:
...
struct AgentEvent: Sendable, Equatable {
    ...
    let cwd: String?
    let message: String?
    /// For a `PreToolUse` event whose `tool_name` is one of Termifier's own
    /// `mcp__termifier-ipc__*` tools, this surface's own peer ID (extracted
    ...
    /// `mcp__termifier-ipc__register_peer`, the peer ID the server just
    ...
    private static let fromKeyToolNames: Set<String> = [
        "mcp__termifier-ipc__send_message", "mcp__termifier-ipc__broadcast",
    ]
    ...
    private static let peerIDKeyToolNames: Set<String> = [
        "mcp__termifier-ipc__receive_messages",
    ]
    ...
    /// `termifier-ipc` tool call that doesn't already know its own peer ID
    /// going in; the server generates it, so it's only available once
    /// the call has returned.
    private static let registerPeerToolName = "mcp__termifier-ipc__register_peer"
