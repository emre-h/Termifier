...
// Termifier
//
// Manages `~/.config/opencode/plugins/termifier-agent-monitor.js`, a
...
// preserve here: the plugin file is entirely Termifier's own, so install is a
...
enum OpenCodePluginManager: Sendable {
    ...
    static let fileName = "termifier-agent-monitor.js"

    /// OpenCode plugin API: `export default async ({ directory }) => ({
    /// event: async ({ event }) => {...} })`. Forwards OpenCode's plugin
    /// events to Termifier's local Agent Monitor IPC endpoint, normalized to
    /// the same `hook_event_name` / `session_id` / `cwd` shape Claude
    /// Code's hooks send (`AgentEvent.decode`), with an
    /// `X-Termifier-Agent-Kind: opencode` header so the server attributes the
    /// entry to OpenCode rather than Claude Code.
    static let scriptBody: String = """
    // termifier-agent-monitor.js
    // Installed by Termifier (OpenCodePluginManager) — do not edit by hand;
    // reinstalling Termifier's AI Agent IPC support overwrites this file.
    //
    // Forwards OpenCode plugin events to Termifier's local Agent Monitor IPC
    // endpoint. agent-endpoint.json (port/token) is cached by mtime and
    // only re-read/re-parsed when the file actually changes, so a Termifier
    ...
    export default async ({ directory }) => {
      // Not launched from a Termifier pane (e.g. a plain terminal running
      // `opencode`) — register no hooks at all. A persistent pane's
      // TERMIFIER_SESSION_ID remains stable when its Ghostty surface changes,
      // so prefer it over the launch-time surface UUID.
      const termifierPaneID =
        process.env.TERMIFIER_SESSION_ID || process.env.TERMIFIER_SURFACE_ID;
      if (!termifierPaneID) {
        return {};
      }
      ...
      // checked in TermifierMCPServer.routeAgentEvent and AgentStateResolver
        ...
        try {
          ...
          await fetch(`http://127.0.0.1:${endpoint.port}/agent-event`, {
            ...
            headers: {
              ...
              "X-Termifier-Agent-Kind": "opencode",
    ...
    /// `<pluginsDirectory>/plugins/termifier-agent-monitor.js`.
