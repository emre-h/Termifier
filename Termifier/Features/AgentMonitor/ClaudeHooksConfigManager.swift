...
// termifier-agent-hook lifecycle hook. Mirrors ClaudeConfigManager's API shape
...
struct ClaudeHooksConfigManager: Sendable {
    ...
    /// The 10 hook events Termifier installs a command entry for, and the
    ...
    /// `termifier-agent-hook`'s subscription so the hook actually fires.
    ...
    /// Merges Termifier's 10 hook entries into `configPath`'s `"hooks"` section,
    /// preserving the user's own existing hook entries and unrelated
    /// top-level keys. Idempotent: re-running replaces Termifier's own prior
    /// entries rather than duplicating them.
    ///
    /// `PermissionRequest` alone gets a second Termifier-owned command entry:
    ...
    /// call until Termifier's own `/approval-request` long-poll resolves.
    ...
    static func installHooks(scriptPath: String, approvalScriptPath: String, configPath: String? = nil) throws {
        ...
        for target in targetEvents {
            ...
            guard let groups = existingValue as? [[String: Any]] else {
                // The existing value for this event has a shape Termifier
                // doesn't recognize (hand-edited, or a future hooks
                // format) — skip installing Termifier's own entry for this
        ...
        // overwrite the user's original pre-Termifier backup with a
        // Termifier-written one the moment nothing had actually changed.
    ...
    /// Removes only Termifier's own command entries (identified by the
    /// `termifier-agent-hook` command path) from `configPath`'s `"hooks"`
    ...
    /// Whether Termifier's own command entry is present for at least one of the
    ...
    /// The Termifier-owned command entries for one target event:
    ...
    /// Builds the matcher-group Termifier installs for one target event:
    ...
    /// Removes Termifier's own command entries from every group in `groups`,
    ...
    /// A command entry is Termifier's own when its `command` path's last
    /// component is `termifier-agent-hook` or `termifier-approval-hook` —
