...
// Termifier
//
// Manages `~/.grok/hooks/termifier.json`, a Grok hook file Grok auto-loads
// and always trusts (global hooks under `~/.grok/hooks/` need no
// folder-trust grant). Grok merges every `.json` file in that directory,
// so this one is entirely Termifier's own: install is a plain whole-file
...
// `termifier-agent-hook` state ping, and the synchronous
// `termifier-approval-hook` gate whose stdout becomes Grok's own decision.
...
// mode; which of those Termifier actually holds for a human is decided
...
struct GrokHooksConfigManager: Sendable {
    ...
    /// Termifier's own file inside `~/.grok/hooks/`. Grok merges every
    /// `.json` file in that directory, so a dedicated name keeps Termifier's
    /// entries from ever colliding with a user's own hook file.
    static let fileName = "termifier.json"

    // MARK: - Public API

    /// Writes the Termifier-owned hook file at `configPath`, creating its
    ...
    /// Deletes the Termifier-owned hook file. A no-op when it does not
    ...
    /// Whether the Termifier-owned hook file exists and names
    /// `AgentHookScript.fileName` in at least one handler command. A file
    /// at Termifier's own path naming no Termifier script is somebody else's, so
