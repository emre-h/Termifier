...
enum AgentSignal: Sendable {
    ...
    /// Termifier's own shell integration reported something for the surface.
    ...
    /// `TermifierShellReportPhase` and `resolveTermifierShellIntegrationReported`.
    case termifierShellIntegrationReported(phase: TermifierShellReportPhase)
...
/// Termifier's own zsh/fish shell integration, or ghostty's OSC 133 fallback.
/// Only `.ghostty` consults `ResolverContext.termifierShellIntegrationSeen`
/// to defer to the more trustworthy Termifier-sourced signal once it has
/// ever reported on the surface.
enum PaneCommandOrigin: Sendable, Equatable {
    case termifierShellIntegration, ghostty
}

/// The phase of a `CommandEvent` Termifier's own shell integration reported,
/// in the resolver's own vocabulary -- kept separate from
/// `CommandEvent.Phase` (`Features/CommandLog/CommandEvent.swift`) so
/// this resolver stays free of the CommandLog feature; `TermifierMCPServer
...
/// that answer to `resolveTermifierShellIntegrationReported`.
enum TermifierShellReportPhase: Sendable, Equatable {
...
/// whether a ghostty pane-exit signal deferred to Termifier's own shell
...
struct AgentEvidence: Sendable, Equatable {
    ...
    /// When ghostty's own pane-exit signal was deferred to Termifier's
    /// shell integration and Termifier has not answered it yet. Nil once
    /// Termifier reports anything for the surface, and nil whenever no
    /// deferral is outstanding. The staleness sweep settles a row whose
    /// deferred signal has gone unanswered: ghostty and Termifier describe
    /// the same command completing and arrive milliseconds apart, so an
    /// unanswered deferral means Termifier's report was lost, and without
    /// this the row could never settle by either route.
    var deferredGhosttyFinishAt: Date? = nil

    /// When Termifier's own shell integration last reported a command's END
    /// for the surface. Read only to decide whether a ghostty deferral
    /// arriving right now is worth recording: a suspend that Termifier just
    ...
    /// `resolveTermifierShellIntegrationReported`.
    var lastTermifierCommandEndAt: Date? = nil

    var isEmpty: Bool {
        screenMissStreak == 0 && blockedSince == nil
            && deferredGhosttyFinishAt == nil && lastTermifierCommandEndAt == nil
    }
    ...
    /// hook event or an MCP connection. Both supersede what Termifier
    ...
    /// answer. What Termifier's own shell integration last reported is
...
struct ResolverContext: Sendable {
    /// Whether `TermifierMCPServer`'s IPC listener is currently running --
    /// gates the `.title` clause's row-creation branch the same way
    /// `TermifierWindowController.pollScreenClassificationIfAgentsSidebarVisible`
    ...
    /// Whether Termifier's own shell integration has ever reported on THIS
    /// surface (`AgentRegistry.termifierShellIntegrationReportedSurfaces`).
    /// Consulted only by a `.paneCommandFinished` signal whose origin is
    /// `.ghostty`.
    var termifierShellIntegrationSeen: Bool
}
...
struct AgentResolution: Sendable {
    ...
    /// its own guards -- `TermifierMCPServer.routeCommandEvent` gates
    /// `ApprovalInboxStore.expireForSurface` on it.
    let didSettle: Bool
    /// A peer ID this signal reported for the surface, if any -- a
    /// termifier-ipc tool call's own self-reported peer. `nil` for every
...
enum AgentStateResolver {
    ...
    /// evidence of a genuine new session Termifier missed the `SessionStart`
    ...
    /// How recently Termifier's own shell integration must have reported a
    /// command's END (`AgentEvidence.lastTermifierCommandEndAt`) for a
    ...
    /// order where Termifier reports FIRST that needs this: without it a
    /// suspend Termifier already answered would record a deferral nothing
    /// can ever clear, and `resolveStaleSweep` would retire a job that is
    /// merely stopped.
    private static let termifierReportGraceWindow: TimeInterval = 2

    /// How long a deferred ghostty pane-exit signal may go unanswered by
    /// Termifier's own shell integration before `resolveStaleSweep` settles
    /// the row on it. Well past the milliseconds the two signals for one
    /// command are actually separated by, so only a Termifier report that
    ...
    static func resolve(
        ...
        case .termifierShellIntegrationReported(let phase):
            resolution = resolveTermifierShellIntegrationReported(phase: phase, evidence: evidence, now: now)
    ...
    /// deferred to Termifier's own shell integration and Termifier never
    ...
    /// outstanding this long means Termifier's own report was lost -- the
    /// shell integration body swallows a failed POST -- and nothing else
    /// will ever settle this row: Termifier never reports the end, and
    ...
    /// the sweep is Termifier's own inference rather than a report from the
    ...
    /// expires a surface's pending approvals -- only a Termifier-sourced one
    /// over `/command-event` does (`TermifierMCPServer.routeCommandEvent`).
    ...
    // MARK: Termifier Shell Integration Report Signal

    /// Records that Termifier's own shell integration reported the END of a
    ...
    /// untouched: neither stamping `lastTermifierCommandEndAt` nor clearing
    /// `deferredGhosttyFinishAt`. A deferral can only be answered by the
    /// fact it is a question about: did the foreground job end.
    private static func resolveTermifierShellIntegrationReported(
        phase: TermifierShellReportPhase,
        evidence: AgentEvidence,
        now: Date
    ) -> AgentResolution {
        ...
        var updated = evidence
        updated.lastTermifierCommandEndAt = now
    ...
    /// of a session Termifier missed the start of.
    ...
    /// false` -- once `context.termifierShellIntegrationSeen` is `true`:
    /// ghostty's own OSC 133 fallback has no way to positively detect a
    /// suspend the way Termifier's own zsh/fish shell integration can
    ...
    /// entirely. `origin == .termifierShellIntegration` never consults it: a
    ...
    /// `resolveStaleSweep` can settle the row if Termifier never answers it
    /// -- see that clause and the field's own doc comment. The one case
    /// that records nothing is a deferral arriving within
    /// `termifierReportGraceWindow` of Termifier's own last command-END report
    /// (`AgentEvidence.lastTermifierCommandEndAt`): Termifier has already spoken
    ...
    ///
    /// Every path with `origin == .termifierShellIntegration` clears the
    ...
    /// return before ever inspecting `current` -- `TermifierMCPServer
    ...
    private static func resolvePaneCommandFinished(
        ...
        if origin == .ghostty, context.termifierShellIntegrationSeen {
            var updated = evidence
            let termifierJustReported = evidence.lastTermifierCommandEndAt.map {
                now.timeIntervalSince($0) < termifierReportGraceWindow
            } ?? false
            if !termifierJustReported {
                updated.deferredGhosttyFinishAt = now
            }
            return .keep(evidence: updated)
        }

        // Termifier has now answered for this surface, on every path below:
    ...
    /// since `TermifierMCPServer.stop()` unconditionally clears every native
    ...
    /// A row for a pane Termifier is watching rather than one an agent is
    ...
    /// field: `termifier-agent-hook` exiting 0 with no output already
