...
// Termifier
//
// The `termifier-approval-hook` shell script installed under
// `~/Library/Application Support/Termifier/bin/`, alongside
// `termifier-agent-hook` (see AgentHookScript.swift). Unlike
// termifier-agent-hook's fire-and-forget async POST to /agent-event, this
...
enum ApprovalHookScript {
    ...
    static let fileName = "termifier-approval-hook"
    ...
    /// MCP client config substitutes a stale `TERMIFIER_SURFACE_ID` inside
    ...
    ///
    /// The `X-Termifier-Surface-ID` header value is
    ...
    /// `TERMIFIER_SURFACE_ID` otherwise -- see `AgentHookScript`'s doc
    ...
    /// documents -- Termifier's own server answers by 570s, strictly before
    ...
    ///   Termifier crashes, `agent-endpoint.json` is left behind pointing at
    ...
    ///   exactly as it would with no Termifier hook installed at all, rather
    ...
    ///   takes over exactly as it would with no Termifier hook installed at
    ...
    #!/bin/sh
    #
    # termifier-approval-hook -- synchronously forwards a PermissionRequest
    # hook's stdin JSON to Termifier's local Agent Monitor IPC endpoint and
    # prints its response verbatim; the response body IS the CLI's
    # permission decision. No output at all (any curl failure, or Termifier
    ...
    # runner injects into every hook process, so it is set for Termifier's
