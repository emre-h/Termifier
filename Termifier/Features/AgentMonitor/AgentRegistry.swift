...
final class AgentRegistry {
    ...
    /// Rows describing agents Termifier learned about from an external
    ...
    /// clears what Termifier merely inferred for the surface, as fresh
    /// self-reported presence supersedes it, while leaving what Termifier's
    /// own shell integration reported alone
    /// (`AgentEvidence.clearingInferredEvidence`).
    private var evidence: [UUID: AgentEvidence] = [:]

    /// Whether `TermifierMCPServer` currently has its IPC listener running.
    /// `AgentStatusView` observes this (not `TermifierMCPServer.isRunning`
    /// directly) so the sidebar's disabled/enabled placeholder redraws —
    /// `TermifierMCPServer` is a plain `@MainActor` class, not `@Observable`,
    ...
    /// periodic staleness sweep. Called by `TermifierMCPServer.finishStart()`.
    ...
    /// staleness sweep. Called by `TermifierMCPServer.stop()` so a disabled IPC
    ...
    ///   `TermifierMCPServer.routeAgentEvent`'s `X-Termifier-Agent-Kind` header.
    ...
    /// Surfaces Termifier's own shell integration has ever reported on --
    /// recorded by `recordTermifierShellIntegrationReported`, which
    /// `TermifierMCPServer.routeCommandEvent` calls unconditionally for both
    ...
    /// prepends its resources dir to `XDG_DATA_DIRS` while Termifier appends,
    ...
    /// pane-exit signal can be written before Termifier's own end-of-command
    ...
    /// Termifier's own zsh/fish shell integration can tell a suspend apart
    ...
    /// .applyTermifierShellIntegrationIfEnabled` gates the whole shell
    /// integration install and env injection on `CommandTrackingSettings
    /// .trackingEnabled`, so with tracking off no Termifier integration is
    ...
    private var termifierShellIntegrationReportedSurfaces: Set<UUID> = []

    /// Records that Termifier's own shell integration is live in
    /// `surfaceID`'s pane, without touching the row's state at all --
    /// the recording half of what `handlePaneCommandFinished` already
    /// does unconditionally on every call of its own. `TermifierMCPServer
    ...
    /// duplicate event still proves Termifier's integration is live in that
    /// pane. See `termifierShellIntegrationReportedSurfaces`'s own doc
    ...
    /// Only `.commandEnd` stamps `AgentEvidence.lastTermifierCommandEndAt`
    /// and answers any outstanding ghostty deferral for the surface --
    /// `AgentStateResolver.resolveTermifierShellIntegrationReported` owns
    ...
    ///
    /// - Parameter now: Injectable for tests (defaults to `Date()`).
    func recordTermifierShellIntegrationReported(
        surfaceID: UUID, phase: TermifierShellReportPhase, now: Date = Date()
    ) {
        termifierShellIntegrationReportedSurfaces.insert(surfaceID)
        apply(
            AgentStateResolver.resolve(
                .termifierShellIntegrationReported(phase: phase),
                surfaceID: surfaceID,
                current: entries[surfaceID],
                evidence: evidence[surfaceID] ?? AgentEvidence(),
                context: context(for: surfaceID),
    ...
    /// `ShellIntegrationInstaller.fishIntegrationBody`'s `_termifier_postexec`
    ...
    /// `TermifierMCPServer.routeCommandEvent` consults this to decide whether
    ...
    /// `surfaceID` via `recordTermifierShellIntegrationReported` first: see
    /// `termifierShellIntegrationReportedSurfaces`'s own doc comment for why
    /// `handleGhosttyCommandFinished` depends on the recording happening
    /// even when this call settles nothing. `TermifierMCPServer
    /// .routeCommandEvent` also calls `recordTermifierShellIntegrationReported`
    ...
    func handlePaneCommandFinished(
        ...
        recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandEnd, now: now)
        let resolution = AgentStateResolver.resolve(
            .paneCommandFinished(exitCode: exitCode, suspended: suspended, origin: .termifierShellIntegration),
            surfaceID: surfaceID,
            current: entries[surfaceID],
            evidence: evidence[surfaceID] ?? AgentEvidence(),
            context: context(for: surfaceID),
    ...
    /// fallback for the shells Termifier's own `/command-event` shell
    ...
    /// leaves the row untouched -- once `termifierShellIntegrationReportedSurfaces`
    /// (surfaced to the resolver as `context(for:)`'s
    /// `termifierShellIntegrationSeen`) already contains this surface: once
    /// Termifier's own integration has ever reported on a surface, it alone
    ...
    /// can settle the row when Termifier's own `/command-event` never
    ...
    /// wiring lives in `TermifierMCPServer.routeCommandEvent`, gated on
    ...
    /// Termifier's own inference about a row it hasn't heard from, not a
    ...
    private func context(for surfaceID: UUID) -> ResolverContext {
        ResolverContext(
            ...
            termifierShellIntegrationSeen: termifierShellIntegrationReportedSurfaces.contains(surfaceID)
        )
    }
    ...
    /// gate in `TermifierWindowController`, which already refuses to feed
    ...
    /// `TermifierMCPServer.stop()` unconditionally clears `entries` via
    ...
    /// always used, but `TermifierMCPServer` also calls it directly now, to
    /// bind a surface the moment its MCP connection's `initialize` (or an
    /// explicit `register_peer` tool call) carries an `X-Termifier-Surface-ID`
    /// header — covering a passive recipient that never calls a
    /// termifier-ipc tool itself and so never fires the hook path at all.
    ...
    /// `TermifierMCPServer` rely on this:
    ...
    /// Every peer ID currently bound to a surface. `TermifierMCPServer`
    ...
    /// `AgentEvent.ipcSelfPeerID` -- see `bindSurface`). `TermifierMCPServer`
    ...
    /// holding a ghostty pane-exit signal Termifier's own shell integration
    /// never answered is settled to `.done`. Neither can expire a
    /// surface's pending approvals: that wiring lives in
    /// `TermifierMCPServer.routeCommandEvent`, gated on
    ...
    func handleSurfaceDestroyed(surfaceID: UUID) {
        ...
        termifierShellIntegrationReportedSurfaces.remove(surfaceID)
        subagentRegistry.handleSurfaceDestroyed(parentSurfaceID: surfaceID)
    }
...
extension Notification.Name {
    ...
    static let termifierSurfaceDestroyed = Notification.Name("com.termifier.agentMonitor.surfaceDestroyed")

    /// Posted when an Agents sidebar row is clicked. `userInfo["surfaceID"]`
    /// carries the `UUID` of the surface to focus.
    static let termifierFocusSurface = Notification.Name("com.termifier.agentMonitor.focusSurface")
}
