...
struct TabSnapshot: Codable, Equatable {
    ...
    let browserURL: URL?
    /// Schema v6: termifier-session references keyed by leaf surface UUID
...
extension Dictionary where Key == UUID {
    ...
    /// `TermifierWindowController.performReconnect` (surface swap) -- there
...
extension TabGroup {
    ...
    ///   Lets
    ///   `TermifierWindowController.windowSnapshot()` delegate its
