...
private let logger = Logger(subsystem: "com.termifier.terminal", category: "SessionPersistence")
...
actor SessionPersistenceActor {
    ...
    init() {
        let termifierDir: URL
        if let testDir = ProcessInfo.processInfo.environment["TERMIFIER_UITEST_SESSION_DIR"] {
            termifierDir = URL(fileURLWithPath: testDir, isDirectory: true)
        } else {
            ...
            termifierDir = homeDir.appendingPathComponent(".termifier", isDirectory: true)
        }

        try? FileManager.default.createDirectory(at: termifierDir, withIntermediateDirectories: true, attributes: [
            .posixPermissions: 0o700
        ])

        self.savePath = termifierDir.appendingPathComponent("sessions.json")
        self.backupPath = termifierDir.appendingPathComponent("sessions.json.bak")
        self.recoveryMarkerPath = termifierDir.appendingPathComponent(".recovery")
        self.recoverySnapshotPath = termifierDir.appendingPathComponent("sessions.recovery.json")
    }
    ...
    /// Migrate session data from legacy Application Support path to ~/.termifier/.
    /// Returns true if migration was performed.
    @discardableResult
    func migrateFromLegacyPath() -> Bool {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let legacyPath = appSupport.appendingPathComponent("Termifier/session.json")
