...
//  Termifier
//
//  Hard-coded data table mapping the 15 languages Termifier ships with to
...
//  user overrides from ~/.config/termifier/lsp.json.
//
...
/// Everything Termifier needs to know about one language server.
///
...
///   destination to redirect Termifier into spawning an attacker-controlled
...
/// Immutable in-memory table of every language server Termifier knows
...
struct LSPServerRegistry: Sendable, Equatable, Codable {
    ...
    static func loaded(overridePath: URL?) throws -> LSPServerRegistry {
        ...
        // manager that consumes paths under ~/.config/termifier.
    ...
    /// The 15 languages Termifier ships with. Order here is the order
