...
// redeclared here: Termifier/Models/SplitTree.swift already defines a
// Swift (not C) `SplitDirection` enum (.horizontal/.vertical) used by
// SplitTree.insert(at:direction:) and TermifierWindowController.performSplit
...
// TermifierTests/Cockpit/CockpitAppAccessSeamTests.swift, which instead
// drives the TermifierWindowController seams (performSplit,
...
/// (`TermifierWindowController.ownsSplitLeaf`/`findTab(bySplitLeaf:)`), the
...
/// `TermifierWindowController` seams this delegates to are tested directly
...
final class LiveCockpitAppAccess: CockpitAppAccessing {
    ...
    private func designatedWindowController(_ appDelegate: AppDelegate) -> TermifierWindowController? {
    ...
    private func owningController(for surfaceID: UUID, in appDelegate: AppDelegate) -> TermifierWindowController? {
                ...
                for tab in group.tabs {
                    ...
                    for leafID in leafIDs {
                        ...
                        panes.append(CockpitPaneInfo(
                            ...
                            termifierSessionID: SessionSurfaceMap.shared.sessionID(for: leafID)
                        ))
                    }
                }
            }
