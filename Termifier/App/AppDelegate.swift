...
private let logger = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "com.termifier.terminal",
...
class AppDelegate: NSObject, NSApplicationDelegate, HerdrSessionPresenceObserver {
    ...
    private var windowControllers: [TermifierWindowController] = []
    ...
    /// tiers: (1) a real key `TermifierWindowController`, if one exists; (2)
    /// else the front-most `TermifierWindowController` among `NSApp.
    ...
    /// at), giving the most-recently-used Termifier window on the current
    /// Space even while none is key (e.g. Termifier is inactive, or some
    ...
    /// for when every Termifier window is minimized, off-Space, or not yet
    ...
    /// through this.
    var currentWindowController: TermifierWindowController? {
    ...
    func windowController(owningSurface id: UUID) -> TermifierWindowController? {
    ...
    /// off-Space, or otherwise not showable), Termifier does nothing further:
    /// the panel orders out and AppKit's own key-window restoration
    /// decides, so Termifier never moves an unrelated window's first
    ...
    /// current window's own geometry changes. Only `.termifierApprovalInboxChanged`
    ...
    /// a window created while Termifier is inactive never becomes key, so
    ...
    /// `TermifierWindowController.windowDidBecomeKey` calls this instead of
    ...
    func windowControllerDidBecomeKey(_ controller: TermifierWindowController) {
        guard currentWindowTracker.didBecomeKey(controller) else { return }
        refreshApprovalPanel()
    }

    /// `TermifierWindowController.windowDidChangeScreen`/full-screen enter/
    ...
    func windowControllerGeometryChanged(_ controller: TermifierWindowController) {
    ...
    /// `.attachHerdrWorkspace(_:)` and `TermifierWindowController`'s
    ...
    private func makeHerdrTabCoordinator(herdrBinPath: String) -> HerdrTabCoordinator {
        let attacher = HerdrNativeTabAttacherLive(
            ...
            sessionKillHook: { surfaceID in
                ...
                // carries no termifier-session identity, so there is never a
    ...
    /// from any single `TermifierWindowController.isClosingForShutdown`. That
    ...
    /// `TermifierWindowController.isAppActuallyTerminating`,
    ...
    var allWindowControllers: [TermifierWindowController] {
        windowControllers
    }
    ...
    func _testInsertWindowController(_ controller: TermifierWindowController) {
    ...
    /// Termifier's own bundled ghostty resources directory, if the bundle
    ...
    /// .trackingEnabled`), installs Termifier's own zsh/fish command-log
    /// shell integration scripts and points this process's environment
    /// at them (`TermifierShellIntegrationEnvironment.apply(rootDirectory:)`)
    ...
    /// touched.
    func applyTermifierShellIntegrationIfEnabled() {
    ...
    /// Re-syncs Termifier's own hook config entries with the current
    ...
    /// managed-block replacement both strip stale Termifier-owned entries
    /// before writing fresh ones -- so re-running it here on every
    /// launch doubles as the migration path for a user who enabled IPC
    /// under an older Termifier version and has not manually re-run "Enable
    ...
    /// which `TermifierMCPServer.routeApprovalRequest`'s own `hookEventName`
    ...
    /// than an all-or-nothing gate on whether ANY tool has Termifier hooks,
    /// so enabling IPC for one CLI can never spread to a second CLI the
    /// user installed afterward but never opted in for.
    ///
    /// Skipped entirely for a `--uitesting` launch: `TermifierUITestCase`
    ...
    /// this lives here rather than in `TermifierWindowController
    ...
    private func playBellAudio() {
        ...
        // (TermifierWindowController.restoreWindow's FullScreenRestoreBox).
    ...
    /// `PATH`, after Termifier launched would otherwise leave this nil for
    ...
    /// tried" would be Termifier deciding herdr is absent on evidence older
    /// than presence's own.
    ///
    ...
    /// (`TermifierWindowController.rebuildSplitContainer`'s own
    ...
    ///
    /// `false` when no target window is available (every Termifier window
    ...
    /// entry and routes to `TermifierWindowController.applyHerdrRatioMutation(
    ...
    /// `TermifierWindowController.closeHerdrTrackedSurface(_:)` -- the
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        // The TermifierTests scheme runs this app itself as its unit-test
        // HOST, so this method runs for real, unguarded, against the
        // developer's own ~/.termifier before a single test method executes.
        ...
        // spawn real persistent termifier-session daemons; and it starts
        // BrowserServer's real loopback listener. TermifierUITests launches the app-under-test as a
        ...
        applyTermifierShellIntegrationIfEnabled()

        // Add CLI to PATH for terminals launched within Termifier
        ...
        // .termifierSurfaceDestroyed self-pruning (HerdrPaneRegistry.swift's
        ...
        // with a fresh, never-before-used `TERMIFIER_UITEST_SESSION_DIR`
        // (`TermifierUITestCase.setUp`), so this always falls through to
        ...
        if !restoreSession() {
            if pendingURLs.isEmpty {
                createNewWindow()
                // Demo-recording scenario only (TermifierUITests
        ...
        // issue #37 recurrence guard: issue #37 ("Termifier closes after
    ...
    /// never terminates Termifier any more, matching Ghostty's own
    ...
    /// Dock-icon click / re-open (double-clicking Termifier.app, or clicking
    ...
    /// The Cmd+Q / "Quit Termifier" path: the only remaining termination
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        // developer's real ~/.termifier even though nothing in this launch
        ...
        // silently leaving the termifier-session running as an orphan even
    ...
    private func registerNotificationObservers() {
        ...
        // observer lives here rather than in `TermifierWindowController
        ...
        // neither may depend on a `TermifierWindowController` existing to
    ...
    @objc private func handleNewTab(_ notification: Notification) {
        ...
        guard let surfaceView = notification.object as? SurfaceView,
            ...
            // No source: only a KEY TermifierWindowController receives the
    ...
    /// `TermifierWindowController` exists to relay anything. The signal fires
    ...
    /// Relays `.termifierSurfaceDestroyed` (posted by `SurfaceRegistry
    ...
    /// `ApprovalInboxStore` posts `.termifierApprovalInboxChanged` on every
    ...
    /// `TermifierWindowController.init(initialHost:)` for the new window's
    ...
    private func openNewWindow(initialHost: String?) {
        ...
        let wc = TermifierWindowController(windowSession: windowSession, initialHost: initialHost)
    ...
    /// (TermifierUITests/DemoRecordingScenario.swift) always frames
    /// identically regardless of this machine's actual screen size or
    /// `TermifierWindowController`'s own 800x600-then-`center()` default.
    ...
    /// production code.
    var _spawnRemoteSessionTabAddTabHookForTesting: ((TermifierWindowController) -> Void)?
    ...
    /// constructs a real `TermifierWindowController` and calls
    ...
    /// own window (not any `TermifierWindowController`'s window) is key -- so
    ...
    /// `TermifierWindowController`'s own window when one owns it at all.
    ...
    func openWindowAtPath(_ pwd: String) {
        ...
        let windowSession = WindowSession(initialTab: initialTab)
        appSession.addWindow(windowSession)
        let wc = TermifierWindowController(windowSession: windowSession)
        windowControllers.append(wc)
        refreshApprovalPanel()
        wc.showWindow(nil)
    }
    ...
    /// `TermifierWindowController._performReconnectCommandObserverForTesting`'s
    ...
    func attachWindow(sessionID: String, cwd: String?, host: String? = nil) {
        ...
        let (window, wc) = makeRestoringWindowController(
            contentRect: NSRect(origin: .zero, size: TermifierWindowController.defaultContentSize),
    ...
    var _focusWindowForExistingSessionShowHookForTesting: ((TermifierWindowController) -> Void)?
    #endif
    ...
    private var nonClosingWindowGroups: [(controller: TermifierWindowController, group: TabGroup)] {
        windowControllers
    ...
    /// containing `surfaceID` (`TermifierWindowController.activateTabContaining`,
    ...
    /// Browser's plain `NSWindow`, not any `TermifierWindowController`'s
    ...
    /// panes structurally unreachable from termifier-session's own
    ...
    /// window.
    ///
    /// No target window available (e.g. every Termifier window closed while
    ...
    /// (`TermifierWindowController.attachRestoredTab(_:)`) instead of
    ...
    private func attachSessionAsNewTab(sessionID: String, cwd: String?, host: String?, in target: TermifierWindowController) {
    ...
    private func makeRestoringWindowController(
        contentRect: NSRect,
        windowSession: WindowSession
    ) -> (window: TermifierWindow, controller: TermifierWindowController) {
        appSession.addWindow(windowSession)
        ...
        let wc = TermifierWindowController(window: window, windowSession: windowSession, restoring: true)
        windowControllers.append(wc)
        refreshApprovalPanel()
        return (window, wc)
    }
    ...
    private func cleanupFailedWindow(
        _ window: TermifierWindow,
        _ windowSession: WindowSession,
        _ wc: TermifierWindowController,
        message: String
    ) {
        window.close()
    ...
    func removeWindowController(_ controller: TermifierWindowController) {
    ...
    /// `TermifierWindowController.closeLastWindow()`, `processCloseWindow()`,
    /// `closeTab`/`closeActiveGroup`/`closeAllTabsInGroup`/
    /// `closeFocusedSessionSurface` (see each's own doc comment in
    /// `TermifierWindowController.swift`). `closeTab` and, since they route
    ...
    /// `TermifierWindowController.closeLastWindow()`'s own choice. There is
    ...
    /// `TermifierWindowController.handleShowChildExitedNotification` and
    /// `handleSessionReconnectDecision`, defer their work instead of
    /// acting immediately (see each's doc comment). The `didSet` below
    /// posts `.termifierConfirmingQuitDidEnd` on the `true` -> `false`
    /// transition so every live `TermifierWindowController` can replay
    ...
    private(set) var isConfirmingQuit: Bool = false {
        didSet {
            ...
            NotificationCenter.default.post(name: .termifierConfirmingQuitDidEnd, object: nil)
        }
    }
    ...
    /// Cmd+Q / "Quit Termifier" path): closing a
    ...
    func confirmQuitIfNeeded() -> Bool {
        ...
        alert.messageText = "Quit Termifier?"
    ...
    func setupMainMenu() {
        ...
        appMenu.addItem(withTitle: "About Termifier", action: #selector(showAboutPanel), keyEquivalent: "")
        ...
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Hide Termifier", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        let hideOthersItem = NSMenuItem(title: "Hide Others", action: #selector(NSApplication.hideOtherApplications(_:)), keyEquivalent: "h")
        hideOthersItem.keyEquivalentModifierMask = [.command, .option]
        appMenu.addItem(hideOthersItem)
        appMenu.addItem(withTitle: "Show All", action: #selector(NSApplication.unhideAllApplications(_:)), keyEquivalent: "")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Quit Termifier", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        ...
        fileMenu.addItem(withTitle: "New Tab", action: #selector(TermifierWindowController.newTab(_:)), keyEquivalent: "t")
        fileMenu.addItem(withTitle: "New Browser Tab", action: #selector(TermifierWindowController.newBrowserTab(_:)), keyEquivalent: "")
        ...
        // shortcut and stable selector — see `NSWindow+TermifierClose.swift`'s
        ...
        // entire point of `termifierPerformClose(_:)` existing as a
        // same-selector-on-every-window method.
        fileMenu.addItem(withTitle: "Close", action: #selector(NSWindow.termifierPerformClose(_:)), keyEquivalent: "w")

        let closeTabItem = NSMenuItem(
            title: "Close Tab",
            action: #selector(TermifierWindowController.closeTab(_:)),
            keyEquivalent: "w")
        ...
        let composeItem = NSMenuItem(
            title: "Compose Input",
            action: #selector(TermifierWindowController.toggleComposeOverlay),
            keyEquivalent: "e"
        )
        ...
        let toggleSidebarItem = NSMenuItem(
            ...
            action: #selector(TermifierWindowController.toggleSidebar),
            keyEquivalent: "s"
        )
        ...
        let paletteItem = NSMenuItem(
            ...
            action: #selector(TermifierWindowController.toggleCommandPalette),
            keyEquivalent: "p"
        )
        ...
        let fullScreenItem = NSMenuItem(
            ...
            action: #selector(TermifierWindowController.toggleFullScreen(_:)),
            keyEquivalent: "f"
        )
        ...
        let nextTabItem = NSMenuItem(title: "Select Next Tab", action: #selector(TermifierWindowController.selectNextTab(_:)), keyEquivalent: "]")
        nextTabItem.keyEquivalentModifierMask = [.command, .shift]
        windowMenu.addItem(nextTabItem)

        let prevTabItem = NSMenuItem(title: "Select Previous Tab", action: #selector(TermifierWindowController.selectPreviousTab(_:)), keyEquivalent: "[")
        prevTabItem.keyEquivalentModifierMask = [.command, .shift]
        windowMenu.addItem(prevTabItem)

        let jumpUnreadItem = NSMenuItem(title: "Jump to Unread Tab", action: #selector(TermifierWindowController.jumpToMostRecentUnreadTab), keyEquivalent: "u")
        ...
        let newGroupItem = NSMenuItem(
            title: "New Group",
            action: #selector(TermifierWindowController.newGroup(_:)),
            keyEquivalent: "n")
        ...
        let closeGroupItem = NSMenuItem(
            title: "Close Group",
            action: #selector(TermifierWindowController.closeGroup(_:)),
            keyEquivalent: "w")
        ...
        let nextGroupItem = NSMenuItem(
            title: "Next Group",
            action: #selector(TermifierWindowController.nextGroup(_:)),
            keyEquivalent: "]")
        ...
        let prevGroupItem = NSMenuItem(
            ...
            action: #selector(TermifierWindowController.previousGroup(_:)),
            keyEquivalent: "[")
    ...
    /// early-returns in the TermifierTests process (see that gate's own doc
    ...
    func notifyPreviousSessionNotRestored() {
        NotificationManager.shared.sendNotification(
            ...
            body: "Termifier didn't restore your previous windows and tabs, but they're safely preserved. " +
    ...
    func restoreTabSurfaces(
        ...
        // like a genuine ULID before it ever reaches termifier-session
    ...
    private func createSurfaceWithPwd(
        ...
        if let oldLeafID {
            ...
            case .bridgeCommand(let command):
                ...
                // A herdr-bridged leaf carries no termifier-session identity
    ...
    /// round-trip completes, without spawning a real `termifier-session`
    ...
    /// per restored surface: N concurrent `termifier-session ls --all --json`
    ...
    func fetchSessionsForAgentResume() {
        guard SessionSettings.agentResumeEnabled else {
            ...
            // handler, which SIGTERMs the underlying `termifier-session`
    ...
    /// The attach-spawned termifier-session daemon always starts
    ...
    /// FIRST `termifier-session attach --create` ghostty actually execs
    ...
    /// creation time, before `termifier-session attach`'s reattach
    ...
    /// `TermifierTests/AppDelegateKeyMonitorTests.swift`).
    ///
    /// Cases:
    ...
    private func debugLog(_ msg: String) {
        let logPath = "/tmp/termifier_debug_select.log"
        let entry = "\(Date()): \(msg)\n"
...
extension Notification.Name {
    ...
    /// test seam. `TermifierWindowController` observes this to replay events
    /// it deferred while the gate was up; see
    /// `drainDeferredReconnectEvents()`.
    static let termifierConfirmingQuitDidEnd = Notification.Name("com.termifier.session.confirmingQuitDidEnd")
}
