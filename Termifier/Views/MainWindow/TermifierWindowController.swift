...
private let logger = Logger(
    ...
    category: "TermifierWindowController"
)

/// Reconnect-flashing-bug fix: narrow `#if DEBUG` override hook for
/// `TermifierWindowController.reconnectEstablishGraceMilliseconds`,
...
@MainActor
class TermifierWindowController: NSWindowController, NSWindowDelegate {
    ...
    /// `allCommands` directly (via `@testable import Termifier`) to assert
    ...
    /// protocol, so `TermifierWindowControllerCopyTitleToClipboardTests` can
    ...
    /// `makeKeyAndOrderFront`, so Termifier is never brought to the front by
    ...
    private func setupCommandRegistry() {
        ...
        commandRegistry.register(PaletteCommand(id: "ipc.enable", title: "Enable AI Agent IPC", category: "IPC", isAvailable: {
            !TermifierMCPServer.shared.isRunning
        }) { [weak self] in
            self?.enableIPC()
        })
        ...
        commandRegistry.register(PaletteCommand(id: "cli.install", title: "Install CLI to PATH", category: "System") {
            ...
            let cliSource = "\(appPath)/Contents/Resources/bin/termifier"
            let cliDest = "/usr/local/bin/termifier"
            ...
            if let appleScript = NSAppleScript(source: script) {
                ...
                if let error {
                    ...
                    alert.informativeText = "The 'termifier' command is now available. Run 'termifier browser --help' to get started."
        ...
        // .attachHerdrWorkspace(_:), one Termifier tab per opened workspace);
    ...
    private func setupUI() {
        ...
        // The AppKit side installs no titlebar material of its own: TermifierWindow already
    ...
    /// Termifier's own reconstruction of the directory libghostty would inherit
    ...
    ///   than anything Termifier could resolve independently.
    ...
    ///   `termifier-session attach --create` command's cwd is fixed at spawn
    ...
    /// libghostty, not Termifier's own possibly-stale `Tab.pwd` snapshot --
    /// with multiple panes in a tab, `tab.pwd` can be a sibling pane's
    /// directory. (Termifier does have a surface-level pwd getter now --
    ...
    /// `TermifierWindowControllerCreateManagedSurfaceRemoteHostTests` drives
    ...
    /// user-initiated pane/tab close ends the underlying termifier-session
    ...
    /// orphans the termifier-session running entirely on the remote host.
    ...
    /// calls `daemonClient.kill` -- the underlying termifier-session keeps
    ...
    private func tearDownSurfaces(in tab: Tab, isTerminating: Bool) {
        for surfaceID in tab.registry.allIDs {
            ...
            // Termifier-side path a herdr-bridged leaf can disappear through
            ...
            if HerdrPaneRegistry.shared.isBridgeSurface(surfaceID) {
                (NSApp.delegate as? AppDelegate)?.herdrTabCoordinator?.handleTermifierSurfaceClosed(surfaceID: surfaceID)
            }
    ...
    /// precedent) -- a `TermifierWindowController` unit test can drive this
    ...
    /// `TermifierWindow` (unlike `GhosttyAppController.shared.app`), so
    ...
    // MARK: - Cockpit Seams
    //
    // Backs LiveCockpitAppAccess (Termifier/Features/Cockpit/CockpitAppAccess.swift)
    // -- see TermifierTests/Cockpit/CockpitAppAccessSeamTests.swift for the
    ...
    /// `nil` in the `TermifierTests` process, so a version of this method
    ...
    /// direct test access" precedent) -- a `TermifierWindowController` unit
    ...
    /// construct a real `TermifierWindow` (unlike `GhosttyAppController
    ...
    /// Not `private`: `TermifierWindowControllerCloseArmsTests`
    ...
    private func registerNotificationObservers() {
        ...
        center.addObserver(self, selector: #selector(handleFocusSurfaceNotification(_:)),
                           name: .termifierFocusSurface, object: nil)
        ...
        center.addObserver(self, selector: #selector(handleConfirmingQuitDidEnd(_:)),
                           name: .termifierConfirmingQuitDidEnd, object: nil)
    ...
    /// (a) `AgentRegistry.isServerRunning`: IPC disabled means Termifier's
    ...
    private func closeSurfaceAndCleanUp(
        ...
        // closing from the Termifier side (Cmd+W, a pane's own close button,
        // ...) BEFORE `SurfaceRegistry.destroySurface` below runs, so
        // `HerdrPaneRegistry.shared` still has the entry to look up --
        // see `HerdrTabCoordinator.handleTermifierSurfaceClosed(surfaceID:)`'s
    ...
    /// comment): `TermifierWindowControllerChildExitedTasksTests` calls
    ...
    func processChildExited(surfaceView: SurfaceView) {
        ...
        // .handleTermifierSurfaceClosed` -- the coordinator's own bookkeeping
    ...
    // test files (TermifierWindowControllerCloseTabTests,
    // TermifierWindowControllerCloseWindowTests,
    // TermifierWindowControllerToggleFullscreenTests,
    // TermifierWindowControllerInitialSizeTests,
    // TermifierWindowControllerRendererHealthTests) for the contract each
    // satisfies.
    //
    ...
    /// Termifier-only feature layered on top), so this matches what the user
    ...
    /// `TermifierWindowControllerProcessCloseTabBatchTests`): `OTHER`/`RIGHT`
    ...
    ///
    /// Not `private`: `TermifierWindowControllerProcessCloseTabBatchTests`
    /// calls this directly.
    ...
    // MARK: - termifierPerformClose (GitHub issue #45)
    //
    // `TermifierWindow.termifierPerformClose(_:)`'s intended receiver (once
    // implemented — see that method's own doc comment and
    // `NSWindow+TermifierClose.swift`'s header for the full root-cause
    ...
    // caller, invoked from `TermifierWindow.termifierPerformClose(_:)` via
    // `window?.windowController as? TermifierWindowController` (mirrors
    ...
    /// `TermifierWindow.shouldPerformZoom`.
    ...
    /// fixture's `TermifierWindow` is constructed but never shown/ordered
    /// front, so `window?.isKeyWindow` is always `false` in this test
    /// host (see `TermifierWindowControllerCloseWindowTests`'s own
    ...
    /// `NSScreen`; see `TermifierWindowControllerFullScreenTests.swift`'s
    ...
    /// may not always find a `"mode"` key in `userInfo`). Termifier implements
    ...
    /// to Termifier's one native implementation" contract is
    ...
    /// `GHOSTTY_FULLSCREEN_MACOS_NON_NATIVE_PADDED_NOTCH`. Termifier has no
    ...
    /// point for when/if Termifier grows a non-native presentation of its own)
    ...
    static func shouldUseNativeFullscreen(for mode: ghostty_action_fullscreen_e?) -> Bool {
        if let mode, mode != GHOSTTY_FULLSCREEN_NATIVE {
            logger.info("Non-native fullscreen mode \(mode.rawValue) requested; Termifier has no non-native fullscreen presentation, falling back to native fullscreen")
        }
        return true
    }
    ...
    /// Three guards gate the resize (see `TermifierWindowControllerInitialSizeTests`):
    ...
    func processRendererHealth(surfaceView: SurfaceView, healthy: Bool) {
        ...
        NotificationManager.shared.sendNotification(
            ...
            body: "This pane's renderer reported a problem. Try resizing the window, or restart Termifier if " +
    ...
    // (TermifierWindowControllerSetTabTitleTests, CopyTitleToClipboardTests,
    ...
    /// `TermifierWindow`'s own identical double-guard on its double-click-to-
    /// zoom title bar handler (`TermifierWindow.swift`).
    ...
    // keybind-action batches above. See `TermifierWindowControllerPromptTitleTests` for the
    ...
    /// doc comment — `TermifierWindowControllerPromptTitleTests` calls this
    /// directly.
    ///
    ...
    /// only ever reported unreachable — the underlying `termifier-session`
    ...
    /// (`termifier-session ls --all` still lists it) or a future restore.
    ///
    ...
    private func sendReconnectGiveUpNotification(tabID: UUID) {
        NotificationManager.shared.sendNotification(
            ...
            body: "Termifier couldn't reattach this session, but it may still be running with its state intact. " +
                  "Reattach it from the session browser once the daemon is reachable again.",
            tabID: tabID
        )
    }

    /// A Termifier-side, persistent in-pane indication for
    ...
    /// Re-runs `termifier-session attach --create` for `sessionID` in a
    ...
    private func performReconnect(oldSurfaceID: UUID, sessionID: String) {
        ...
        if let host {
            ...
            guard let localCommand = SessionCommandSynthesizer.reattachCommand(sessionID: sessionID, cwd: tab.pwd ?? NSHomeDirectory()) else {
                logger.error("No termifier-session binary resolvable; cannot reconnect session \(sessionID, privacy: .public)")
                return
            }
        ...
        // termifier-session daemon, which can never have a matching
    ...
    private static var reconnectEstablishGraceMilliseconds: UInt64 {
        #if DEBUG
        if let override = TermifierWindowControllerReconnectGraceOverrides.reconnectEstablishGraceMilliseconds {
            return override
        }
        #endif
        return 2000
    ...
    @objc private func handleGotoSplitNotification(_ notification: Notification) {
        ...
        // clear zoom — Termifier doesn't implement ghostty's
    ...
    /// `GHOSTTY_ACTION_TOGGLE_SPLIT_ZOOM` receiver (Termifier's newest split
    /// feature, ported from upstream ghostty's `BaseTerminalController
    /// .ghosttyDidToggleSplitZoom` / `SplitTree.zoomed` — see
    /// `Termifier/Models/SplitTree.swift`'s own "MARK: - Zoom" section).
    ///
    ...
    /// own "Not `private`" precedent, so `TermifierWindowControllerToggleSplitZoomTests`
    ...
    /// Issue #45 defense in depth (see `NSWindow+TermifierClose.swift`'s
    /// header comment for the full root-cause writeup):
    /// `termifierPerformClose(_:)` closes the Cmd+W hole structurally, but
