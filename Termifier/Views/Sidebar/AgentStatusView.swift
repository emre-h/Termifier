...
struct AgentStatusView: View {
    ...
    /// .title(for:)`, threaded down from `TermifierWindowController`), passed
    ...
    /// .cwd(for:)`, threaded down from `TermifierWindowController`), passed
    ...
    var body: some View {
        content
            ...
            // `TermifierWindowController`'s screen-classification poll can
    ...
    private var content: some View {
        Group {
            ...
            // TermifierMCPServer.isRunning: TermifierMCPServer is a plain
    ...
    /// rows keep the sidebar showing rows even while Termifier's own IPC
...
enum AgentSidebarGate {
    ...
    /// rows even while Termifier's own IPC server is stopped, since it
    ...
    /// explanatory notice that Termifier's own agent monitoring is disabled,
    ...
    /// Termifier's own IPC server) would be missing from the list with no
    /// explanation. The coherent design shows the rows AND flags why
    /// Termifier's own agents might be absent -- so this is `true` only when
    /// Termifier's own server is off AND something else (herdr) is why rows
    /// are showing at all: `isServerRunning == true` never warns (Termifier's
...
enum AgentRowFocusTarget {
    ...
    /// highlight) rather than posting `.termifierFocusSurface` for a surface
    /// nothing can resolve. Resolution order:
    ///   1. `focusSurfaceID`, when set, always wins — it is the explicit
    ///      "focus this Termifier surface instead" pointer `AgentEntry`
...
private struct AgentRowView: View {
    ...
    var body: some View {
        ...
        // Termifier pane closed while the pointer was still over the row).
        ...
        // Termifier pane closing prunes `focusSurfaceID`), which tears down
...
enum AgentRowDisplay {
    ...
    /// `.external` rows so a row sourced from herdr rather than Termifier's
