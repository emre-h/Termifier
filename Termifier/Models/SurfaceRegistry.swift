...
private let logger = Logger(subsystem: "com.termifier.terminal", category: "SurfaceRegistry")
...
final class SurfaceRegistry {
    ...
    /// same rationale as `TermifierMCPServer.agentRegistry`.
    ...
    /// (see `TermifierWindowController`'s reconnect path, which
    ...
    /// `TermifierWindowController.windowWillClose`, iterate `allIDs` to
    ...
    /// `pwd`) so the surface's child process is a `termifier-session attach`
    ...
    func destroySurface(_ id: UUID) {
        guard var entry = entries[id] else {
            ...
            // -- unregisterView + the .termifierSurfaceDestroyed post must
            ...
            NotificationCenter.default.post(
                name: .termifierSurfaceDestroyed,
                object: nil,
    ...
    /// own doc comment). `TermifierWindowController.performReconnect` calls
