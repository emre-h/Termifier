...
/// collapsed), so `TermifierWindowController.renameHost(for:in:)` must pick
...
/// set on `Tab.renameRequest` by `TermifierWindowController.processPromptTitle
...
class Tab: Identifiable {
    ...
    /// termifier-session references for this tab's persistent-session
    ...
    /// Set by `TermifierWindowController.processPromptTitle(surfaceView:scope:)`
