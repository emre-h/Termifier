...
private let logger = Logger(subsystem: "com.termifier.terminal", category: "GhosttyAction")
...
enum GhosttyActionRouter {
    ...
    static func handleAction(
        ...
        // actions is a deliberate decision, not a placeholder for later work.
        ...
        // exactly the wrong outcome for a keybind Termifier has consciously
        // decided has nothing to do here.

        case GHOSTTY_ACTION_CHECK_FOR_UPDATES:
            // Termifier intentionally has no in-app updater. Consume Ghostty's
            // updater action so a user keybind cannot leak into the shell.
            return true

        case GHOSTTY_ACTION_TOGGLE_TAB_OVERVIEW:
            // AdwTabOverview (GTK's tab grid) only exists when GTK's
            // libadwaita is >= 1.4 -- a GTK/Linux-only surface. Termifier
    ...
    /// `TermifierWindowController` to feed into `AgentRegistry.
    ...
    /// pane-level fallback for the shells Termifier's own `/command-event`
    ...
    /// `CommandLogStore` still relies on Termifier's own hook for the command
    ...
    /// not this mapper's. Not `private`: `@testable import Termifier` reaches
    ...
    private static func handleInitialSize(
        ...
        // shape) so TermifierWindowController.init can recover this for a
    ...
    /// Deliberately has no `TermifierWindowController` observer for
    ...
    /// unimplemented" too (`Ghostty.App.swift:659-664`). Termifier additionally
    ...
    /// the WINDOW level. `TermifierWindow.minSize = NSSize(width: 400, height:
    /// 300)` (`TermifierWindow.swift`) is this app's real, window-level
    ...
    /// title bar/decorations to match. Termifier's chrome color is instead
    ...
    /// know WHICH leaf to toggle, and `TermifierWindowController
    ...
    // `TermifierWindowController`-owned `.ghostty*` notification: none of
    ...
    /// `GHOSTTY_ACTION_PRESENT_TERMINAL`. Reuses `.termifierFocusSurface`
    /// rather than introducing a new notification name —
    /// `TermifierWindowController.handleFocusSurfaceNotification` already
    ...
    private static func handlePresentTerminal(
        ...
        NSApp.activate()
        NotificationCenter.default.post(name: .termifierFocusSurface, object: nil, userInfo: ["surfaceID": surfaceID])
        return true
    }
    ...
    // logic lives here — `TermifierWindowController`'s `findTab(for:)`-based
    // observers own that (see TermifierWindowController.swift's own "MARK: -
    ...
    // these six wire-ups; see each Swift-side `TermifierWindowController*Tests`
    // suite instead.
    ...
    /// `["mode": mode]`): `TermifierWindowController.handlePromptTitleNotification`
