...
private let logger = Logger(subsystem: "com.termifier.terminal", category: "SurfaceView")
...
class SurfaceView: NSView {
    ...
    /// from inside `ghostty_surface_new`, before `TermifierWindowController
    /// .registerNotificationObservers()` has run for a brand-new window's
    /// very first surface -- `TermifierWindowController.init` reads this
    ...
    /// anything unhealthy). Set by `TermifierWindowController
    ...
    /// `TermifierWindowController._isKeyWindowOverrideForTesting`'s identical
    ...
    /// on `TermifierWindowController`'s: `SurfaceView` is earlier in the
    ...
    /// target here FIRST -- `TermifierWindowController.validateMenuItem(_:)`'s
...
extension SurfaceView {
    ...
    // Issue #45 defense in depth (see `TermifierWindowController.swift`'s
    // `keyWindowGatedActions` doc comment for the full root-cause
    // writeup): `SurfaceView` sits earlier than `TermifierWindowController` in
    ...
    //   chain, `TermifierWindowController`'s own matching branches/actions are
    ...
    //   the About panel (a non-`TermifierWindow` `NSPanel`, `canBecomeMain ==
    //   false`) key, `NSApp.mainWindow` stays whatever `TermifierWindow` was
    ...
    //   item can't silently reopen this gap. `TermifierWindowController`
    ...
    /// mirroring `TermifierWindowController.isKeyWindowForMenuValidation`.
    ...
    @objc func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        // the search field, TermifierWindowController provides the same gating in
        ...
        // to `return true`) and TermifierWindowController's matching gate would
        // never be consulted.
    ...
    private var isActiveTabSplit: Bool {
        guard let controller = window?.windowController as? TermifierWindowController else {
            return false
        }
