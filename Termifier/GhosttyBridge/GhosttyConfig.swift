...
private let logger = Logger(subsystem: "com.termifier.terminal", category: "GhosttyConfig")
...
final class GhosttyConfigManager {
    ...
    /// Keys that Termifier manages (overrides from user's ghostty config).
    ...
    private static var termifierConfigDir: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/termifier", isDirectory: true)
    }

    private static var termifierGlassPresetURL: URL {
        termifierConfigDir.appendingPathComponent("termifier-glass.conf", isDirectory: false)
    }

    private static var termifierRuntimeOverrideURL: URL {
        termifierConfigDir.appendingPathComponent("termifier-runtime.conf", isDirectory: false)
    }
    ...
    /// Note: configLoadFile for termifier-glass.conf runs on EVERY loadDefaultConfig call regardless of this flag.
    ...
    static func loadDefaultConfig() -> ghostty_config_t? {
        ...
        if !hasAppliedPreset {
            applyTermifierGlassPresetIfPossible()
        ...
        // Load Termifier glass preset after default files so its values take precedence.
        let presetPath = termifierGlassPresetURL.path
    ...
    /// Builds the text of the Termifier-managed runtime override block, plus an
    ...
    private static func applyRuntimeOverrides(_ cfg: ghostty_config_t) {
        ...
        do {
            try fm.createDirectory(at: termifierConfigDir, withIntermediateDirectories: true)
            ...
            try (finalText + "\n").write(to: termifierRuntimeOverrideURL, atomically: true, encoding: .utf8)
            GhosttyFFI.configLoadFile(cfg, path: termifierRuntimeOverrideURL.path)
        } catch {
            logger.warning("Failed to apply runtime overrides: \(error.localizedDescription, privacy: .public)")
        }
    }

    private static func applyTermifierGlassPresetIfPossible() {
        let fm = FileManager.default
        let presetConfigURL = termifierGlassPresetURL

        do {
            // 1. Create ~/.config/termifier/ directory if needed.
            try fm.createDirectory(at: termifierConfigDir, withIntermediateDirectories: true)
            ...
            if let oldBundleID = Bundle.main.bundleIdentifier,
               let oldAppSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
                let oldPath = oldAppSupport
                    ...
                    .appendingPathComponent("termifier-glass.conf", isDirectory: false)
            ...
            // 3. Create termifier-glass.conf with defaults if it doesn't exist.
            if !fm.fileExists(atPath: presetConfigURL.path) {
                ...
                // 4. Remove deprecated keys from existing termifier-glass.conf (migration).
            ...
            logger.warning("Failed to apply Termifier glass preset: \(error.localizedDescription, privacy: .public)")
        }
    }

    /// Removes any stale Termifier-managed blocks from the main ghostty config file.
    ...
    private static func cleanupMainGhosttyConfig(fileManager fm: FileManager) {
        ...
        // Remove all known Termifier-managed block types.
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Termifier Visual Defaults (managed) ---",
            endMarker: "# --- End Termifier Visual Defaults ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Termifier Glass Preset (managed) ---",
            endMarker: "# --- End Termifier Glass Preset ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Termifier Include (managed) ---",
            endMarker: "# --- End Termifier Include ---"
        )
        ...
        if cleaned != existingMain {
            do {
                ...
                logger.info("Cleaned stale Termifier managed blocks from ghostty config")
            } catch {
