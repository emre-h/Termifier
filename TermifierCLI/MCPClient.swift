...
//  TermifierCLI
//
//  HTTP client that communicates directly with Termifier BrowserServer.
//
...
struct BrowserClient {
    ...
    /// Read connection info from ~/.config/termifier/browser.json
    static func fromStateFile() throws -> BrowserClient {
        let stateFile = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/termifier/browser.json")
...
enum CLIError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .notRunning:
            return "Termifier is not running. Start Termifier first."
        case .invalidStateFile:
            return "Invalid state file at ~/.config/termifier/browser.json"
