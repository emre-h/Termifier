...
# Builds the termifier-session daemon/CLI binary.
#
...
#
# termifier-session's `vt` crate links a Zig-built shim (`gvt`) against
...
SESSION_DIR="$REPO_ROOT/termifier-session"
...
echo "=== Building termifier-session (host-only, release) ==="
...
rm -f "$OUT_DIR/termifier-session"
cp "$SESSION_DIR/target/release/termifier-session" "$OUT_DIR/termifier-session"
chmod 0755 "$OUT_DIR/termifier-session"

echo "Built $OUT_DIR/termifier-session"
...
for entry in "${REMOTE_TARGETS[@]}"; do
    ...
    echo "=== Building termifier-session ($rust_triple, release) ==="
...
for entry in "${REMOTE_TARGETS[@]}"; do
    ...
    mkdir -p "$dest_dir"
    rm -f "$dest_dir/termifier-session"
    cp "$SESSION_DIR/target/$rust_triple/release/termifier-session" "$dest_dir/termifier-session"
    chmod 0755 "$dest_dir/termifier-session"

    echo "Built $dest_dir/termifier-session"
done
