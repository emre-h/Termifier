...
//  session.newRemote, TermifierWindowController.swift ~537/594) and a button
...
final class AppDelegateSessionBrowserMenuItemTests: XCTestCase {
    ...
    func test_setupMainMenu_doesNotContainCheckForUpdates() throws {
        let appDelegate = AppDelegate()
        appDelegate.setupMainMenu()
        ...
        XCTAssertFalse(
            items.contains(where: { $0.title.localizedCaseInsensitiveContains("check for updates") }),
            "The application menu must not expose the removed in-app updater"
        )
    }
