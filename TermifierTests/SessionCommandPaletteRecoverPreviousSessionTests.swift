...
//  throughout TermifierWindowController for every other AppDelegate-level
...
//    }
//
//  Registered in TermifierWindowController.setupCommandRegistry, alongside
...
//  using TermifierWindowController(restoring: true) so no real ghostty
...
final class SessionCommandPaletteRecoverPreviousSessionTests: XCTestCase {
    ...
    func test_recoverPreservedSession_withNothingPreserved_isSafeNoOpAndLeavesFlagFalse() async throws {
        ...
        setenv("TERMIFIER_UITEST_SESSION_DIR", resolvedDir.path, 1)
        addTeardownBlock {
