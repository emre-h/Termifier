//
//  NSWindowTermifierCloseTests.swift
//  TermifierTests
//
//  GitHub issue #45. Full root-cause writeup:
//  `NSWindow+TermifierClose.swift`'s header comment.
//
//  Exercises `NSWindow.termifierPerformClose(_:)`'s DEFAULT behavior — i.e.
//  the base `NSWindow` extension, not `TermifierWindow`'s override — on a
//  PLAIN, bare `NSWindow` (About panel/Settings/Session Browser stand-in;
//  none of those subclass `TermifierWindow`, so they all fall through to
//  this exact default implementation). Deliberately does NOT construct a
//  `TermifierWindowController` at all: these windows' delegate is this
//  file's own tiny test double, never `TermifierWindowController`, so the
...
//  `TermifierWindowControllerCloseWindowTests`'s header and
//  `AppDelegateCloseAllWindowsTests`'s DANGER note) cannot happen here —
//  there is no `TermifierWindowController` anywhere in this file for that
...
//  correct `termifierPerformClose(_:)` default — see `NSWindow+TermifierClose
...
//
//  What each test pins:
//   - `test_termifierPerformClose_onPlainWindow_queriesWindowShouldClose`
//     pins the fix: the pre-fix stub body was empty, so the delegate was
//     never queried (`windowShouldCloseCallCount` stayed 0, not 1).
//   - `test_termifierPerformClose_onSheetWindow_doesNotQueryWindowShouldClose`
...
@MainActor
final class NSWindowTermifierCloseTests: XCTestCase {
    ...
    private func makeBareWindow() -> NSWindow {
        ...
        // (unlike `TermifierWindow`, which sets `false` in `setupWindow()`):
    ...
    func test_termifierPerformClose_onPlainWindow_queriesWindowShouldClose() {
        ...
        XCTAssertEqual(
            ...
            "termifierPerformClose(_:) must consult windowShouldClose(_:) exactly like performClose(_:) does, " +
    ...
    /// the wrong reason.
    func test_termifierPerformClose_onSheetWindow_doesNotQueryWindowShouldClose() {
        ...
        sheet.termifierPerformClose(nil)
