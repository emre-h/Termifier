...
//  TermifierTests scheme uses Termifier.app as its test HOST, and
//  applicationDidFinishLaunching runs its full production launch body
//  against the developer's real ~/.termifier environment before any test
...
//
//  Proposed API (Termifier/Helpers/LaunchEnvironmentPolicy.swift), sibling to
...
final class LaunchEnvironmentPolicyTests: XCTestCase {
    ...
    func test_xcTestPresent_noUITestingFlag_isUnitTestHost() {
        XCTAssertTrue(
            ...
            "XCTest loaded and no --uitesting flag is exactly the TermifierTests host shape"
        )
    }
    ...
    func test_noXCTest_noUITestingFlag_isNotUnitTestHost() {
        XCTAssertFalse(
            LaunchEnvironmentPolicy.isUnitTestHost(xcTestPresent: false, arguments: ["/path/to/Termifier"]),
    ...
    func test_noXCTest_withUITestingFlag_isNotUnitTestHost() {
        XCTAssertFalse(
            LaunchEnvironmentPolicy.isUnitTestHost(xcTestPresent: false, arguments: ["/path/to/Termifier", "--uitesting"]),
            "the real TermifierUITests app-under-test process shape (--uitesting, no XCTest loaded) must run the full launch"
        )
    }
    ...
    func test_realProcess_insideUnitTestHost_isUnitTestHost() {
        XCTAssertTrue(
            ...
            "this very test process is a unit-test host: XCTest is loaded and the TermifierTests scheme never passes --uitesting"
        )
    }
}
