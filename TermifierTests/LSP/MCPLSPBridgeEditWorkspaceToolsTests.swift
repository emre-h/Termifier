...
//        lsp_workspace_apply_edit       -> Termifier applies the edit locally
...
final class MCPLSPBridgeEditWorkspaceToolsTests: XCTestCase {
    ...
    private let workspaceA = URL(fileURLWithPath: "/tmp/termifier-mcp-lsp-bridge-edit-ws-A")
    private let fileA = "file:///tmp/termifier-mcp-lsp-bridge-edit-ws-A/main.ts"
    ...
    func test_tools_listIncludesEditWorkspaceTools() {
        ...
        XCTAssertEqual(
            names.count,
            70,
            "tools count must be 43 existing + 11 edit+workspace + 13 file-ops/AI + 3 notebook = 70; got \(names.count) names=\(names)"
        )
    }
    ...
    func test_lspWorkspaceExecuteCommand_sendsCorrectLSPRequest() async throws {
        ...
        let args: [String: AnyCodable] = [
            "workspace_root": AnyCodable(workspaceA.path),
            "language_id": AnyCodable("typescript"),
            "command": AnyCodable("termifier.organizeImports"),
        ...
        XCTAssertEqual(captured.count, 1)
        XCTAssertEqual(captured[0]["command"] as? String, "termifier.organizeImports")
