...
//  `Termifier/Features/LSP/LSPSession.swift`.
//
//  Spec entry points:
...
final class LSPSessionServerHandlersTests: XCTestCase {
    ...
    private let testWorkspaceRoot = URL(fileURLWithPath: "/tmp/termifier-lsp-session-server-handlers-test")
    ...
    func test_workspaceWorkspaceFolders_returnsCurrentRoot() async throws {
        let customRoot = URL(fileURLWithPath: "/tmp/termifier-lsp-session-server-handlers-test/sub-root")
