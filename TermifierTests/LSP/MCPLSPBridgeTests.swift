...
//  exposes LSP requests as MCP tools to the `TermifierMCPServer`.
//
...
//  `Termifier/Features/LSP/MCPLSPBridge.swift` (with a sibling file
...
final class MCPLSPBridgeTests: XCTestCase {
    ...
    private let workspaceA = URL(fileURLWithPath: "/tmp/termifier-mcp-lsp-bridge-A")
    private let workspaceB = URL(fileURLWithPath: "/tmp/termifier-mcp-lsp-bridge-B")
    private let fileA = "file:///tmp/termifier-mcp-lsp-bridge-A/main.ts"
    private let fileB = "file:///tmp/termifier-mcp-lsp-bridge-B/main.ts"
