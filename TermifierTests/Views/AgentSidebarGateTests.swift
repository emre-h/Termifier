...
//  visible even while Termifier's own IPC server is stopped, since it
...
//  explanatory "Termifier's own agent monitoring is disabled" notice. Each
...
final class AgentSidebarGateTests: XCTestCase {
    ...
    func test_decide_serverStopped_hasExternal_stillShowsRows() {
        ...
        // visible even while Termifier's own IPC server is off.
        XCTAssertTrue(
    ...
    func test_ipcOnly_showsRows_withNoBanner() {
        XCTAssertTrue(
            ...
            "Termifier's own IPC server running -> rows"
        )
        XCTAssertFalse(
            AgentSidebarGate.showsMonitoringDisabledBanner(isServerRunning: true, hasExternal: false),
            "Termifier's own agent monitoring IS running, so there is nothing to warn about"
        )
    }

    func test_herdrOnly_showsRows_withMonitoringDisabledBanner() {
        // The case this change adds: herdr rows are visible while
        // Termifier's own IPC server is off, but that must not be silent --
        // a user's own locally-run agents (which DO depend on Termifier's
        ...
        XCTAssertTrue(
            ...
            "Termifier's own agent monitoring is OFF while rows are shown from herdr alone -- this must be flagged, " +
            "not silent"
        )
    }
    ...
    func test_both_showsRows_withNoBanner() {
        ...
        XCTAssertFalse(
            ...
            "Termifier's own agent monitoring IS running (in addition to herdr), so there is nothing to warn about"
        )
    }
}
