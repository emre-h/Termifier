...
//  TermifierWindowController.setupUI()/rebuildSplitContainer() to update
//  focusedLeafID AND call requestSave() -- see
//  TermifierWindowControllerFocusSyncTests
...
//  `TermifierWindowController`'s EXISTING closure wiring (unchanged --
...
//  contract exactly. It CANNOT pin that TermifierWindowController's
//  production closures actually route this into `requestSave()` for a
//  REAL window (TermifierWindowController cannot be instantiated with a live
