//
//  TermifierWindowControllerFocusSyncTests.swift
//  TermifierTests
//
...
//    `TermifierWindowController.restoreFocus()` reads the stale
...
//    • Wire `TermifierWindowController` to update
//      `activeTab?.splitTree.focusedLeafID` through the callback.
//
//  Approach:
//    These tests DO NOT instantiate `TermifierWindowController` (which requires
...
@MainActor
final class TermifierWindowControllerFocusSyncTests: XCTestCase {
    ...
    /// `TermifierWindowController` is expected to install.
    private func makeTwoPaneFixture(initialFocus: SplitContainerViewFocus = .first) -> Fixture {
        ...
        // mirrors the `TermifierWindowController` wiring so the closure does not
    ...
    ///        `TermifierWindowController.attemptFocusRestore` (see
    ///        `TermifierWindowController.swift:1022-1024`):
    ...
    func testFocusedReadPathReflectsClick() {
        ...
        XCTAssertTrue(
            ...
            "The TermifierWindowController restore-focus read path must resolve to the clicked pane's SurfaceView"
        )
    }
    ...
    /// `TermifierWindowController.setupUI()` and `rebuildSplitContainer()`)
    ...
    ///   `TermifierWindowController` cannot be instantiated in a unit test
