...
final class ClaudeConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_createsConfigFromScratch() throws {
        ...
        // Then: file is created with correct termifier-ipc entry
        ...
        XCTAssertEqual(termifierIPC?["type"] as? String, "http")
    ...
    func test_enableIPC_addsToExistingConfig() throws {
        ...
        // Then: termifier-ipc is added, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNotNil(mcpServers?["termifier-ipc"], "termifier-ipc should be added")
        XCTAssertNotNil(mcpServers?["other-server"], "other-server should be preserved")
    }

    func test_enableIPC_updatesExistingEntry() throws {
        // Given: termifier-ipc already exists with old token
        let existingJSON = """
        {
            "mcpServers": {
                "termifier-ipc": {
        ...
        // Then: termifier-ipc is updated with new values
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let termifierIPC = mcpServers?["termifier-ipc"] as? [String: Any]
    ...
    func test_enableIPC_createsBackup() throws {
        ...
        XCTAssertFalse(bakContent.contains("termifier-ipc"),
                       "Backup should not contain the newly added termifier-ipc entry")
    }
    ...
    func test_enableIPC_preservesOtherKeys() throws {
        ...
        // And termifier-ipc should be added
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNotNil(mcpServers?["termifier-ipc"], "termifier-ipc should be added")
    }
    ...
    // passive recipient that never calls a termifier-ipc tool (and so never
    // fires the PreToolUse/PostToolUse hook the old binding path relied
    // on). Must be the `${TERMIFIER_SURFACE_ID:-}` empty-default form — an
    ...
    func test_enableIPC_headers_includeSurfaceIDPlaceholderWithEmptyDefault() throws {
        ...
        // Then: headers contain exactly Authorization, X-Termifier-Surface-ID, and
        // X-Termifier-Session-ID, the latter two as literal `${VAR:-}` expansion
        ...
        XCTAssertEqual(headers?["X-Termifier-Surface-ID"], "${TERMIFIER_SURFACE_ID:-}",
                       "X-Termifier-Surface-ID must be the literal ${TERMIFIER_SURFACE_ID:-} placeholder " +
                       "(empty default) so Claude Code's own env expansion fills it per-pane, and an " +
                       "external terminal with no TERMIFIER_SURFACE_ID env still parses the config")
        XCTAssertEqual(headers?["X-Termifier-Session-ID"], "${TERMIFIER_SESSION_ID:-}",
                       "X-Termifier-Session-ID must be present too: a persistent termifier-session daemon " +
                       "leaks a stale TERMIFIER_SURFACE_ID to every session shell it spawns, so " +
                       "TermifierMCPServer needs the stable session ID to tell concurrent Claude Code " +
                       "panes apart")
        XCTAssertEqual(headers?.count, 3,
                       "headers must contain exactly Authorization, X-Termifier-Surface-ID, and " +
                       "X-Termifier-Session-ID, no more")
    }

    func test_enableIPC_updatesExistingEntry_headersIncludeSurfaceIDPlaceholder() throws {
        // Given: termifier-ipc already exists from an older config (single header)
        let existingJSON = """
        {
            "mcpServers": {
                "termifier-ipc": {
        ...
        // Then: the regenerated entry gains the X-Termifier-Surface-ID header too
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let termifierIPC = mcpServers?["termifier-ipc"] as? [String: Any]
        let headers = termifierIPC?["headers"] as? [String: String]
        ...
        XCTAssertEqual(headers?["Authorization"], "Bearer new-token")
        XCTAssertEqual(headers?["X-Termifier-Surface-ID"], "${TERMIFIER_SURFACE_ID:-}",
                       "Re-running enableIPC on an older entry must add X-Termifier-Surface-ID, " +
                       "not just refresh Authorization")
        XCTAssertEqual(headers?["X-Termifier-Session-ID"], "${TERMIFIER_SESSION_ID:-}",
                       "Re-running enableIPC on an entry written before X-Termifier-Session-ID existed must " +
                       "add it too, not just X-Termifier-Surface-ID")
    }

    // MARK: - disableIPC

    func test_disableIPC_removesTermifierEntry() throws {
        // Given: config has termifier-ipc and another server
        let existingJSON = """
        {
            "mcpServers": {
                "termifier-ipc": {
        ...
        // Then: termifier-ipc is removed, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNil(mcpServers?["termifier-ipc"], "termifier-ipc should be removed")
        XCTAssertNotNil(mcpServers?["other-server"], "other-server should be preserved")
    }

    func test_disableIPC_removesMcpServersIfEmpty() throws {
        // Given: config has only termifier-ipc in mcpServers
        let existingJSON = """
        {
            "mcpServers": {
                "termifier-ipc": {
    ...
    func test_isIPCEnabled_trueWhenPresent() {
        // Given: config has termifier-ipc
        let existingJSON = """
        {
            "mcpServers": {
                "termifier-ipc": {
    ...
    func test_isIPCEnabled_falseWhenAbsent() {
        // Given: config has mcpServers but no termifier-ipc
        let existingJSON = """
        {
            "mcpServers": {
                "other-server": {
    ...
    // entirely for that (legitimate, self-authored) setup. Termifier now
    ...
    func test_symlink_followedToRealFile_writesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        XCTAssertNotNil(mcpServers?["termifier-ipc"], "enableIPC must write through the symlink into the real file")
    ...
    func test_multiHopDanglingSymlink_writesToFinalDestinationKeepingBothLinksIntact() throws {
        ...
        XCTAssertNotNil((json?["mcpServers"] as? [String: Any])?["termifier-ipc"])
