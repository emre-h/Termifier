...
final class MCPProtocolTests: XCTestCase {
    ...
    func test_initializeResponse_matchesMCPSpec() throws {
        ...
        XCTAssertEqual(initResult.serverInfo.name, "termifier-ipc",
                       "Server name must be termifier-ipc")
    ...
    // TermifierMCPServerAgentEventTests' `test_initialize_withoutSurfaceHeader_
    // doesNotAutoRegisterPeer`) — an external client with no
    // X-Termifier-Surface-ID (e.g. OpenCode) has no surface for a renamed peer
