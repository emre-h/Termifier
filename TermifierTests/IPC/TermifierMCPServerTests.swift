//
//  TermifierMCPServerTests.swift
//  TermifierTests
//
//  Tests for TermifierMCPServer: the MCP server's handleJSONRPC method
...
@MainActor
final class TermifierMCPServerTests: XCTestCase {
    ...
    /// `TermifierMCPServer` instance in this suite, including the ad-hoc
    ...
    func test_handleJSONRPC_initialize_returnsInitResult() async throws {
        ...
        XCTAssertEqual(serverInfo["name"] as? String, "termifier-ipc",
                       "serverInfo.name must be termifier-ipc")
    ...
    func test_startStop_lifecycle() throws {
        // Arrange
        let srv = TermifierMCPServer(agentEndpointDirectory: agentEndpointDir)
    ...
    // observes AgentRegistry (not TermifierMCPServer, which isn't @Observable)
    ...
    // ~/Library/Application Support/Termifier path in every test in this suite.
    ...
    func test_start_survivesAgentEndpointFileWriteFailure() throws {
        ...
        // ENOTDIR (the lock file itself lives elsewhere, under Termifier's
        ...
        let srv = TermifierMCPServer(agentEndpointDirectory: blockedPath)
        srv.agentRegistry = AgentRegistry()
    ...
    // stop()s. TermifierMCPServer.stop() passes AgentEndpointFile.remove(
    ...
    func test_stop_onNeverStartedServer_doesNotDeleteAnotherServersEndpointFile() throws {
        ...
        let serverA = TermifierMCPServer(agentEndpointDirectory: agentEndpointDir)
        ...
        // TermifierMCPServer instance that some other code path constructed
        // and never wired up.
        let serverB = TermifierMCPServer(agentEndpointDirectory: agentEndpointDir)
    ...
    // unreachable endpoint to both MCP clients and termifier-agent-hook.
    //
    // Real Termifier instances hold 41830-41839 on this host, so both tests
    ...
    /// Find a free loopback port via `TermifierMCPServer.askKernelForFreeLoopbackPort()`
    ...
    /// window, since the real Termifier app may be holding `41830` on this
    /// host.
    ...
    func test_start_preferredPortZero_recordsActualBoundPortAndIsReachable() async throws {
        ...
        XCTAssertNotEqual(
            publishedPort, 0,
            "agent-endpoint.json must not publish port: 0 — termifier-agent-hook cannot reach that"
        )
    ...
    func test_headersOnlyRequest_neverReceivesASecondResponseAfter408() async throws {
        ...
        // itself. See both counters' doc comments on `TermifierMCPServer`.
    ...
    // `TermifierMCPServer.ReceiveAccumulator`'s append loop — must still
...
/// `TermifierMCPServerTests`) so it carries no `@MainActor` isolation and
...
/// `TermifierMCPServer` double-sends on this connection. Callers that only
...
/// `TermifierMCPServer.ReceiveAccumulator` to accumulate one logical
