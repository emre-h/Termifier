//
//  TermifierMCPServerAgentEventTests.swift
//  TermifierTests
//
//  Covers TermifierMCPServer.route(request:): the extracted HTTP
...
//  - Missing / malformed X-Termifier-Surface-ID header → 400
...
@MainActor
final class TermifierMCPServerAgentEventTests: XCTestCase {
    ...
    private func agentEventRequest(
        ...
        if let kindHeader { headers["X-Termifier-Agent-Kind"] = kindHeader }
    ...
    // MARK: - X-Termifier-Agent-Kind propagation (Phase 2)

    func test_route_agentEvent_agentKindHeader_propagatesToRegistryEntryKind() async {
        // Case A: X-Termifier-Agent-Kind: codex must propagate to the entry's kind.
        ...
        XCTAssertEqual(registryWithKindHeader.entries[surfaceWithKindHeader]?.kind, "codex",
                       "X-Termifier-Agent-Kind: codex must propagate to the registry entry's kind")
        ...
        XCTAssertEqual(registryWithoutKindHeader.entries[surfaceWithoutKindHeader]?.kind, "claude-code",
                       "A missing X-Termifier-Agent-Kind header must default to claude-code")
    }
    ...
    func test_route_agentEvent_emptyStringKindHeader_defaultsToClaudeCode() async {
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.kind, "claude-code",
                       "An empty-string X-Termifier-Agent-Kind header must default to claude-code")
    }
    ...
    func test_route_agentEvent_whitespaceOnlyKindHeader_defaultsToClaudeCode() async {
        ...
        XCTAssertEqual(response.statusCode, 204)
        XCTAssertEqual(registry.entries[surfaceID]?.kind, "claude-code",
                       "A whitespace-only X-Termifier-Agent-Kind header must default to claude-code")
    }
    ...
    func test_agentEventBinding_postToolUseRegisterPeerResponse_bindsSurface_firstMessageLightsUpBadge() async throws {
        ...
        // one of the other termifier-ipc tools — so a message sent to a pane
        ...
        let bindingRequest = agentEventRequest(
            ...
            {"hook_event_name":"PostToolUse","session_id":"s1",
             "tool_name":"mcp__termifier-ipc__register_peer",
    ...
    func test_agentEventBinding_lspToolCall_doesNotRefreshInboxCounts_messagingToolDoes() async throws {
        ...
        XCTAssertEqual(registry.entries[boundSurface]?.unreadCount, 1,
                       "A termifier-ipc messaging tools/call (get_peer_status here) must still trigger " +
                       "syncBoundPeerInboxCounts and pick up the count left stale by the earlier lsp_* call")
    }

    // MARK: - MCP-connection surface binding (X-Termifier-Surface-ID
    // on /mcp itself), so a passive recipient that never calls a
    // termifier-ipc tool still gets its unread badge lit.
    //
    // All three tests below drive `initialize`/`register_peer` through
    // `route(request:)` with an `HTTPRequest` carrying the
    // `X-Termifier-Surface-ID` header, exactly as Claude Code's own MCP
    ...
    /// (optionally) an `X-Termifier-Surface-ID` header, mirroring
    ...
    func test_userScenario_bothPanesInitializeWithSurfaceHeader_passiveRecipientGetsBadgeWithoutCallingAnyTool() async throws {
        ...
        // ClaudeConfigManager emits `X-Termifier-Surface-ID` on the termifier-ipc
        // MCP entry.
        ...
        // termifier-ipc tool itself, so the PreToolUse/
        ...
        XCTAssertEqual(registry.entries[surfaceB]?.unreadCount, 1,
                       ...
                       "never called a termifier-ipc tool")
    ...
    // gated on a valid `X-Termifier-Surface-ID` header (see
        ...
        for testCase in cases {
            ...
            guard testCase.shouldBind else {
                ...
                XCTAssertFalse(instructions.contains("Your peer_id is:"),
                               "[\(testCase.label)] an invalid/missing X-Termifier-Surface-ID header must not " +
            ...
            XCTAssertEqual(registry.entries[surfaceID]?.unreadCount ?? 0, 1,
                           "[\(testCase.label)] a valid X-Termifier-Surface-ID header must auto-register and " +
    ...
    func test_registerPeerToolCall_surfaceHeader_bindsNewPeerToSurface() async throws {
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.unreadCount, 1,
                       "register_peer's own X-Termifier-Surface-ID header must bind the newly created " +
    ...
    func test_initialize_surfaceAlreadyBound_secondInitializeDoesNotDisruptMessageDelivery() async throws {
        ...
        // same TERMIFIER_SURFACE_ID env var) resolves to the SAME peer — the
    ...
    func test_surfaceIDHeader_leadingTrailingWhitespace_parsesIdenticallyOnMCPAndAgentEvent() async throws {
        ...
        XCTAssertEqual(sessionStart.statusCode, 204,
                       "A padded X-Termifier-Surface-ID must still parse on /agent-event, not 400")
        XCTAssertEqual(registry.entries[surfaceID]?.sessionID, "session-1",
        ...
        XCTAssertEqual(sendResponse.statusCode, 200)
        XCTAssertEqual(registry.entries[surfaceID]?.unreadCount, 1,
                       "A padded X-Termifier-Surface-ID on /mcp's initialize must bind the trimmed UUID, " +
    ...
    func test_registerPeer_noSurfaceHeader_alwaysRegistersNewPeer() async throws {
        ...
        // An external MCP client without X-Termifier-Surface-ID (e.g. a
    ...
    // external MCP client with no X-Termifier-Surface-ID (e.g. OpenCode) has
    ...
    // valid X-Termifier-Surface-ID is present; instructions retain the
    ...
    func test_initialize_withoutSurfaceHeader_doesNotAutoRegisterPeer() async throws {
        ...
        // MCP client that can't send X-Termifier-Surface-ID.
        let initResponse = await server.route(request: mcpRequest(
        ...
        XCTAssertEqual(countAfter, countBefore,
                       "initialize without X-Termifier-Surface-ID must not auto-register a new peer — " +
    ...
    func test_registerPeer_rename_missingRoleArgument_preservesExistingRole() async throws {
        ...
        // TermifierMCPServer's `initialize` case) — the value the omitted
    ...
    // elsewhere in `TermifierMCPServer` (`parseSurfaceID`, the
    // `X-Termifier-Agent-Kind` handling in `routeAgentEvent`).
    ...
    // without a real termifier-session daemon.
    ...
    /// termifier-session ID, so `recordAgentSession` would actually reach
    ...
    func test_routeAgentEvent_payloadOverTheOldSharedCap_stillReturns204() async {
        ...
        // contents through termifier-agent-hook's verbatim stdin forwarding.
