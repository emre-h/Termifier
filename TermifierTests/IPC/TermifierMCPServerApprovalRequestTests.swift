...
//    true }`
//  - Unknown X-Termifier-Agent-Kind and the agentHookApprovalEnabled toggle
...
@MainActor
final class TermifierMCPServerApprovalRequestTests: XCTestCase {

    // MARK: - Properties

    private var server: TermifierMCPServer!
    private let testToken = "approval-request-test-token"
    private let settingsSuiteName = "com.termifier.tests.TermifierMCPServerApprovalRequestTests"

    /// Test-isolated `agent-endpoint.json` directory, passed to every
    /// `TermifierMCPServer` instance in this suite via the required
    /// `agentEndpointDirectory` init parameter -- same rationale as
    /// TermifierMCPServerAgentEventTests: this suite never calls `start()`,
    ...
    /// surface -- mirrors this codebase's other `TermifierMCPServer`
    ...
    func test_route_unknownKind_returns200EmptyBody_andSubmitsNothing() async {
        ...
        XCTAssertEqual(response.statusCode, 200,
                       "an unrecognized X-Termifier-Agent-Kind must still return 200, not an error status")
        XCTAssertNil(response.body, "an unrecognized X-Termifier-Agent-Kind must return an EMPTY body")
        XCTAssertTrue(approvalInbox.pending.isEmpty,
                      "an unrecognized X-Termifier-Agent-Kind must never submit a request to the inbox, " +
    ...
    /// pre-migration Termifier install's synchronous approval hook entry
    ...
    func test_route_pending_submitsAgentHookRequest_withSurfaceAndSummary() async throws {
        ...
        XCTAssertEqual(pendingRequest.targetSurfaceID, surfaceID,
                       "targetSurfaceID must be the surface resolved from X-Termifier-Surface-ID")
        ...
        case .agentHook(let toolName, let kind, let summary, let offers):
            ...
            XCTAssertEqual(kind, AgentEntry.claudeCodeKind,
                           "a missing X-Termifier-Agent-Kind header must default to claude-code, same as /agent-event")
            XCTAssertEqual(summary, command, "Bash's summary must be its tool_input.command")
            XCTAssertFalse(offers.cliOwnsPersistence,
                           "no CLI always-allow row was offered, so Termifier's own pane memory is the only " +
    ...
    func test_route_defaultTimeout_is570000ms() {
        let freshServer = TermifierMCPServer(agentEndpointDirectory: agentEndpointDir)
    ...
    /// No new seam on `TermifierMCPServer` is needed -- this existing one
    ...
    /// back to Termifier's own pane-scoped Always-Allow row.
