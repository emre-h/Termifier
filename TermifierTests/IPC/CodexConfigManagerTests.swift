...
final class CodexConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_createsNewFile() throws {
        ...
        XCTAssertTrue(content.contains("[mcp_servers.termifier-ipc]"))
        XCTAssertTrue(content.contains("http://127.0.0.1:41830/mcp"))
        XCTAssertTrue(content.contains("Bearer abc123"))
        XCTAssertTrue(
            content.contains("\"X-Termifier-Agent-Kind\" = \"codex\""),
            "Codex's MCP connection must identify itself so initialize can create the Agents row"
        )
        XCTAssertTrue(
            content.contains(
                "env_http_headers = { \"X-Termifier-Surface-ID\" = \"TERMIFIER_SURFACE_ID\", " +
                "\"X-Termifier-Session-ID\" = \"TERMIFIER_SESSION_ID\" }"
            ),
            "Codex must read both the surface ID and the stable session ID from its own " +
            "environment; a persistent termifier-session daemon leaks a stale TERMIFIER_SURFACE_ID to " +
            "every session shell it spawns, so TermifierMCPServer needs TERMIFIER_SESSION_ID to tell " +
    ...
    func test_enableIPC_addsToExistingFile() throws {
        ...
        // Then: original content preserved and termifier-ipc appended
        let content = readConfig()
    ...
    func test_enableIPC_updatesExistingEntry() throws {
        // Given: file already has termifier-ipc section with old port/token
        let existing = """
    ...
    func test_enableIPC_upgradesPreExistingSingleHeaderSection_exactlyOneSectionWithBothHeaders() throws {
        // Given: file already has a termifier-ipc section written before
        // TERMIFIER_SESSION_ID existed -- the old single-header form.
        let existing = """
        [mcp_servers.termifier-ipc]
        url = "http://localhost:40000/mcp"
        http_headers = { "Authorization" = "Bearer old-token" }
        ...
        // Then: exactly one termifier-ipc section, carrying both headers
        let content = readConfig()
        let headerCount = content.components(separatedBy: "[mcp_servers.termifier-ipc]").count - 1
        ...
        XCTAssertFalse(
            content.contains("env_http_headers = { \"X-Termifier-Surface-ID\" = \"TERMIFIER_SURFACE_ID\" }"),
    ...
    func test_disableIPC_removeSection() throws {
        // Given: file has termifier-ipc section
        let existing = """
        [mcp_servers.termifier-ipc]
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc]"))
        XCTAssertFalse(content.contains("http://localhost:41830/mcp"))
    }

    func test_disableIPC_preservesOtherMCPServers() throws {
        // Given: file has termifier-ipc and another mcp server
        let existing = """
        [mcp_servers.other]
        url = "http://localhost:9999/mcp"
        ...
        // Then: termifier-ipc removed, other preserved
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc]"))
        XCTAssertTrue(content.contains("[mcp_servers.other]"))
        XCTAssertTrue(content.contains("http://localhost:9999/mcp"))
    ...
    func test_disableIPC_noSection() throws {
        // Given: file exists but has no termifier-ipc section
        let existing = """
        [general]
        model = "gpt-4"
        """
    ...
    func test_disableIPC_unreadableFile() throws {
        ...
        writeConfig("[mcp_servers.termifier-ipc]\nurl = \"http://localhost:41830/mcp\"\n")
    ...
    // rejection silently broke IPC configuration for that setup. Termifier
    ...
    func test_enableIPC_symlinkFollowedToRealFile_writesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        XCTAssertTrue(realContent.contains("[mcp_servers.termifier-ipc]"),
    ...
    func test_disableIPC_symlinkFollowedToRealFile_removesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        let realContent = try String(contentsOfFile: realFile, encoding: .utf8)
        XCTAssertFalse(realContent.contains("[mcp_servers.termifier-ipc]"),
    ...
    func test_enableIPC_duplicateSections() throws {
        // Given: file has TWO termifier-ipc sections
        let existing = """
        [mcp_servers.termifier-ipc]
        url = "http://localhost:40000/mcp"
    ...
    func test_enableIPC_arrayOfTablesIgnored() throws {
        ...
        // Then: termifier-ipc's own body is removed, but [[array_of_tables]]
        // (a table header, hence a section boundary) and everything under
        // it survives.
        let content = readConfig()
        XCTAssertTrue(content.contains("[[array_of_tables]]"),
                       "[[array_of_tables]] is a table header and ends the termifier-ipc section")
    ...
    // MARK: - Regression (Phase 2): [[hooks.*]] adjacent to termifier-ipc section
    ...
    func test_disableIPC_preservesAdjacentCodexHooksArrayOfTablesBlock() throws {
        ...
        // block immediately following `[mcp_servers.termifier-ipc]` was never
        // recognized as the section's boundary and got swallowed as if it
        // were part of the termifier-ipc section body.
        let existing = """
        [mcp_servers.termifier-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc]"), "termifier-ipc section must be removed")
        XCTAssertTrue(content.contains("[[hooks.SessionStart]]"),
                      "A [[hooks.*]] array-of-tables block immediately after [mcp_servers.termifier-ipc] " +
    ...
    func test_disableIPC_preservesAdjacentTermifierAgentHooksManagedBlock() throws {
        // Regression: the same bug applied to Termifier's own future
        // CodexHooksConfigManager-managed block — its BEGIN marker line is
        // an ordinary comment line (not a table header), so it too got
        // swallowed by the termifier-ipc section removal.
        let existing = """
        [mcp_servers.termifier-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }

        # BEGIN TERMIFIER AGENT HOOKS (managed by Termifier, do not edit)
        [[hooks.SessionStart]]
        [[hooks.SessionStart.hooks]]
        type = "command"
        command = '"/path/to/termifier-agent-hook" codex'
        timeout = 5
        # END TERMIFIER AGENT HOOKS
        """
        writeConfig(existing)
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc]"), "termifier-ipc section must be removed")
        XCTAssertTrue(content.contains("# BEGIN TERMIFIER AGENT HOOKS"),
                      "The managed block's BEGIN marker line itself must survive disableIPC")
        XCTAssertTrue(content.contains("[[hooks.SessionStart]]"))
        XCTAssertTrue(content.contains("# END TERMIFIER AGENT HOOKS"),
    ...
    func test_disableIPC_noBlankLineBeforeHooksArrayOfTablesBlock_survives() throws {
        ...
        // immediately following termifier-ipc with NO blank-line separator must
    ...
    func test_disableIPC_noBlankLineBeforeTermifierAgentHooksMarker_survives() throws {
        // Regression: same as above, but for the `# BEGIN TERMIFIER` managed
        // block marker directly abutting termifier-ipc with no blank line.
        let existing = """
        [mcp_servers.termifier-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
    ...
    func test_disableIPC_removesTermifierIpcSubTables() throws {
        // A [mcp_servers.termifier-ipc.*] / [[mcp_servers.termifier-ipc.*]]
        // sub-table header is still part of the termifier-ipc entry per TOML's
        ...
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
        [mcp_servers.termifier-ipc.env]
        FOO = "bar"
        [[mcp_servers.termifier-ipc.extra]]
        name = "baz"
        [other.section]
        key = "value"
        """
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc]"))
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc.env]"))
        XCTAssertFalse(content.contains("FOO = \"bar\""))
        XCTAssertFalse(content.contains("[[mcp_servers.termifier-ipc.extra]]"))
    ...
    func test_concurrentEnableDisable_noCorruption() throws {
        ...
        if FileManager.default.fileExists(atPath: configPath) {
            ...
            // If termifier-ipc section exists, it should have the url key
            if content.contains("[mcp_servers.termifier-ipc]") {
                XCTAssertTrue(content.contains("url = \"http://127.0.0.1:"),
                              "termifier-ipc section should have url key")
            }
        }
