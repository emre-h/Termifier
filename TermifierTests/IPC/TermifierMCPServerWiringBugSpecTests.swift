//
//  TermifierMCPServerWiringBugSpecTests.swift
//  TermifierTests
//
//  Race-safety regression tests for the `TermifierMCPServer` start/stop
...
@MainActor
final class TermifierMCPServerWiringBugSpecTests: XCTestCase {

    // MARK: - Properties

    /// Each test uses a dedicated instance instead of `.shared` so the
    /// global server state observed elsewhere in the suite (and by
    /// production code in `TermifierWindowController`) is not perturbed.
    ...
    /// `TermifierMCPServerLSPIntegrationTests.waitUntil`.
    @discardableResult
