...
// - Strict managed-block detection (BEGIN + END + `termifier-ipc:` all required)
...
final class HermesConfigManagerTests: XCTestCase {
    ...
    private let beginLine = "# BEGIN TERMIFIER IPC (managed by Termifier, do not edit)"
    /// Canonical END line literal as written by enableIPC.
    private let endLine = "# END TERMIFIER IPC"
    ...
    func test_enableIPC_createsConfigFromScratch() throws {
        ...
        XCTAssertTrue(content.contains("termifier-ipc:"), "Should contain termifier-ipc: key")
        ...
        XCTAssertTrue(content.contains("X-Termifier-Surface-ID: \"${TERMIFIER_SURFACE_ID}\""),
                      "Hermes must forward the ordinary Termifier surface identity")
        ...
        XCTAssertTrue(content.contains("X-Termifier-Agent-Kind: \"hermes\""),
    ...
    func test_enableIPC_appendsToFileWithoutMcpServers() throws {
        ...
        XCTAssertTrue(content.contains("termifier-ipc:"),
                      "Managed block must contain termifier-ipc:")
    ...
    func test_enableIPC_insertsIntoExistingMcpServers() throws {
        ...
        XCTAssertTrue(content.contains("termifier-ipc:"),
                      "termifier-ipc child should be added")
        // Marker comments are indented to child level (2 spaces) for Case B.
        XCTAssertTrue(content.contains("  # BEGIN TERMIFIER IPC"),
                      "BEGIN marker should be indented 2 spaces (child level)")
        XCTAssertTrue(content.contains("  # END TERMIFIER IPC"),
    ...
    func test_enableIPC_insertsWith4SpaceIndent() throws {
        ...
        XCTAssertTrue(content.contains("    termifier-ipc:"),
                      "termifier-ipc: key should be at 4-space indent")
    }
    ...
    func test_enableIPC_repeatedUpdatePreservesFollowingTopLevelSection() throws {
        ...
        mcp_servers:
          stripe:
            url: "https://mcp.stripe.com"
          # BEGIN TERMIFIER IPC (managed by Termifier, do not edit)
          termifier-ipc:
    ...
    // config managers. Termifier now follows the link and writes through to
    ...
    func test_disableIPC_removesManagedBlock_preservesUserContent() throws {
        ...
        XCTAssertFalse(content.contains("termifier-ipc:"),
                       "termifier-ipc key should be removed")
    }
    ...
    func test_disableIPC_keepsExistingMcpServers() throws {
        ...
        XCTAssertTrue(afterEnable.contains("termifier-ipc:"))
        ...
        XCTAssertFalse(content.contains("termifier-ipc:"),
                       "termifier-ipc child should be removed")
    }

    func test_disableIPC_removesEmptyMcpServersWhenOnlyChild() throws {
        // Given: Case A setup — mcp_servers: was created by enableIPC and contains only termifier-ipc
        try HermesConfigManager.enableIPC(port: 41830, token: "tok", configPath: configPath)
        ...
        XCTAssertFalse(content.contains("termifier-ipc:"),
                       "termifier-ipc: should be removed")
        XCTAssertFalse(content.contains(beginLine),
                       "BEGIN line should be removed")
        XCTAssertFalse(content.contains(endLine),
                       "END line should be removed")
    ...
    func test_disableIPC_throwsOnMalformedBlock_missingTermifierIpc() throws {
        // Given: pre-existing BEGIN/END pair but NO termifier-ipc: key between them
        let existing = """
        agent_name: "hermes"
        # BEGIN TERMIFIER IPC (managed by Termifier, do not edit)
        # someone deleted the termifier-ipc body
        # END TERMIFIER IPC
        max_tokens: 4096
        """
        try writeConfig(existing)
    ...
    func test_isIPCEnabled_falseForBeginOnly() throws {
        // Given: BEGIN line only (no END, no termifier-ipc body in well-formed pair)
        let existing = """
        # BEGIN TERMIFIER IPC (managed by Termifier, do not edit)
        mcp_servers:
          termifier-ipc:
    ...
    func test_isIPCEnabled_falseForBeginEndWithoutTermifierIpc() throws {
        // Given: BEGIN/END pair but no termifier-ipc: between
        let existing = """
        # BEGIN TERMIFIER IPC (managed by Termifier, do not edit)
        # nothing useful here
        # END TERMIFIER IPC
    ...
    func test_isIPCEnabled_ignoresUserCommentMentioningBeginTermifierIpc() throws {
        // Given: a user-written line that mentions "BEGIN TERMIFIER IPC" mid-line —
        // this is NOT a real managed block start because the regex requires line-start.
        let existing = """
        agent_name: "hermes"
        # something # BEGIN TERMIFIER IPC blah blah — note from a user, not a real marker
        mcp_servers:
          termifier-ipc:
            url: "http://localhost:1111/mcp"
        """
        ...
        // When/Then: not a real managed block (no real BEGIN/END pair surrounding termifier-ipc)
        XCTAssertFalse(HermesConfigManager.isIPCEnabled(configPath: configPath),
                       "isIPCEnabled should ignore mid-line mentions of BEGIN TERMIFIER IPC")
    }
}
