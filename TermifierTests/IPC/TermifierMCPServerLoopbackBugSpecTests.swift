//
//  TermifierMCPServerLoopbackBugSpecTests.swift
//  TermifierTests
//
//  Regression tests for two related defects in the IPC MCP server
//  surface area:
//
//    1. Dual-stack loopback mismatch. `TermifierMCPServer` binds its
...
//
//  Both tests live under `TermifierTests/IPC/` to sit next to the existing
//  `TermifierMCPServerWiringBugSpecTests` — the established home for
...
@MainActor
final class TermifierMCPServerLoopbackBugSpecTests: XCTestCase {

    // MARK: - Properties

    /// Each test uses its own server instance to avoid perturbing
    /// `TermifierMCPServer.shared` (which `TermifierWindowController` and other
    /// suites observe).
    ...
    override func setUp() {
        ...
        server = TermifierMCPServer(agentEndpointDirectory: tempDir)
    ...
    private func readPublishedURL() throws -> String {
        ...
        let termifierEntry = try XCTUnwrap(
            mcpServers["termifier-ipc"] as? [String: Any],
            "published config must contain a `termifier-ipc` entry"
        )
        return try XCTUnwrap(
            termifierEntry["url"] as? String,
            "termifier-ipc entry must contain a `url` field"
        )
    }
    ...
    /// the helper shape in `TermifierMCPServerWiringBugSpecTests`.
    @discardableResult
    ...
    /// caps the server at 10 simultaneous Termifier instances and leaves no
    ...
    ///      `TermifierMCPServer.start`'s linear scan to fail on every
    ///      iteration.
