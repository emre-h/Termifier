...
final class OpenCodeConfigManagerTests: XCTestCase {
    ...
    /// BEGIN delimiter marker for the Termifier IPC managed block in AGENTS.md.
    private let beginDelimiter = "<!-- BEGIN TERMIFIER IPC"
    /// END delimiter marker for the Termifier IPC managed block in AGENTS.md.
    private let endDelimiter = "<!-- END TERMIFIER IPC -->"
    ...
    func test_enableIPC_createsOpenCodeJSONFromScratch() throws {
        ...
        XCTAssertNotNil(termifierIPC, "termifier-ipc entry should exist")
        XCTAssertEqual(termifierIPC?["type"] as? String, "remote",
                       "OpenCode MCP entry type should be 'remote'")
        XCTAssertEqual(termifierIPC?["url"] as? String, "http://127.0.0.1:41830/mcp")
        ...
        XCTAssertEqual(headers?["X-Termifier-Agent-Kind"], "opencode")
    }
    ...
    func test_enableIPC_createsAgentsMDFromScratch() throws {
        ...
        XCTAssertTrue(content.contains(beginDelimiter),
                      "AGENTS.md should contain BEGIN TERMIFIER IPC delimiter")
        XCTAssertTrue(content.contains(endDelimiter),
                      "AGENTS.md should contain END TERMIFIER IPC delimiter")
    ...
    func test_enableIPC_preservesOtherMCPServers() throws {
        ...
        // Then: other-server preserved, termifier-ipc added
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNotNil(mcp?["other-server"], "other-server should be preserved")
        XCTAssertNotNil(mcp?["termifier-ipc"], "termifier-ipc should be added")
    ...
    func test_enableIPC_preservesOtherTopLevelKeys() throws {
        ...
        // Then: theme and model still present, termifier-ipc added
        let dict = try readOpenCodeDict()
    ...
    func test_enableIPC_upsertsExistingTermifierEntry() throws {
        // Given: opencode.json already has termifier-ipc with old port/token
        let existingJSON = """
        {
            "mcp": {
                "termifier-ipc": {
        ...
        // Then: termifier-ipc is REPLACED with new values (not duplicated, not merged)
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertEqual(mcp?.count, 1, "There should be only one entry under mcp (no duplicates)")

        let termifierIPC = mcp?["termifier-ipc"] as? [String: Any]
        XCTAssertEqual(termifierIPC?["url"] as? String, "http://127.0.0.1:55555/mcp",
    ...
    func test_enableIPC_correctURLFormat() throws {
        ...
        let mcp = dict["mcp"] as? [String: Any]
        let termifierIPC = mcp?["termifier-ipc"] as? [String: Any]
        XCTAssertEqual(termifierIPC?["url"] as? String, "http://127.0.0.1:12345/mcp",
    ...
    func test_enableIPC_preservesAgentsMDUserContent() throws {
        ...
        // Then: user content preserved AND Termifier IPC block present
        let content = try readAgentsMD()
        ...
        XCTAssertTrue(content.contains("- write tests"),
                      "User bullet should be preserved")
        XCTAssertTrue(content.contains(beginDelimiter),
                      "Termifier IPC BEGIN delimiter should be appended")
        XCTAssertTrue(content.contains(endDelimiter),
                      "Termifier IPC END delimiter should be appended")
        XCTAssertTrue(content.contains(canarySubstring),
                      "Termifier IPC canary substring should be present")
    }

    func test_enableIPC_replacesExistingTermifierBlock() throws {
        // Given: AGENTS.md already contains a (stale) Termifier IPC block, plus user content
        let stale = """
        # User Rules

        do what you need to

        \(beginDelimiter) (managed by Termifier, do not edit) -->
        ## Termifier IPC
    ...
    func test_enableIPC_symlinkFollowedToRealFile_opencodeJson() throws {
        ...
        // Then: the real file received the termifier-ipc entry...
        ...
        XCTAssertNotNil(mcp?["termifier-ipc"], "enableIPC must write through the symlink into the real opencode.json")
    ...
    func test_enableIPC_partialFailure_jsonWrittenAgentsFailed() throws {
        ...
        XCTAssertNotNil(mcp?["termifier-ipc"], "partial-failure leaves termifier-ipc entry in opencode.json")
    }
    ...
    func test_disableIPC_removesTermifierFromOpenCodeJSON() throws {
        // Given: opencode.json has termifier-ipc and another server
        let existingJSON = """
        {
            "mcp": {
                "termifier-ipc": {
        ...
        // Then: termifier-ipc removed, other-server preserved
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["termifier-ipc"], "termifier-ipc should be removed")
        XCTAssertNotNil(mcp?["other-server"], "other-server should be preserved")
    }

    func test_disableIPC_removesMcpKeyIfEmpty() throws {
        // Given: opencode.json has only termifier-ipc under mcp, plus another top-level key
        let existingJSON = """
        {
            "mcp": {
                "termifier-ipc": {
    ...
    func test_disableIPC_removesBlockFromAgentsMD() throws {
        // Given: AGENTS.md has user content surrounding a Termifier IPC block
    ...
    func test_disableIPC_onlyOpenCodeJsonExists() throws {
        // Given: only opencode.json exists (with termifier-ipc)
        let existingJSON = """
        {
            "mcp": {
                "termifier-ipc": {
        ...
        // Then: opencode.json has termifier-ipc removed; AGENTS.md still missing
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["termifier-ipc"], "termifier-ipc should be removed")
        XCTAssertFalse(FileManager.default.fileExists(atPath: agentsMDPath),
    ...
    func test_disableIPC_onlyAgentsMDExists() throws {
        // Given: only AGENTS.md exists (with Termifier block)
        let existing = """
        # Header
    ...
    func test_disableIPC_symlinkFollowedToRealFile() throws {
        ...
        let realDict = try JSONSerialization.jsonObject(with: realData) as? [String: Any]
        let mcp = realDict?["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["termifier-ipc"], "disableIPC must remove the entry from the real file reached through the symlink")
    ...
    func test_isIPCEnabled_trueWhenPresent() {
        // Given: opencode.json has termifier-ipc entry
        let existingJSON = """
        {
            "mcp": {
                "termifier-ipc": {
    ...
    func test_isIPCEnabled_falseWhenAbsent() {
        // Given: opencode.json has mcp but no termifier-ipc
        let existingJSON = """
        {
            "mcp": {
                "other-server": {
