...
//  file system, real TermifierMCPServer, or any networking.
//
...
/// mirrors TermifierMCPServer.stop()'s own effect on this seam's three
...
final class IPCActivationCoordinatorTests: XCTestCase {
    ...
    func test_enable_serverAlreadyRunning_reusesEndpointWithoutRestartOrNewToken() {
        ...
        // it would tear down connected agents (TermifierMCPServer.start stops
    ...
    func test_disableThenEnable_takesFreshStartBranch() {
        ...
        // the real TermifierMCPServer.stop() does, so the following enable()
