//
//  TermifierMCPServerLSPIntegrationTests.swift
//  TermifierTests
//
//  Integration tests for the merge of the LSP tool surface (MCPLSPBridge)
//  into the existing IPC-flavoured MCP server (TermifierMCPServer / MCPRouter).
//
//  Scope:
...
//  - `TermifierMCPServer.lspBridge` is `nil` until `startLSP()` runs.
...
//  `MCPRouter.isLSPTool`,
//  `TermifierMCPServer.lspBridge`, `TermifierMCPServer.startLSP()`, and
//  `TermifierMCPServer._testInjectLSPBridge(_:)`, declared in
//  `Termifier/Features/IPC/MCPProtocol.swift` and
//  `Termifier/Features/IPC/TermifierMCPServer.swift`. Every
...
/// `TermifierMCPServer` can dispatch real `lsp_*` tool calls without a
...
// MARK: - TermifierMCPServerLSPIntegrationTests

@MainActor
final class TermifierMCPServerLSPIntegrationTests: XCTestCase {

    // MARK: - Properties

    private var server: TermifierMCPServer!
    private let testToken = "lsp-integration-token"
    private let workspace = URL(fileURLWithPath: "/tmp/termifier-lsp-integration")
    private let fileURI = "file:///tmp/termifier-lsp-integration/main.ts"
    ...
    func test_termifierMCPServer_lspBridge_isNilBeforeStartLSP() {
        XCTAssertNil(server.lspBridge,
                     "lspBridge must be nil before startLSP() is called")
    }

    // 7. After startLSP(), lspBridge is non-nil.
    func test_termifierMCPServer_startLSP_initializesBridge() async {
    ...
    func test_termifierMCPServer_testInjectLSPBridge_replacesBridge() async {
    ...
    func test_termifierMCPServer_handleConnection_lspToolCall_routesToBridge() async throws {
    ...
    func test_termifierMCPServer_handleConnection_unknownLSPTool_returnsError() async throws {
        // Arrange
        let (bridge, _) = await makeBridge()
        server._testInjectLSPBridge(bridge)
    ...
    func test_termifierMCPServer_handleConnection_ipcToolCall_stillWorks() async throws {
    ...
    func test_termifierMCPServer_handleConnection_lspBridgeNotStarted_returnsError() async throws {
