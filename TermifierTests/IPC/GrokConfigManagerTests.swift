...
//  `[mcp_servers.termifier-ipc]` entry in `~/.grok/config.toml`.
//
//  Grok's remote-MCP shape differs from Codex's in two ways this suite
//  pins: headers live in a `[mcp_servers.termifier-ipc.headers]` sub-table
...
//  as `${TERMIFIER_SURFACE_ID:-}` / `${TERMIFIER_SESSION_ID:-}` literals rather
...
//  same sub-table, same four header keys, no Termifier ownership marker.
...
final class GrokConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_writesSectionHeadersSubTableAndEnvInterpolatedIdentities() throws {
        ...
        XCTAssertTrue(content.contains("[mcp_servers.termifier-ipc.headers]"),
        ...
        XCTAssertTrue(content.contains("X-Termifier-Agent-Kind = \"grok\""),
        ...
        XCTAssertTrue(content.contains("X-Termifier-Session-ID = \"${TERMIFIER_SESSION_ID:-}\""),
                      "A persistent pane's termifier-session ID outlives its surface UUID, and the server " +
                      "resolves X-Termifier-Session-ID first")
    }
    ...
    func test_enableIPC_adoptsUnmarkedExistingEntry_withoutDuplicating() throws {
        ...
        // Termifier marker, already in the managed layout, stale port/token.
        writeConfig("""
        [compat.claude]
        hooks = true
        ...
        X-Termifier-Agent-Kind = "grok"
        """)

        try GrokConfigManager.enableIPC(port: 41830, token: dummyToken, configPath: configPath)

        let content = readConfig()
        XCTAssertEqual(occurrences(of: "[mcp_servers.termifier-ipc]", in: content), 1,
        ...
        XCTAssertEqual(occurrences(of: "[mcp_servers.termifier-ipc.headers]", in: content), 1,
    ...
    /// TOML lets `[mcp_servers.termifier-ipc.headers]` sit anywhere in the
    /// file, so a hand-written entry can separate it from its own
    /// `[mcp_servers.termifier-ipc]` parent. A sub-table left behind on that
    /// layout is not a cosmetic leftover: the fresh entry appends a
    /// second `[mcp_servers.termifier-ipc.headers]`, defining one TOML table
    ...
    func test_enableIPC_subTableSeparatedFromParent_leavesOneHeadersTable() throws {
        ...
        XCTAssertTrue(content.contains("theme = \"dark\""),
                      "Only the termifier-ipc entry is removed, never the body of the table between them")
    }
    ...
    func test_disableIPC_subTableSeparatedFromParent_removesItAndItsToken() throws {
        ...
        XCTAssertFalse(content.contains("[mcp_servers.termifier-ipc.headers]"),
    ...
    func test_enableIPC_parentDirectoryMissing_throwsDirectoryNotFound() {
        ...
        XCTAssertThrowsError(
            ...
            "Grok not being installed must never make Termifier create ~/.grok on the user's behalf"
        ) { error in
    ...
    func test_disableIPC_removesEntryAndItsHeadersSubTable_preservingNeighbours() throws {
        ...
        XCTAssertTrue(content.contains("[[hooks.Stop]]"),
                      "An array-of-tables header ends the termifier-ipc entry and must survive removal")
    }
