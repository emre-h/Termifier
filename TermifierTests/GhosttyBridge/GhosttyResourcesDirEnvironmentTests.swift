...
//  §3). Termifier's own bundled ghostty resources are version-matched to the
...
//  match the libghostty version Termifier actually embeds.
//
//  Under test: GhosttyResourcesDirEnvironment.
//
//  Proposed API (Termifier/GhosttyBridge/GhosttyResourcesDirEnvironment.swift):
//
...
//
//  Environment save/restore mirrors the TERMIFIER_SESSION_BIN idiom already
//  established by AppDelegateRestoreRemoteSessionTests/
//  TermifierWindowControllerRemoteReconnectCommandTests (save the original
...
final class GhosttyResourcesDirEnvironmentTests: XCTestCase {
    ...
    func test_apply_withResolvedPath_overwritesPreExistingBogusValue() {
        ...
        GhosttyResourcesDirEnvironment.apply("/opt/termifier-fixture/Resources/ghostty")

        XCTAssertEqual(
            currentValue(), "/opt/termifier-fixture/Resources/ghostty",
            "a resolved bundled path must overwrite an inherited value, since Termifier's bundled scripts are version-matched to the embedded libghostty and an inherited path may not be"
        )
    }
    ...
    func test_apply_withResolvedPath_setsVariableWhenNoneWasSetBefore() {
        ...
        XCTAssertEqual(currentValue(), "/opt/termifier-fixture/Resources/ghostty")
    }
