...
//  gap). Full root-cause writeup: `TermifierWindowController.swift`'s
//  `keyWindowGatedActions` doc comment ("Known gap" paragraph, right next
//  to that Set's declaration) and `NSWindow+TermifierClose.swift`'s header
//  comment.
//
...
//  in that chain than `TermifierWindowController`, so `-[NSApplication
...
//  `TermifierWindowController.validateMenuItem(_:)`'s own matching
...
//  key-window check of its own -- so while a non-`TermifierWindow` panel
...
//  mirroring `TermifierWindowController`'s `keyWindowGatedActions`/
...
//  this test host -- see `TermifierWindowControllerCloseWindowTests`'s own
//  precondition assertion that a freshly constructed, never-shown
//  `TermifierWindow` is never key).
//
...
//  About panel (a non-`TermifierWindow` `NSPanel`, `canBecomeMain == false`)
//  key, `NSApp.mainWindow` stays whatever `TermifierWindow` was last main, so
...
//  `TermifierWindowControllerKeyWindowMenuValidationTests`'s own `findNext`/
...
//  the real `TermifierWindow.contentView` hierarchy so `surfaceView.window`
//  resolves back to the owning `TermifierWindowController` --
//  `isActiveTabSplit`'s own `window?.windowController as?
//  TermifierWindowController` lookup requires this. See that method's doc
...
final class SurfaceViewKeyWindowMenuValidationTests: XCTestCase {
    ...
    /// Builds a `TermifierWindowController` whose active tab's split tree
    ...
    /// .windowController as? TermifierWindowController` lookup (SurfaceView
    /// .swift, "MARK: - Menu Actions" extension) requires `surfaceView
    /// .window` to actually resolve to this fixture's own `TermifierWindow`,
    /// which only happens once the view is part of that window's view
    /// hierarchy -- unlike `window.windowController` itself, which
    /// `TermifierWindowController.init`'s `super.init(window:)` already sets
    /// reciprocally regardless of view hierarchy (see
    /// `TermifierWindowTermifierPerformCloseRoutingTests` for a fixture that
    /// relies on exactly that reciprocal link without ever touching
    /// `contentView`).
    ///
    /// `window.contentView` already holds `setupUI()`'s real
    /// `NSHostingView` (added synchronously inside `TermifierWindowController
    ...
    /// that builds a `TermifierWindowController` directly) skips `init`'s
    ...
    private func makeFixture(split: Bool) -> SurfaceFixture {
        ...
        guard let contentView = window.contentView else {
            XCTFail("Precondition: TermifierWindowController.init must leave window.contentView non-nil")
    ...
    /// own `TermifierWindowController` (e.g. the `addSubview` linkage in
    ...
    /// `TermifierWindowControllerCloseWindowTests`'s own precondition-
    ...
    func test_fixture_surfaceViewWindow_resolvesToOwningController() {
        ...
        XCTAssertTrue(
            ...
            "Precondition: surfaceView must be part of its owning TermifierWindowController's window hierarchy"
        )
    }
    ...
    func test_validateMenuItem_paste_disabledWhenNotKeyWindow() {
        ...
        XCTAssertFalse(
            result,
            "Paste must be disabled while this window is not the key window -- this is what stops Cmd+V, pressed while a non-TermifierWindow panel like About is key, from silently pasting into a background terminal's shell"
        )
    }
