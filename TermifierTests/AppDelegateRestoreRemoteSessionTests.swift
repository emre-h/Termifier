...
//  ssh, not the local termifier-session binary, no --runtime-dir/--state-dir
...
//  never hit the "no termifier-session binary resolvable" degrade branch
...
//    reattachCommand (regression guard: the TERMIFIER_SESSION_BIN env
...
//    local termifier-session env sentinel never appears at all.
//
...
final class AppDelegateRestoreRemoteSessionTests: XCTestCase {
    ...
    /// termifier-session binary and the ssh binary, so the two synthesized
    ...
    func test_restoreTabSurfaces_localSessionRef_synthesizesReattachCommand_notSSH() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(localBin)"),
                          ...
                          "is the local termifier-session binary -- regression guard for existing, unchanged behavior")
    ...
    func test_restoreTabSurfaces_remoteSessionRef_synthesizesSSHCommand_notLocalReattach() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(sshBin)"),
                          ...
                          "word is the ssh binary, never the local termifier-session binary")
            XCTAssertFalse(command.contains(localBin),
                           "A remote restore must never invoke the local termifier-session binary directly")
