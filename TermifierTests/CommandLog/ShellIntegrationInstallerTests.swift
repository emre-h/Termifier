...
//
//  Script bodies (zshenvBody/termifierZshBody/fishIntegrationBody) are
...
final class ShellIntegrationInstallerTests: XCTestCase {
    ...
    func test_install_writesThreeFilesWithExactBodiesAt0644() throws {
        ...
        let paths: [(URL, String)] = [
            ...
            (ShellIntegrationInstaller.termifierZshPath(in: installRoot), ShellIntegrationInstaller.termifierZshBody),
    ...
    func test_remove_deletesAllThreeInstalledFiles() throws {
        ...
        for path in [
            ...
            ShellIntegrationInstaller.termifierZshPath(in: installRoot),
    ...
    func test_termifierZshBody_isNonEmptyAndPassesZshSyntaxCheck() throws {
        XCTAssertFalse(ShellIntegrationInstaller.termifierZshBody.isEmpty,
                       "termifierZshBody must contain the real preexec/precmd hook registration")

        let exitCode = try syntaxCheckExitCode(interpreter: "/bin/zsh", body: ShellIntegrationInstaller.termifierZshBody)
        XCTAssertEqual(exitCode, 0, "termifierZshBody must be syntactically valid zsh (`zsh -n`)")
    }
    ...
    /// termifier.zsh is correct) rather than actually exercising the hooks.
    ...
    private func writeFixtureEndpoint(port: Int, token: String) throws {
        let appSupportDir = tempDir.appendingPathComponent("Library/Application Support/Termifier")
    ...
    func test_realZshSession_singleCommand_postsMatchingStartAndEndEvents() async throws {
        ...
        try runInteractiveZsh(command: "true; exit", extraEnv: ["TERMIFIER_SURFACE_ID": surfaceID.uuidString])
        ...
        XCTAssertTrue(startLine.contains("X-Termifier-Surface-ID: \(surfaceID.uuidString)"),
                     "the start POST must carry the TERMIFIER_SURFACE_ID as X-Termifier-Surface-ID")
    ...
    func test_realZshSession_nonZeroExitCommand_postsMatchingExitCode() async throws {
        ...
        try runInteractiveZsh(command: "(exit 3); exit", extraEnv: ["TERMIFIER_SURFACE_ID": surfaceID.uuidString])
    ...
    func test_realZshSession_emptyInteraction_postsNothing() async throws {
        ...
        try runInteractiveZsh(command: "true; exit", extraEnv: ["TERMIFIER_SURFACE_ID": sanitySurfaceID.uuidString])
        ...
        try runInteractiveZsh(command: "exit", extraEnv: ["TERMIFIER_SURFACE_ID": surfaceID.uuidString])
    ...
    func test_realZshSession_surfaceAndSessionIDBothUnset_postsNothing() async throws {
        ...
        let lines = await waitForCurlLogLines(minCount: 1, timeout: 0.5)
        XCTAssertTrue(lines.isEmpty,
                     "with both TERMIFIER_SURFACE_ID and TERMIFIER_SESSION_ID unset (a plain, non-Termifier zsh), " +
    ...
    func test_realZshSession_restoresOriginalZdotdirAndSourcesItsZshenv() async throws {
        ...
        try runInteractiveZsh(
            ...
            extraEnv: ["TERMIFIER_ZSH_ZDOTDIR": userZdotdir.path]
        )
        ...
        XCTAssertEqual(
            ...
            "Termifier's own .zshenv must restore ZDOTDIR to the user's original value from TERMIFIER_ZSH_ZDOTDIR " +
    ...
    // _termifier_preexec exactly as fish's own event system would for a real
    ...
    // printed straight to the user's terminal, with `_termifier_cmd_active`
    ...
    /// `_termifier_cmd_active`'s resulting value. The internal `sleep 1`
    ...
    private func runFishDirectPreexec(
        ...
        echo "ACTIVE=$_termifier_cmd_active"
        sleep 1
        """
    ...
    func test_realFishSession_multiLineCommand_setsTrackingStateAndPostsStartWithoutError() async throws {
        ...
        let (stdout, stderr) = try runFishDirectPreexec(
            fishPath: fishPath,
            command: "echo hello\necho world",
            extraEnv: ["TERMIFIER_SESSION_ID": UUID().uuidString]
        )
        ...
        XCTAssertTrue(
            ...
            "a real (non-whitespace) multi-line command must still set _termifier_cmd_active -- stdout: \(stdout)"
        )
    ...
    func test_realFishSession_whitespaceOnlyCommand_postsNothingAndLeavesTrackingInactive() async throws {
        ...
        XCTAssertFalse(
            stdout.contains("ACTIVE=1"),
            "a whitespace-only $argv[1] must return before setting _termifier_cmd_active -- stdout: \(stdout)"
        )
    ...
    // suspend on its own, _termifier_postexec must detect a stopped job
    ...
    /// Extracts the body of `function _termifier_postexec --on-event
    /// fish_postexec ... end` from `fishIntegrationBody`. Anchored on
    /// the exact signature line, capturing non-greedily up to the next
    /// bare `end` line -- safe because `_termifier_postexec` is the last
    ...
    private func fishPostexecFunctionBody() throws -> String {
        ...
        let pattern = #"function _termifier_postexec --on-event fish_postexec\n(.*?)\nend"#
        let regex = try NSRegularExpression(pattern: pattern, options: [.dotMatchesLineSeparators])
        let range = NSRange(body.startIndex..., in: body)
        let match = try XCTUnwrap(
            regex.firstMatch(in: body, range: range),
            "fishIntegrationBody must define `function _termifier_postexec --on-event fish_postexec ... end`"
        )
        let bodyRange = try XCTUnwrap(Range(match.range(at: 1), in: body))
        return String(body[bodyRange])
    }

    /// Extracts the body of `_termifier_precmd() { ... }` from
    /// `termifierZshBody`, the zsh counterpart to
    /// `fishPostexecFunctionBody()` above -- same non-greedy, anchored
    /// extraction, safe for the same reason (no nested `{`/`}` inside
    /// `_termifier_precmd`'s own statements).
    private func zshPrecmdFunctionBody() throws -> String {
        let body = ShellIntegrationInstaller.termifierZshBody
        let pattern = #"_termifier_precmd\(\) \{\n(.*?)\n\}"#
        let regex = try NSRegularExpression(pattern: pattern, options: [.dotMatchesLineSeparators])
        let range = NSRange(body.startIndex..., in: body)
        let match = try XCTUnwrap(
            regex.firstMatch(in: body, range: range),
            "termifierZshBody must define `_termifier_precmd() { ... }`"
        )
        let bodyRange = try XCTUnwrap(Range(match.range(at: 1), in: body))
        return String(body[bodyRange])
    }
    ...
    func test_fishIntegrationBody_postexec_consultsJobsForAStoppedState() throws {
        ...
        XCTAssertTrue(
            ...
            "_termifier_postexec must consult fish's own `jobs` builtin to detect a merely-suspended " +
            "(not exited) job -- fish has no --stopped filter, so jobs' own output must be inspected directly"
        )
        XCTAssertTrue(
            postexecBody.contains("stopped"),
            "_termifier_postexec must check jobs' own State column for its literal `stopped` value -- " +
    ...
    func test_fishIntegrationBody_postexec_postsSuspendedFlagAndNeverOverwritesStatus() throws {
        ...
        XCTAssertTrue(
            ...
            "_termifier_postexec must post a separate suspended flag when a stopped job is detected -- fish's own " +
            "$status cannot itself distinguish a suspend from the previous command's real exit code"
        )
        XCTAssertFalse(
            postexecBody.contains("146"),
            "_termifier_postexec must never substitute a stop-signal number into exit_code -- the real $status is " +
            "always posted as-is, with a suspend reported separately"
        )
    }

    func test_termifierZshBody_precmd_keepsRelayingRawStatusWithNoRedundantJobStateCheck() throws {
        let precmdBody = try zshPrecmdFunctionBody()

        XCTAssertTrue(
            precmdBody.contains("local code=$?"),
            "_termifier_precmd must keep relaying the shell's own raw $? unconditionally -- zsh already " +
        ...
        XCTAssertFalse(
            ...
            "_termifier_precmd must not grow fish's own stopped-job detection -- that would duplicate " +
    ...
    /// _termifier_cmd_active/_termifier_cmd_id are set exactly as a real
    ...
    private func runFishPostexecWithBackgroundJob(
        ...
        let stopStep = stopJob ? "kill -STOP $_termifier_test_job_pid" : "true"
        let script = """
        source \(fishIntegrationFile.path)
        emit fish_preexec "sleep 300"
        sleep 300 &
        set -g _termifier_test_job_pid $last_pid
        sleep 0.2
        \(stopStep)
        sleep 0.2
        emit fish_postexec "sleep 300"
        sleep 1
        kill -9 $_termifier_test_job_pid 2>/dev/null
        """
    ...
    func test_realFishSession_postexecWithStoppedJob_postsSuspendedFlagAndRawStatus() async throws {
        ...
        let (_, stderr) = try runFishPostexecWithBackgroundJob(
            fishPath: fishPath, stopJob: true, extraEnv: ["TERMIFIER_SESSION_ID": UUID().uuidString]
        )
        XCTAssertFalse(
    ...
    func test_realFishSession_postexecWithRunningJobOnly_postsRawExitCode() async throws {
        ...
        let (_, stderr) = try runFishPostexecWithBackgroundJob(
            fishPath: fishPath, stopJob: false, extraEnv: ["TERMIFIER_SESSION_ID": UUID().uuidString]
        )
        XCTAssertFalse(
            stderr.contains("Unknown command"), "stderr must not show a broken harness script -- stderr: \(stderr)"
        )
