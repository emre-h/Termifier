//
//  TermifierShellIntegrationEnvironmentTests.swift
//  TermifierTests
//
...
//  TermifierShellIntegrationEnvironment.swift's own header for why this
//  follows that shape rather than an injected seam).
//
//  TermifierShellIntegrationEnvironment.apply/remove are no-op stubs this
...
final class TermifierShellIntegrationEnvironmentTests: XCTestCase {

    private let zdotdirVariableName = "ZDOTDIR"
    private let originalZdotdirVariableName = "TERMIFIER_ZSH_ZDOTDIR"
    ...
    override func setUp() {
        ...
        root = URL(fileURLWithPath: "/opt/termifier-fixture/shell-integration")
    }
    ...
    func test_apply_setsZdotdirToRootZshSubdirectory() {
        ...
        TermifierShellIntegrationEnvironment.apply(rootDirectory: root)

        XCTAssertEqual(currentValue(zdotdirVariableName), expectedZdotdir,
                       "apply must point ZDOTDIR at <rootDirectory>/zsh")
    }

    func test_apply_withExistingZdotdir_savesOriginalIntoTermifierZshZdotdir() {
        ...
        XCTAssertEqual(currentValue(originalZdotdirVariableName), "/Users/alice/.config/zsh",
                       "an existing ZDOTDIR must be preserved into TERMIFIER_ZSH_ZDOTDIR before being overwritten")
        XCTAssertEqual(currentValue(zdotdirVariableName), expectedZdotdir)
    }

    func test_apply_withNoExistingZdotdir_leavesTermifierZshZdotdirUnset() {
        unsetenv(zdotdirVariableName)
        unsetenv(originalZdotdirVariableName)
        ...
        XCTAssertNil(
            ...
            "with no original ZDOTDIR, TERMIFIER_ZSH_ZDOTDIR must be ABSENT (not present-and-empty) -- " +
            "the installed .zshenv distinguishes unset (fall back to $HOME) from set-to-empty via " +
            "${TERMIFIER_ZSH_ZDOTDIR+X}, matching ghostty's own GHOSTTY_ZSH_ZDOTDIR chain"
        )
    }

    func test_apply_withZdotdirGenuinelyChangedExternally_overwritesTheStaleTermifierZshZdotdir() {
        // ZDOTDIR now holds a genuinely DIFFERENT external value (not
        // our own installed dir) -- e.g. some other tool re-pointed it
        // between two apply() calls. re-deriving TERMIFIER_ZSH_ZDOTDIR from
    ...
    func test_remove_restoresOriginalZdotdirWhenCurrentValueStillPointsAtOurDir() {
        ...
        TermifierShellIntegrationEnvironment.remove(rootDirectory: root)
    ...
    func test_remove_unconditionallyUnsetsTermifierZshZdotdirEvenWhenZdotdirNoLongerOurs() {
        // TERMIFIER_ZSH_ZDOTDIR is purely our own internal
        ...
        XCTAssertNil(currentValue(originalZdotdirVariableName),
                     "remove() must unset TERMIFIER_ZSH_ZDOTDIR unconditionally, even when ZDOTDIR no longer " +
