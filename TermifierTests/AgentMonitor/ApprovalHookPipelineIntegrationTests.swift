...
//  termifier-approval-hook pipeline: the real INSTALLED script, run as a
//  real child `/bin/sh` process, POSTing into a real `TermifierMCPServer`
...
//  TermifierMCPServerApprovalRequestTests) plus one addition that harness
...
//  string contract, `TermifierMCPServerApprovalRequestTests` drives
...
//  and TERMIFIER_SURFACE_ID/TERMIFIER_SESSION_ID both unset never even attempts
...
/// `TermifierMCPServer.swift`) short-circuits any other value (including a
...
final class ApprovalHookPipelineIntegrationTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.termifier.tests.ApprovalHookPipelineIntegrationTests"
    ...
    /// Runs the installed `termifier-approval-hook` script as a real child
    ...
    ///
    /// `surfaceID` sets `TERMIFIER_SURFACE_ID` when non-nil; `nil` (used only
    /// by the missing-surface-env test) leaves it entirely unset,
    /// matching a plain (non-Termifier) terminal invocation. `kindArgument`,
    ...
    private nonisolated static func runHookScript(
        ...
        if let surfaceID {
            env["TERMIFIER_SURFACE_ID"] = surfaceID.uuidString
        }
    ...
    /// `TermifierMCPServerApprovalRequestTests.permissionBehavior(fromResponseBody:)`.
    ...
    func test_missingSurfaceEnv_printsNothing_neverContactsServer() async throws {
        ...
        XCTAssertEqual(stdout, "")
        XCTAssertTrue(approvalInbox.pending.isEmpty,
                      "with neither TERMIFIER_SURFACE_ID nor TERMIFIER_SESSION_ID set, the script must exit " +
