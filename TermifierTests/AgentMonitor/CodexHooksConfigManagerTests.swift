...
//  termifier-agent-hook script. Follows the tempDir + configPath injection
...
//    TOML tables injected inside the comment-delimited Termifier block survive
//    remove -> reinstall
...
//    strip the BEGIN line plus everything recognizable as Termifier's own
...
final class CodexHooksConfigManagerTests: XCTestCase {
    ...
    private static let beginLine = "# BEGIN TERMIFIER AGENT HOOKS (managed by Termifier, do not edit)"
    private static let endLine = "# END TERMIFIER AGENT HOOKS"
    ...
    func test_installHooks_newFile_writesAllNineEventPairsWithCommandEntry() throws {
        ...
        XCTAssertEqual(occurrences(of: expectedApprovalCommandLine, in: content), 1,
                       "PermissionRequest's approval entry must POST through termifier-approval-hook with the codex argv")
    ...
    func test_removeThenInstall_preservesCodexHookTrustStateInjectedInsideManagedBlock() throws {
        ...
        // Its config writer can place that table between Termifier's hook entries,
        // which means comment markers alone do not prove every enclosed line
        // belongs to Termifier.
        ...
        XCTAssertTrue(disabled.contains("[hooks.state]"),
                      "Disabling Termifier IPC must not delete Codex-owned hook state")
        ...
        if let stateRange, let beginRange {
            XCTAssertLessThan(stateRange.lowerBound, beginRange.lowerBound,
                              "Preserved Codex state must be moved outside Termifier's deletable block")
        }
    }
    ...
    func test_removeThenInstall_preservesForeignTOMLTableInjectedInsideManagedBlock() throws {
        ...
        // while the final active table happens to be inside Termifier's comment
        // markers. Comments have no ownership semantics in TOML, so Termifier
        ...
        XCTAssertTrue(disabled.contains("[shell_environment_policy.set]"),
                      "Disabling Termifier IPC must not delete an unrelated TOML table")
        ...
        if let foreignRange, let beginRange {
            XCTAssertLessThan(foreignRange.lowerBound, beginRange.lowerBound,
                              "Preserved foreign tables must move outside Termifier's deletable block")
        }
    }
    ...
    /// Termifier version would have written: a user-owned `profile` root key,
    ...
    ///
    /// This is the actual upgrade path a user who installed Termifier before
    /// this migration goes through, so it pins `removingManagedBlock` /
    /// `foreignTableLines` / `isTermifierGeneratedBlockLine` against a future
    /// change that stops recognizing the old PreToolUse-nested approval
    /// entry as Termifier-owned -- which would instead preserve it as foreign
    ...
    func test_removeHooks_removesManagedBlockOnlyAndRestoresUserContent() throws {
        ...
        for eventName in Self.expectedEvents {
            XCTAssertFalse(content.contains("[[hooks.\(eventName)]]"),
                           "\(eventName)'s Termifier entry must be removed")
        }
    }
    ...
    func test_installHooks_orphanBeginMarker_selfHealsWithoutDuplicating() throws {
        ...
        // strips a stale/partial set of Termifier's own entries — identified by
        // their `command` line referencing termifier-agent-hook's own script
        // filename — so a reinstall produces exactly one clean block.
        let staleScriptPath = tempDir + "/old-install/termifier-agent-hook"
        let existing = """
        profile = "default"
        \(Self.beginLine)
        [[hooks.SessionStart]]
    ...
    // isTermifierGeneratedBlockLine must also recognize a command line
    // referencing termifier-approval-hook (not just termifier-agent-hook), so an
    // orphan BEGIN left over from a Termifier version that already installed
    ...
    func test_installHooks_orphanBeginMarker_withStaleApprovalCommand_selfHeals() throws {
        let staleApprovalPath = tempDir + "/old-install/termifier-approval-hook"
        let existing = """
        profile = "default"
        \(Self.beginLine)
        [[hooks.PreToolUse]]
    ...
    func test_removeHooks_orphanBeginImmediatelyFollowedByUserContent_onlyStripsOwnBlockLines() throws {
        ...
        // remove the BEGIN line plus lines that look like Termifier's own
    ...
    // broke hooks installation entirely in that setup. Termifier now follows
    ...
    func test_installHooks_scriptPathContainingSingleQuote_throws() {
        let badScriptPath = tempDir + "/it's-a-path/termifier-agent-hook"
    ...
    func test_installHooks_approvalScriptPathContainingSingleQuote_throws() {
        let badApprovalScriptPath = tempDir + "/it's-a-path/termifier-approval-hook"
