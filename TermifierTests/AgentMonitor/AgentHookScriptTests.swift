...
//  termifier-agent-hook script body invariants, its 0755 installation, and the
//  0600 agent-endpoint.json port/token file it reads on every invocation.
//
//  Coverage:
//  - scriptBody guards on TERMIFIER_SURFACE_ID being unset, always exits 0,
//    posts stdin verbatim via curl -m 2 --data-binary, sends the
//    X-Termifier-Surface-ID header, and reads agent-endpoint.json
...
final class AgentHookScriptTests: XCTestCase {
    ...
    func test_scriptBody_exitsEarlyWhenSurfaceIDUnset() {
        ...
        XCTAssertTrue(body.contains("TERMIFIER_SURFACE_ID"),
                     "Script must reference TERMIFIER_SURFACE_ID")
        XCTAssertTrue(body.contains("exit 0"),
                     "Script must exit 0 when TERMIFIER_SURFACE_ID is unset (external terminals unaffected)")
    }
    ...
    func test_scriptBody_sendsSurfaceIDHeader() {
        XCTAssertTrue(AgentHookScript.scriptBody.contains("X-Termifier-Surface-ID"),
                     "Script must send the X-Termifier-Surface-ID header")
    }
    ...
    // MARK: - Phase 2: kind argv + X-Termifier-Agent-Kind header
    ...
    func test_scriptBody_sendsAgentKindHeader() {
        XCTAssertTrue(AgentHookScript.scriptBody.contains("X-Termifier-Agent-Kind"),
    ...
    func test_agentEndpointFile_write_isAtomicAndLeavesNoTempFile() throws {
        ...
        // concurrent reader (termifier-agent-hook, invoked from every active
