...
//  TermifierTests
//
//  Covers ApprovalHookScript: the `termifier-approval-hook` script
//  installed alongside `termifier-agent-hook` (see AgentHookScriptTests.swift,
//  which this mirrors). Unlike termifier-agent-hook's fire-and-forget async
...
//
//  Coverage:
//  - scriptBody guards on both TERMIFIER_SURFACE_ID and TERMIFIER_SESSION_ID being
...
final class ApprovalHookScriptTests: XCTestCase {
    ...
    func test_fileName_isTermifierApprovalHook() {
        XCTAssertEqual(ApprovalHookScript.fileName, "termifier-approval-hook")
    }
    ...
    func test_scriptBody_guardsOnMissingSurfaceAndSessionID() {
        ...
        XCTAssertTrue(body.contains("[ -z \"$TERMIFIER_SURFACE_ID\" ] && [ -z \"$TERMIFIER_SESSION_ID\" ]"),
                     "Script must guard on BOTH TERMIFIER_SURFACE_ID and TERMIFIER_SESSION_ID being unset " +
                     "(a plain, non-Termifier-launched terminal must be unaffected)")
    }
    ...
    func test_scriptBody_sendsSurfaceIDHeaderPreferringSessionID() {
        XCTAssertTrue(ApprovalHookScript.scriptBody.contains("X-Termifier-Surface-ID: ${TERMIFIER_SESSION_ID:-$TERMIFIER_SURFACE_ID}"),
                     "Script must send X-Termifier-Surface-ID, preferring TERMIFIER_SESSION_ID (survives " +
                     "ghostty surface reconnect) over TERMIFIER_SURFACE_ID")
    }

    func test_scriptBody_sendsAgentKindHeader() {
        XCTAssertTrue(ApprovalHookScript.scriptBody.contains("X-Termifier-Agent-Kind: $kind"),
                     "Script must send the X-Termifier-Agent-Kind header so the server can attribute " +
        ...
        for kindArgument in kindArguments {
            for exitCode in nonZeroExitCodes {
                ...
                let result = try runFakeCurlHookScript(
                    ...
                    extraEnv: ["TERMIFIER_SURFACE_ID": UUID().uuidString], kindArgument: kindArgument
                )
                ...
                XCTAssertEqual(result.stdout, "",
                               ...
                               "must never fabricate a decision when its own connection to Termifier failed")
            }
        }
    }
