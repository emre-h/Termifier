...
//  `<pluginsDirectory>/plugins/termifier-agent-monitor.js`, a Bun-runtime
...
//  - scriptBody invariants: TERMIFIER_SURFACE_ID env guard (`return {}`),
//    agent-endpoint.json reference, X-Termifier-Agent-Kind: opencode header,
...
final class OpenCodePluginManagerTests: XCTestCase {
    ...
    /// isInstalled() operate on `<this>/plugins/termifier-agent-monitor.js`.
    ...
    private var expectedPluginPath: String {
        configRoot + "/plugins/termifier-agent-monitor.js"
    }
    ...
    func test_scriptBody_guardsOnMissingPaneIdentityAndPrefersStableSessionID() {
        ...
        XCTAssertTrue(body.contains("TERMIFIER_SURFACE_ID"),
                     "Plugin must support an ordinary pane's TERMIFIER_SURFACE_ID")
        XCTAssertTrue(body.contains("TERMIFIER_SESSION_ID"),
                     "Plugin must support a persistent pane's stable TERMIFIER_SESSION_ID")
        XCTAssertTrue(
            body.contains("process.env.TERMIFIER_SESSION_ID || process.env.TERMIFIER_SURFACE_ID"),
            "Plugin must prefer TERMIFIER_SESSION_ID over the reconnect-sensitive surface UUID"
        )
    ...
    func test_scriptBody_sendsAgentKindHeaderAsOpenCode() {
        let body = OpenCodePluginManager.scriptBody
        guard let headerRange = body.range(of: "X-Termifier-Agent-Kind") else {
            XCTFail("scriptBody must send the X-Termifier-Agent-Kind header")
            return
        }
        ...
        XCTAssertTrue(body[headerRange.upperBound..<windowEnd].contains("opencode"),
                     "X-Termifier-Agent-Kind header value must be opencode")
    }
    ...
    func test_scriptBody_cachesEndpointFileByMtime() {
        ...
        XCTAssertTrue(body.contains("import { stat }") && body.contains("node:fs/promises"),
                     "Plugin must stat the endpoint file to detect changes (e.g. a Termifier server restart)")
    }
    ...
    // (AgentEvent.isSubagentEvent / TermifierMCPServer.routeAgentEvent). The
    ...
    func test_install_symlinkAtDestinationPath_followsAndOverwritesRealFileKeepingLinkIntact() throws {
        ...
        let destinationPath = pluginsDir + "/termifier-agent-monitor.js"
    ...
    func test_install_multiHopDanglingSymlink_writesToFinalDestinationKeepingBothLinksIntact() throws {
        ...
        let finalTarget = dotfilesDir + "/termifier-agent-monitor.js"
