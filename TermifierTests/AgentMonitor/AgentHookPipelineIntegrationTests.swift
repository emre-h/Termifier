...
//  pipeline: termifier-agent-hook (a real /bin/sh script, run as a real
//  child Process) -> a real TermifierMCPServer bound to a real loopback
//  port -> a real AgentRegistry.
//
//  Every individual layer of this pipeline (AgentEvent.decode,
//  TermifierMCPServer.routeAgentEvent, AgentRegistry.handleHookEvent) already
...
//  - TERMIFIER_SURFACE_ID unset (a plain, non-Termifier terminal) never reaches
//    the registry
...
final class AgentHookPipelineIntegrationTests: XCTestCase {
    ...
    /// its stdin. `surfaceID` is set as `TERMIFIER_SURFACE_ID` when non-nil;
    /// when nil the variable is left entirely unset, matching a plain
    /// (non-Termifier) terminal invocation. `kindArgument`, when non-nil, is
    ...
    // MARK: - TERMIFIER_SURFACE_ID unset (plain terminal)
    ...
    func test_realHookScript_surfaceIDUnset_leavesRegistryUntouched() async throws {
        ...
        // TERMIFIER_SURFACE_ID is set, proving the assertion below is caused
        ...
        XCTAssertNotNil(sanityEntry, "Precondition: the pipeline must register an entry when TERMIFIER_SURFACE_ID is set")

        let exitCode = try runHookScript(stdinJSON: stdin, surfaceID: nil)

        XCTAssertEqual(exitCode, 0)
        XCTAssertEqual(registry.entries.count, 1,
                       "A hook invocation with no TERMIFIER_SURFACE_ID must never reach the registry " +
