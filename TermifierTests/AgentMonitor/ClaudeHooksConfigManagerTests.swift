...
//  ~/.claude/settings.json for the termifier-agent-hook script, following the
...
//  - removeHooks removes only Termifier's own entries, identified by command
...
final class ClaudeHooksConfigManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        scriptPath = tempDir + "/bin/termifier-agent-hook"
        approvalScriptPath = tempDir + "/bin/termifier-approval-hook"
    }
    ...
    // PreToolUse's Termifier-owned group carries only the async monitor
    // entry. PermissionRequest's carries both that same monitor entry
    // and a second, synchronous approval entry (termifier-approval-hook)
    // that blocks the tool call until Termifier's own /approval-request
    ...
    // group holds exactly two Termifier-owned command entries (monitor +
    ...
    func test_installHooks_preservesExistingUserHooksAndOtherTopLevelKeys() throws {
        ...
        let termifierGroup = preToolUseGroups.first { ($0["matcher"] as? String) == "*" }
        XCTAssertNotNil(termifierGroup, "Termifier's own '*' matcher group must be added")
    ...
    func test_installHooks_reinstall_isIdempotent() throws {
        ...
        for eventName in Self.expectedEvents {
            ...
            XCTAssertEqual(commands.count, expectedCount,
                           "\(eventName) must contain exactly \(expectedCount) Termifier hook entry(ies) " +
        ...
        // writes permission grants here) depends on to avoid racing Termifier's
        // own writes.
    ...
    // PermissionRequest's group carries two Termifier-owned command entries
    ...
    // Termifier's own /approval-request long-poll resolves. PermissionRequest
    ...
    func test_installHooks_permissionRequest_writesMonitorAndApprovalEntriesWithCorrectShapes() throws {
        ...
        XCTAssertEqual(commands.count, 2, "PermissionRequest must have exactly two Termifier command entries")
    ...
    // A config written by a pre-migration Termifier version has the
    ...
    // isOwnCommandEntry must recognize termifier-approval-hook's own filename
    ...
    func test_removeHooks_stripsApprovalOnlyEntryByCommandPath() throws {
        ...
        let dict = try readConfigDict()
        let commands = commandEntries(hookGroups(dict, event: "PreToolUse"))
        XCTAssertEqual(commands.count, 1, "Only Termifier's own approval entry should be removed")
    ...
    func test_removeHooks_removesOnlyOwnEntriesByCommandPath() throws {
        let termifierCommand = "\"\(scriptPath!)\""
        let existingConfig: [String: Any] = [
            "hooks": [
                "Stop": [
                    ["hooks": [
                        ["type": "command", "command": termifierCommand, "timeout": 5, "async": true],
        ...
        XCTAssertEqual(stopCommands.count, 1,
                       "Only Termifier's own Stop hook entry should be removed; the co-located user hook must remain")
        ...
        XCTAssertTrue(sessionStartCommands.isEmpty,
                      "Termifier's only SessionStart hook entry should leave the group empty")
    ...
    // installHooks must add both of Termifier's own entries without
    // disturbing it, and removeHooks must take only Termifier's entries back
    // out.
    func test_installThenRemove_permissionRequest_preservesUserOwnHookAndRemovesOnlyTermifierEntry() throws {
        let existingConfig: [String: Any] = [
            "hooks": [
                "PermissionRequest": [
                    ["hooks": [
        ...
        XCTAssertEqual(installedCommands.count, 3,
                       "installHooks must add both of Termifier's own PermissionRequest entries (monitor + " +
        ...
        XCTAssertEqual(remainingCommands.count, 1,
                       "removeHooks must remove only Termifier's own PermissionRequest entries")
    ...
    func test_removeHooks_dropsEmptyEventKeysAndHooksKeyWhenAllEmpty() throws {
        // The 10 events installHooks writes only ever contain Termifier's own
    ...
    // real-environment reproduction). Termifier now follows the link and
    ...
    func test_installHooks_createsBackupOfExistingFile() throws {
        ...
        XCTAssertFalse(bakContent.contains("termifier-agent-hook"),
                       "Backup must contain the pre-install content, not the newly added Termifier hooks")
    }
    ...
    func test_removeHooks_createsBackupOfPreRemovalContent() throws {
        ...
        let bakContent = try String(contentsOfFile: bakPath, encoding: .utf8)
        XCTAssertTrue(bakContent.contains("termifier-agent-hook"),
    ...
    // pre-Termifier backup with a Termifier-already-installed snapshot.
    func test_installHooks_reinstallWithNoContentChange_bakStillHoldsOriginalPreTermifierContent() throws {
        let originalConfig: [String: Any] = [
            "hooks": [
                "PreToolUse": [
                    ["matcher": "Bash", "hooks": [
        ...
        XCTAssertTrue(bakContentAfterFirstInstall.contains("user-hook"),
                     "Precondition: the first install's .bak must hold the user's pre-Termifier content")
        XCTAssertFalse(bakContentAfterFirstInstall.contains("termifier-agent-hook"),
                       "Precondition: the first install's .bak must not already contain Termifier's own hook entries")
        ...
        XCTAssertEqual(bakContentAfterSecondInstall, bakContentAfterFirstInstall,
                       ...
                       "pre-Termifier content, not the now-already-Termifier-modified content written by the first install")
    }
    ...
    func test_installHooks_unrecognizedEventShape_isLeftUntouched() throws {
        ...
        // replaced with just Termifier's own group.
        let existingConfig: [String: Any] = [
            "hooks": [
    ...
    func test_installHooks_dotfilesSymlinkedSettingsJSON_writesRealFilePreservesUserHooksAndKeepsLinkIntact() throws {
        ...
        // The real dotfiles-managed file received Termifier's 10 events...
