...
final class AgentRegistryTests: XCTestCase {
    ...
    func test_handleHookEvent_sameSessionStopAfterSessionEnd_leavesRowDone() {
        // termifier-agent-hook's command entries run "async": true, so a
    ...
    /// `termifier-agent-hook` POSTs are independent fire-and-forget `curl`
    ...
    func test_sessionMismatch_entryNotDoneButEventForwardMoving_replacesEntry() {
        ...
        // (or Termifier restarted) after the session's own SessionStart
    ...
    func test_permissionRequest_sessionMismatch_entryIdle_replacesEntryAsBlocked() {
        ...
        // Termifier restarted) can leave a stale-but-idle entry sitting on a
    ...
    func test_handleHookEvent_preToolUseWithIpcSelfPeerID_learnsBindingReflectedViaSyncInboxCounts() {
        ...
        let peerID = UUID()
        // A PreToolUse for a termifier-ipc tool carries this surface's own
    ...
    func test_handleHookEvent_postToolUseRegisterPeerResponse_learnsBinding() {
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.unreadCount, 2,
                       ...
                       "immediately, before any other termifier-ipc tool call")
    }
    ...
    // for the shells Termifier's own /command-event integration does not
    ...
    func test_handleGhosttyCommandFinished_hooksEntry_becomesDone() {
        ...
        XCTAssertEqual(entry?.state, .done,
                       ...
                       "Termifier's own shell-integration signal")
        XCTAssertEqual(entry?.source, .hooks)
    }
    ...
    // MARK: - handleGhosttyCommandFinished: deferring to Termifier's own /command-event

    // Once Termifier's own shell integration has reported a phase: end event
    // for a surface, that surface's shell is one Termifier's own zsh/fish
    ...
    // detect a suspend at all, so it must defer entirely once Termifier has
    ...
    func test_handleGhosttyCommandFinished_surfaceWithPriorTermifierStopSignalReport_doesNotSettle() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        registry.handleHookEvent(event("SessionStart", sessionID: "session-a"), surfaceID: surfaceID)
        registry.handleHookEvent(event("UserPromptSubmit", sessionID: "session-a"), surfaceID: surfaceID)
        // Termifier's own shell integration reports here through the real
        // entry point -- a stop-signal exit code, so the row itself is
        // left untouched, but the report must still be remembered.
        let termifierSettled = registry.handlePaneCommandFinished(surfaceID: surfaceID, exitCode: 146)
        XCTAssertFalse(termifierSettled, "Precondition: the stop-signal report itself must not settle the row")
        let before = registry.entries[surfaceID]
        ...
        XCTAssertFalse(ghosttySettled,
                       "A surface Termifier's own /command-event has reported on must ignore a ghostty-sourced " +
    ...
    func test_handleGhosttyCommandFinished_surfaceWithPriorTermifierFishSuspendReport_doesNotSettle() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        registry.handleHookEvent(event("SessionStart", sessionID: "session-a"), surfaceID: surfaceID)
        registry.handleHookEvent(event("UserPromptSubmit", sessionID: "session-a"), surfaceID: surfaceID)
        ...
        let termifierSettled = registry.handlePaneCommandFinished(surfaceID: surfaceID, exitCode: 0, suspended: true)
        XCTAssertFalse(termifierSettled, "Precondition: fish's suspended flag must not settle the row either")
        let before = registry.entries[surfaceID]
        ...
        XCTAssertFalse(ghosttySettled,
                       ...
                       "way to detect fish's suspend at all, so it must defer to Termifier's own integration once " +
                       "that integration has reported on this surface")
        XCTAssertEqual(registry.entries[surfaceID], before)
    }

    func test_handleGhosttyCommandFinished_surfaceRevivedAfterTermifierSettle_stillIgnoresGhosttySettle() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        registry.handleHookEvent(event("SessionStart", sessionID: "session-a"), surfaceID: surfaceID)
        registry.handleHookEvent(event("UserPromptSubmit", sessionID: "session-a"), surfaceID: surfaceID)
        ...
        XCTAssertFalse(ghosttySettled,
                       "Termifier reported on this surface once, before the revive -- the memory of that report " +
    ...
    func test_handleGhosttyCommandFinished_afterHandleSurfaceDestroyed_settlesAgainForANewRowOnTheSameID() {
        ...
        XCTAssertTrue(settled,
                      "handleSurfaceDestroyed must drop the memory of Termifier's prior report -- a new " +
    ...
    // The deferral above is only safe while Termifier's own shell
    ...
    // the row at all: Termifier never reports the end, and ghostty's own
    // signal already deferred to the report that never came. The row
    // sits at whatever it was, red included, until the pane is
    // destroyed.
    //
    // ghostty's signal and Termifier's phase: end describe the same command
    // completing and so arrive within milliseconds of each other, which
    // makes a deferral Termifier never answered evidence that Termifier's report
    ...
    func test_sweepStaleEntries_unansweredGhosttyDeferral_settlesTheRowToDone() {
        ...
        let surfaceID = UUID()
        let started = Date()
        // A long-running foreground command: Termifier's integration reported
        ...
        registry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandStart, now: started)
        ...
        XCTAssertFalse(ghosttySettled,
                       "Precondition: ghostty still defers to Termifier's own integration for this surface")
        XCTAssertEqual(registry.entries[surfaceID]?.state, .working,
        ...
        // Termifier's matching phase: end never arrives.
        registry.sweepStaleEntries(now: finished.addingTimeInterval(90))

        XCTAssertEqual(registry.entries[surfaceID]?.state, .done,
                       "A ghostty deferral Termifier never answered means Termifier's own end report was lost, so " +
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.lastEventAt, started,
                       ...
                       "reported, and this settle is Termifier's own inference, not a report")
    }
    ...
    // finished, so `AgentStateResolver.resolveTermifierShellIntegrationReported`
    ...
    func test_sweepStaleEntries_laterPhaseStart_mustNotEraseUnresolvedGhosttyDeferral() {
        ...
        // Termifier's own shell integration has reported on this surface
        // well before the ghostty signal below, so ghostty defers to it.
        registry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandStart, now: base.addingTimeInterval(5))
        ...
        // TermifierMCPServer.routeCommandEvent calls exactly this method,
        // unconditionally, for both a start and an end -- must not erase
        // the PREVIOUS command's unresolved deferral.
        registry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandStart, now: base.addingTimeInterval(26))
        XCTAssertEqual(registry.entries[surfaceID]?.state, .blocked,
                       "Precondition: the new phase: start does not itself touch the row's state")

        // Termifier's phase: end for the FIRST command never arrives (the
    ...
    // `AgentEvidence.lastTermifierCommandEndAt` is stamped only by a
    ...
    // own grace-window check (`termifierReportGraceWindow`, 2s) as "Termifier
    ...
    func test_sweepStaleEntries_shortLivedCommand_ghosttySignalWithinStartGraceWindow_stillSettlesViaDeferral() {
        ...
        // signal lands within termifierReportGraceWindow (2s) of its OWN
        ...
        // Termifier's own phase: end for this command never arrives. Sweeping
    ...
    // The ordering trap. On a fish Ctrl-Z, Termifier's own suspend report and
    // ghostty's pane-exit signal race, and Termifier's can land FIRST. A
    // deferral recorded after Termifier already answered would then have
    // nothing left to clear it, and the sweep would retire a job that is
    // merely stopped. The grace window on how recently Termifier reported is
    // what makes the dangerous ordering safe.

    func test_sweepStaleEntries_termifierSuspendReportBeforeGhosttySignal_neverSettlesTheSuspendedRow() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        let base = Date()
        registry.handleHookEvent(event("SessionStart", sessionID: "session-a"), surfaceID: surfaceID, now: base)
        ...
        let termifierSettled = registry.handlePaneCommandFinished(
            surfaceID: surfaceID, exitCode: 0, suspended: true, now: base
        )
        XCTAssertFalse(termifierSettled, "Precondition: fish's suspended flag must not settle the row")
        let before = registry.entries[surfaceID]
        XCTAssertEqual(before?.state, .working, "Precondition: still working after the suspend report")
        ...
        XCTAssertNotEqual(registry.entries[surfaceID]?.state, .done,
                          "Termifier answered this command already, milliseconds before ghostty's signal for it " +
    ...
    // event and Termifier's own report are two independent POSTs describing
    // one moment, and the hook one says nothing about whether Termifier's
    ...
    func test_sweepStaleEntries_hookEventBetweenTermifierReportAndGhosttySignal_neverSettlesTheRow() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        let base = Date()
        registry.handleHookEvent(event("SessionStart", sessionID: "session-a"), surfaceID: surfaceID, now: base)
        // Termifier's own integration reports for the pane, and the agent's
        // own hook event lands milliseconds behind it.
        registry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandEnd, now: base)
        registry.handleHookEvent(
        ...
        XCTAssertFalse(ghosttySettled, "Precondition: ghostty defers for a surface Termifier has reported on")
        XCTAssertEqual(registry.entries[surfaceID], before, "Precondition: the deferred signal changes nothing")

        registry.sweepStaleEntries(now: base.addingTimeInterval(90))

        XCTAssertNotEqual(registry.entries[surfaceID]?.state, .done,
                          "Termifier spoke moments before ghostty's signal, so that signal is answered already " +
                          "and must never be recorded as a deferral -- a hook event arriving in between " +
                          "says nothing about when Termifier last reported and must not wipe it")
        XCTAssertEqual(registry.entries[surfaceID], before,
                       "The live row must come through the sweep exactly as it went in")
    }

    func test_sweepStaleEntries_termifierAnsweredTheGhosttyDeferral_doesNotReSettleOrChurnTheRow() {
        let registry = AgentRegistry()
        let surfaceID = UUID()
        let started = Date()
        let finished = started.addingTimeInterval(5 * 60)
        ...
        // Termifier's own phase: end lands right behind it and is the one
        // that settles the row, exactly as it does today.
        let termifierSettled = registry.handlePaneCommandFinished(
            surfaceID: surfaceID, exitCode: 0, now: finished.addingTimeInterval(0.05)
        )
        XCTAssertTrue(termifierSettled, "Precondition: Termifier's own end report settles the row")
        let before = registry.entries[surfaceID]
        ...
        XCTAssertEqual(registry.entries[surfaceID], before,
                       "Termifier answered the deferral, so the sweep has nothing left to act on -- it must not " +
    ...
    func test_sweepStaleEntries_settlingAnUnansweredDeferral_leavesPendingApprovalsAlone() {
        // Only a Termifier-sourced settle over /command-event expires a
    ...
    // Termifier trusts) -- must not be wholesale-replaced by a stray hook
    ...
    // MARK: - reset(): termifierShellIntegrationReportedSurfaces survives wholesale clearing

    func test_reset_doesNotClearTermifierShellIntegrationReportedSurfaces() {
        // termifierShellIntegrationReportedSurfaces's own doc comment states it
        // is deliberately never cleared wholesale, not even by reset(): a
        // pane whose shell already sourced Termifier's own zsh/fish integration
        ...
        // settling rows for panes where Termifier's own integration is live and
        // CAN tell a suspend apart from a real exit -- a user hitting
        // Ctrl-Z would see their agent's row retire.
        let registry = AgentRegistry()
        let surfaceID = UUID()
        registry.recordTermifierShellIntegrationReported(surfaceID: surfaceID, phase: .commandStart)
        ...
        XCTAssertFalse(settled,
                       "reset() must not clear termifierShellIntegrationReportedSurfaces -- ghostty's fallback must " +
                       "keep deferring to Termifier's own shell integration's memory for this surface")
        XCTAssertEqual(registry.entries[surfaceID]?.state, .idle,
    ...
    func test_reset_clearsAgentEvidence() {
        // The deliberate opposite of termifierShellIntegrationReportedSurfaces's
