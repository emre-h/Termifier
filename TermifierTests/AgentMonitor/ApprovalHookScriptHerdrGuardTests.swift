...
//  (termifier-approval-hook) carries -- the approval-hook counterpart of
//  AgentHookScriptHerdrGuardTests (termifier-agent-hook's identical guard).
//  Running a herdr server from inside a Termifier pane's shell means every
//  shell herdr itself spawns can inherit that pane's TERMIFIER_SURFACE_ID;
//  without a guard, a PermissionRequest hook running INSIDE a
//  herdr-managed pane would POST to /approval-request under that stale,
//  misattributed surface, routing a real permission decision to the
//  wrong pane's approval inbox. The fix: termifier-approval-hook exits 0
//  WITHOUT posting whenever HERDR_PANE_ID is set and non-empty,
//  mirroring both the script's own existing
//  TERMIFIER_SURFACE_ID/TERMIFIER_SESSION_ID early-exit guard and
//  termifier-agent-hook's identical HERDR_PANE_ID guard.
//
...
//  own real-TermifierMCPServer-plus-AgentRegistry pipeline style (mirrors
...
final class ApprovalHookScriptHerdrGuardTests: XCTestCase {
    ...
    override func setUp() {
        ...
        fixture = try! makeFakeCurlScriptFixture(
            port: 41_831, token: "approval-herdr-guard-test-token",
    ...
    /// Runs the installed `termifier-approval-hook` script via the shared
    ...
    {"session_id":"herdr-guard-session","tool_name":"Bash","hook_event_name":"PermissionRequest"}
    """
    ...
    func test_surfaceIDSet_herdrPaneIDSet_exitsZeroWithoutInvokingCurl() throws {
        ...
        XCTAssertEqual(curlInvocationCount(), 0,
                       ...
                       "can inherit a stale TERMIFIER_SURFACE_ID from the pane herdr's own server was started " +
