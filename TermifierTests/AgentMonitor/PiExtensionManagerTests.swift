...
//  TermifierTests
//
//  Installs and removes `<agent root>/extensions/termifier.ts`, the
...
//  a silent drift in any of them produces a pi pane Termifier either cannot
//  see or cannot gate.
//
//  Coverage:
...
//    ApprovalHookTiming, the single `termifier` dispatcher tool, the block
...
final class PiExtensionManagerTests: XCTestCase {
    ...
    /// remove()/isInstalled() operate on `<this>/extensions/termifier.ts`,
    ...
    private var expectedExtensionPath: String {
        agentRoot + "/extensions/termifier.ts"
    }
    ...
    func test_scriptBody_sendsAgentKindHeaderAsPi() {
        assertScriptContains("\"X-Termifier-Agent-Kind\":\"pi\"",
        ...
        assertScriptContains("\"X-Termifier-Surface-ID\":",
    ...
    func test_scriptBody_guardsOnPaneIdentityAndHerdrBeforeRegisteringAnything() {
        assertScriptContains("process.env.TERMIFIER_SESSION_ID||process.env.TERMIFIER_SURFACE_ID",
                             "A persistent pane's TERMIFIER_SESSION_ID stays stable across surface changes, " +
        ...
        assertScriptContains("process.env.HERDR_PANE_ID",
                             "A pi running inside a herdr pane is mirrored into Termifier by herdr itself, " +
        ...
        XCTAssertTrue(guardReturn.upperBound <= firstRegistration,
                      ...
                      "first and checking later would leave a pi started outside Termifier carrying a live " +
    ...
    func test_scriptBody_postsCanonicalSnakeCaseStatePings() {
        assertScriptContains("/agent-event",
                             "Row state reaches Termifier through the agent event endpoint")
    ...
    func test_scriptBody_readsEndpointFileThroughNodeAndCachesItByMtime() {
        assertScriptContains("agent-endpoint.json",
                             "Port and token are read from the endpoint file, so a Termifier restart or a " +
    ...
    func test_scriptBody_approvalRequestUsesThePermissionRequestEnvelope() {
        assertScriptContains("hook_event_name:\"\(ApprovalHookEvent.name)\"",
                             "The approval request must name the event Termifier's gate answers for. A " +
    ...
    func test_scriptBody_blocksOnlyOnAnExplicitDeny() {
        ...
        XCTAssertEqual(occurrences(of: "block:true"), 1,
                       ...
                       "unreachable Termifier into a pi that cannot run any tool at all")
    ...
    func test_scriptBody_registersASingleTermifierDispatcherTool() {
        XCTAssertEqual(occurrences(of: "pi.registerTool("), 1,
                       "One dispatcher, not one tool per Termifier IPC tool. Registering all of them " +
                       "individually puts every name and description into pi's system prompt on every " +
                       "turn")
        assertScriptContains("name:\"termifier\"",
    ...
    func test_scriptBody_bridgesTermifierIPCThroughTheMCPEndpoint() {
        assertScriptContains("/mcp",
                             "The dispatcher proxies to Termifier's MCP endpoint")
    ...
    func test_install_symlinkAtDestinationPath_followsAndOverwritesRealFileKeepingLinkIntact() throws {
        ...
        let destinationPath = extensionsDir + "/termifier.ts"
