...
//  installed hook script (termifier-agent-hook / termifier-approval-hook) as a
...
//  `Library/Application Support/Termifier` layout matches exactly what the
...
/// `Library/Application Support/Termifier` and `install`'s script installed
/// into its `bin/` -- the exact HOME-relative layout every hook script
/// itself reads at invocation time (`$HOME/Library/Application
/// Support/Termifier/agent-endpoint.json`). `fakeBinDir` is a second,
...
func makeFakeCurlScriptFixture(
    ...
    let appSupportDir = home + "/Library/Application Support/Termifier"
...
/// (e.g. `TERMIFIER_SURFACE_ID` so the script's own early guard doesn't
