//
//  TermifierWindowControllerRemoteReconnectEstablishTests.swift
//  TermifierTests
//
...
//  SessionDaemonClient.shared -- the LOCAL termifier-session daemon)
...
//  driveReconnect shape (TermifierWindowControllerReconnectGraceOverrides
...
//  the TERMIFIER_SESSION_BIN env sentinel, pumpRunLoop), plus
...
@MainActor
final class TermifierWindowControllerRemoteReconnectEstablishTests: XCTestCase {
    ...
    func test_performReconnect_remoteSession_establishesFromSurfaceSurvivalAlone_withoutDaemonProbe() throws {
        ...
        setenv("TERMIFIER_SESSION_BIN", localBin, 1)
        defer {
            if let originalBinPath { setenv("TERMIFIER_SESSION_BIN", originalBinPath, 1) } else { unsetenv("TERMIFIER_SESSION_BIN") }
        }
