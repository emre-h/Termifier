...
//  termifier-session daemon/CLI resolves its on-disk state root
//  ($HOME/.termifier: run/sessiond.sock, state/) from the literal HOME env
//  var at process start (see termifier-session/crates/daemon/src/session.rs),
...
//  Termifier.app itself has, and neither consults FileManager
//  .homeDirectoryForCurrentUser (which ignores HOME entirely -- proven
//  live: an app launched with HOME=/tmp/x still wrote ~/.termifier/sessions.json
...
//  `SessionDaemonClient` agree on the termifier-session binary path) so a
...
//    present-but-empty-string handling for TERMIFIER_SESSION_BIN)
//
...
final class SessionRootResolverTests: XCTestCase {
    ...
    func test_resolve_returnsInjectedHOMEValue_whenPresentAndNonEmpty() {
        let resolver = SessionRootResolver(environment: ["HOME": "/opt/termifier-fixture/custom-home"])

        XCTAssertEqual(resolver.resolve(), "/opt/termifier-fixture/custom-home",
                       "With HOME present in the injected environment, resolve() must return exactly that " +
                       "value -- this is the literal env var termifier-session's own daemon/CLI resolves its " +
    ...
    func test_resolve_fallsBackToNSHomeDirectory_whenHOMEPresentButEmpty() {
        ...
        XCTAssertEqual(resolver.resolve(), NSHomeDirectory(),
                       ...
                       "handling for TERMIFIER_SESSION_BIN")
    }
}
