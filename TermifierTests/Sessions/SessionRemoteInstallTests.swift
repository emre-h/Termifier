...
//  turning (host, bundled resource paths) into the termifier-session CLI's
//  own `remote-install` argv (termifier-session/crates/cli/src/commands/remote_install.rs,
...
//    - Resources/session-remote/<x86_64|aarch64>/termifier-session -- the
//      cross-compiled Linux musl payloads (cycle 4)
//    - Resources/bin/termifier-session -- the LOCAL termifier-session binary,
...
final class RemoteInstallArgvBuilderTests: XCTestCase {
    ...
    func test_buildArgv_includesEveryFlagWhenEveryPathIsGiven() {
        let argv = RemoteInstallArgvBuilder.buildArgv(
            host: "devbox",
            payloadX86_64Path: "/bundle/session-remote/x86_64/termifier-session",
            payloadAarch64Path: "/bundle/session-remote/aarch64/termifier-session",
            hostBinaryPath: "/bundle/bin/termifier-session",
            terminfoPath: "/bundle/terminfo/78/xterm-ghostty"
        )

        XCTAssertEqual(argv, [
            "remote-install", "devbox",
            "--payload-x86-64", "/bundle/session-remote/x86_64/termifier-session",
            "--payload-aarch64", "/bundle/session-remote/aarch64/termifier-session",
            "--host-binary", "/bundle/bin/termifier-session",
...
final class SessionDaemonClientInstallRemoteTests: XCTestCase {
    ...
    func test_installRemote_runsLocalBinaryWithBuiltArgv() async {
        ...
        let client = SessionDaemonClient(
            resolver: FixedBinaryResolver(path: "/opt/termifier-fixture/bin/termifier-session"),
            commandRunner: runner,
            payloadResolver: FakePayloadResolver(
                x86_64Path: "/bundle/session-remote/x86_64/termifier-session",
                aarch64Path: "/bundle/session-remote/aarch64/termifier-session",
        ...
        XCTAssertEqual(recorded[0].executable, "/opt/termifier-fixture/bin/termifier-session",
                       "installRemote must invoke the LOCAL bundled termifier-session binary as the executable")
        XCTAssertEqual(recorded[0].arguments, [
            "remote-install", "devbox.example.com",
            "--payload-x86-64", "/bundle/session-remote/x86_64/termifier-session",
            "--payload-aarch64", "/bundle/session-remote/aarch64/termifier-session",
            "--host-binary", "/opt/termifier-fixture/bin/termifier-session",
            "--terminfo", "/bundle/terminfo/78/xterm-ghostty",
    ...
    func test_installRemote_noLocalBinaryResolvable_returnsNilWithoutRunningAnything() async {
        ...
        XCTAssertNil(result, "With no local termifier-session binary resolvable, installRemote must return " +
