...
//  TermifierWindowController.killSessionIfPersistent never
...
//  shells `ssh -- <host> "$HOME/.termifier/bin/termifier-session kill
...
//    argv exactly ["--", host, "$HOME/.termifier/bin/termifier-session kill
//    '<sessionID>'"] -- no -t flag
...
final class SessionDaemonClientKillRemoteTests: XCTestCase {
    ...
    func test_killRemote_shellsSSHWithHostAndRemoteKillCommand_noPTYFlag() async {
        ...
        let client = SessionDaemonClient(
            ...
            sshResolver: FakeSSHResolver(path: "/opt/termifier-fixture/bin/ssh")
        )
        ...
        XCTAssertEqual(recorded[0].executable, "/opt/termifier-fixture/bin/ssh",
                       "killRemote must invoke the resolved ssh binary as the executable, never the local " +
                       "termifier-session binary")
        XCTAssertEqual(recorded[0].arguments, [
            "--", "devbox.example.com",
            "$HOME/.termifier/bin/termifier-session kill '01ARZ3NDEKTSV4RRFFQ69G5FAV'",
    ...
    func test_killRemote_sessionIDContainingSingleQuote_isEscaped() async {
        ...
        XCTAssertEqual(recorded.first?.arguments.last,
                       "$HOME/.termifier/bin/termifier-session kill '01ARZ3'\\''; touch pwned; '\\'''",
    ...
    func test_killRemote_noLocalBinaryResolvable_stillInvokesSSH() async {
        ...
        XCTAssertEqual(recorded.count, 1,
                       ...
                       "killRemote only ever needs the ssh binary -- the local termifier-session binary's " +
