//
//  TermifierWindowControllerRemoteReconnectCommandTests.swift
//  TermifierTests
//
//  Remote sessions. THE DEFECT:
//  TermifierWindowController.performReconnect(oldSurfaceID:sessionID:)
//  (TermifierWindowController.swift near :2378) always synthesizes its
...
//  _performReconnectSurfaceCreationHookForTesting, the TERMIFIER_SESSION_BIN/
//  TERMIFIER_SSH_BIN env sentinel convention from
...
@MainActor
final class TermifierWindowControllerRemoteReconnectCommandTests: XCTestCase {
    ...
    private func withBinarySentinels(
        ...
        let originalLocal = ProcessInfo.processInfo.environment["TERMIFIER_SESSION_BIN"]
        let originalSSH = ProcessInfo.processInfo.environment["TERMIFIER_SSH_BIN"]
        setenv("TERMIFIER_SESSION_BIN", local, 1)
        setenv("TERMIFIER_SSH_BIN", ssh, 1)
        defer {
            if let originalLocal { setenv("TERMIFIER_SESSION_BIN", originalLocal, 1) } else { unsetenv("TERMIFIER_SESSION_BIN") }
            if let originalSSH { setenv("TERMIFIER_SSH_BIN", originalSSH, 1) } else { unsetenv("TERMIFIER_SSH_BIN") }
        }
        try body()
    }

    func test_performReconnect_localSession_synthesizesReattachCommand_notSSH() throws {
        let localBin = "/tmp/termifier-session-sentinel-\(UUID().uuidString)"
    ...
    func test_performReconnect_remoteSession_synthesizesSSHCommand_notLocalReattach() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(sshBin)"),
                          ...
                          "whose first word is the ssh binary, never the local termifier-session binary")
            XCTAssertFalse(command.contains(localBin), "A remote reconnect must never invoke the local termifier-session binary directly")
