...
//  binary as a single-quoted '~/.termifier/bin/termifier-session'. That is WRONG:
...
//  instead leaves the literal text $HOME/.termifier/bin/termifier-session
...
//  remote termifier-session binary without needing to be shell-safe in any
//  other sense.
//
...
//  - the remote command word references $HOME/.termifier/bin/termifier-session
...
//    fixture directory containing a stub termifier-session, actually
...
final class SessionCommandSynthesizerRemoteAttachTests: XCTestCase {
    ...
    /// `<fixtureHome>/.termifier/bin/termifier-session` works directly) that
    ...
    /// containing a stub termifier-session (LAYER 2), and returns what that
    ...
    private func roundTripRemoteAttachCommand(
        ...
        let fixtureHome = uniqueTempPath("termifier-fixture-remote-home")
        let remoteOutputPath = uniqueTempPath("termifier-session-remote-argv-capture")
        defer {
        ...
        let remoteTermifierSessionPath = fixtureHome + "/.termifier/bin/termifier-session"
        try makeSelfPathAndArgvDumperScript(at: remoteTermifierSessionPath, outputPath: remoteOutputPath)
        ...
        guard let remoteSelfPath = remoteLines.first else {
            throw RoundTripCaptureFailure(description: "remote termifier-session stub was never invoked " +
    ...
    func test_remoteAttachCommand_remoteCommandReferencesHomeTermifierBinTermifierSessionUnquoted() {
        let command = SessionCommandSynthesizer.remoteAttachCommand(
            host: "build-box.example.com", sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", cwd: "/home/dev/repo"
        )
        XCTAssertTrue(command.contains("$HOME/.termifier/bin/termifier-session"),
                       "The remote binary must be referenced as the literal text $HOME (a REMOTE-shell " +
                       "parameter expansion), never as a single-quoted '~/.termifier/bin/termifier-session' -- " +
        ...
        XCTAssertFalse(command.contains("'~/.termifier/bin/termifier-session'"),
    ...
    func test_remoteAttachCommand_execution_fullRoundTrip_remoteShellExpandsHomeAndArgvSurvivesBothLayers() throws {
        ...
        XCTAssertTrue(result.remoteSelfPath.hasSuffix("/.termifier/bin/termifier-session"),
    ...
    func test_remoteAttachCommand_execution_initialGridSizeSurvivesBothShellLayers() throws {
        ...
        XCTAssertEqual(
            ...
            "the host layout's initial grid must reach remote termifier-session as intact argv words"
        )
    }
    ...
    func test_remoteAttachCommand_execution_sessionIDWithSemicolonCannotInjectASecondRemoteShellStatement() throws {
        let marker = uniqueTempPath("termifier-remote-injection-marker")
        ...
        XCTAssertEqual(result.remoteArgv,
                       ...
                       "sessionID containing ; must survive as ONE intact argument to termifier-session on " +
    ...
    func test_remoteAttachCommand_execution_cwdWithDollarParenAndBackticksSurviveBothLayersUnexpanded() throws {
        let markerA = uniqueTempPath("termifier-remote-injection-marker-a")
        let markerB = uniqueTempPath("termifier-remote-injection-marker-b")
        defer {
