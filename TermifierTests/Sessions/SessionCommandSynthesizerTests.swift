...
//  termifier-session binary, rather than string-matching
...
final class SessionCommandSynthesizerTests: XCTestCase {
    ...
    /// the real (nonexistent in tests) termifier-session binary so the
    ...
    func test_attachCommand_basicForm_includesExecCreateIdCwdInOrder() throws {
        ...
        // now (termifier-session/crates/cli/src/cli.rs:15-20), prepended
        ...
        XCTAssertEqual(argv, ["--runtime-dir", expectedRoot + "/.termifier/run",
    ...
    func test_attachCommand_binaryPathWithSpace_isInvokedCorrectly() throws {
        let dirWithSpace = FileManager.default.temporaryDirectory
            .appendingPathComponent("termifier dumper dir \(UUID().uuidString)").path
        try FileManager.default.createDirectory(atPath: dirWithSpace, withIntermediateDirectories: true)
        let binaryPath = dirWithSpace + "/termifier-session"
        let outputPath = uniqueTempPath("termifier-argv-capture")
        defer {
    ...
    private func makeMarkerTouchingScript(markerPath: String) throws -> String {
        let scriptPath = uniqueTempPath("termifier-injection-script")
    ...
    private func assertCannotInject(
        ...
        let marker = uniqueTempPath("termifier-injection-marker")
