...
//  (termifier-session/crates/cli/src/cli.rs:15-16, `global = true`) is what
...
//  `--runtime-dir <root>/.termifier/run` argument prepended before each
...
//    ["--runtime-dir", "<rootResolver's root>/.termifier/run"] before their
...
//    equals <real $HOME>/.termifier/run (regression guard against the Rust
//    CLI's own default)
//
...
/// with, standing in for the real termifier-session subprocess call so the
...
final class SessionDaemonClientRuntimeDirArgsTests: XCTestCase {
    ...
    func test_allFourOperations_prependRuntimeDirFlagBeforeSubcommand_environmentStaysNil() async {
        ...
        let runtimeDirFlag = ["--runtime-dir", "/opt/termifier-fixture/custom-home/.termifier/run"]
        ...
        XCTAssertEqual(recorded[0].arguments, runtimeDirFlag + ["ls", "--all", "--json"],
                       "listAll must prepend --runtime-dir <root>/.termifier/run before its ls --all --json " +
                       "subcommand arguments")
        XCTAssertEqual(recorded[1].arguments, runtimeDirFlag + ["ls", "--all", "--json"],
                       "sessionState must prepend --runtime-dir <root>/.termifier/run before the same ls --all " +
                       "--json subcommand it shares with listAll")
        XCTAssertEqual(recorded[2].arguments, runtimeDirFlag + ["kill", "01ARZ3NDEKTSV4RRFFQ69G5FAV"],
                       "kill must prepend --runtime-dir <root>/.termifier/run before its kill <id> subcommand " +
                       "arguments")
        XCTAssertEqual(recorded[3].arguments, runtimeDirFlag + ["meta", "set", "01ARZ3NDEKTSV4RRFFQ69G5FAV", "k=v"],
                       "setMeta must prepend --runtime-dir <root>/.termifier/run before its meta set <id> " +
    ...
    func test_defaultRootResolver_composesRealHOMETermifierRunDir() async {
        let runner = RecordingCommandRunner()
        ...
        XCTAssertEqual(recorded.first?.arguments,
                       ["--runtime-dir", expectedRoot + "/.termifier/run", "ls", "--all", "--json"],
                       "With no rootResolver override (real production use), the composed --runtime-dir " +
                       "value must equal <real $HOME>/.termifier/run -- identical to the Rust CLI's own " +
                       "default_home_subdir fallback (termifier-session/crates/cli/src/commands/mod.rs:74), so " +
