...
//  the exec'd termifier-session binary. That stamp existed only to make the
...
//  `--runtime-dir`/`--state-dir` flags (termifier-session/crates/cli/src/cli.rs:15-20,
//  `global = true`, so every subcommand accepts them, not just `daemon`)
//  that `attach` both accepts and, when it auto-spawns the daemon,
//  forwards verbatim (termifier-session/crates/cli/src/commands/attach.rs's
...
//  first word is simply the absolute termifier-session binary path again,
...
//  `.shell`-variant command Termifier always produces); its own single
...
//  substituting a small stub script (in place of the real termifier-session
...
//  `--runtime-dir`/`--state-dir` values equal `<real $HOME>/.termifier/{run,state}`
//  -- identical to the Rust CLI's own fallback
//  (termifier-session/crates/cli/src/commands/mod.rs:74's
...
//  - both prepend `--runtime-dir <root>/.termifier/run --state-dir
//    <root>/.termifier/state` immediately before the `attach` subcommand,
...
//    `<real $HOME>/.termifier/{run,state}` (regression guard against the
...
final class SessionCommandSynthesizerRuntimeStateDirFlagsTests: XCTestCase {
    ...
    func test_reattachCommand_containsNoEnvWrapperOrHomeStamp() throws {
        let binaryPath = uniqueTempPath("termifier-session-dumper")
    ...
    func test_attachCommand_containsNoEnvWrapperOrHomeStamp() {
        let command = SessionCommandSynthesizer.attachCommand(
            binaryPath: "/usr/local/bin/termifier-session",
            sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV",
            cwd: "/Users/dev/repo",
            rootResolver: FakeRootResolver(root: "/opt/termifier-fixture/custom-home")
        )
    ...
    func test_reattachCommand_prependsRuntimeAndStateDirFlagsBeforeAttach_ambientHOMEIrrelevant() throws {
        let binaryPath = uniqueTempPath("termifier-session-dumper")
        let outputPath = uniqueTempPath("termifier-home-argv-capture")
        defer {
        ...
        try runShCEmulatingGhosttyWrapping(command, ambientHome: "/tmp/termifier-wrong-ambient-home")
        let lines = readCapturedLines(at: outputPath)

        XCTAssertEqual(lines.first, "HOME=/tmp/termifier-wrong-ambient-home",
                       "The exec'd termifier-session process's own $HOME must be left completely untouched -- " +
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", "/opt/termifier-fixture/custom-home/.termifier/run",
                        "--state-dir", "/opt/termifier-fixture/custom-home/.termifier/state",
                       ...
                       "before-subcommand order the Rust CLI itself uses (termifier-session/crates/cli/src/commands/attach.rs's " +
    ...
    func test_attachCommand_prependsRuntimeAndStateDirFlagsBeforeAttach_ambientHOMEIrrelevant() throws {
        ...
        let command = SessionCommandSynthesizer.attachCommand(
            ...
            sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV",
            cwd: "/Users/dev/repo",
            rootResolver: FakeRootResolver(root: "/opt/termifier-fixture/spawn-home")
        )
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", "/opt/termifier-fixture/spawn-home/.termifier/run",
                        "--state-dir", "/opt/termifier-fixture/spawn-home/.termifier/state",
                        "attach", "01ARZ3NDEKTSV4RRFFQ69G5FAV", "--create", "--cwd", "/Users/dev/repo"],
    ...
    func test_attachCommand_defaultRootResolver_composesRealHOMETermifierRunAndStateDirs() throws {
        let binaryPath = uniqueTempPath("termifier-session-dumper")
        let outputPath = uniqueTempPath("termifier-argv-capture")
        defer {
            try? FileManager.default.removeItem(atPath: binaryPath)
            try? FileManager.default.removeItem(atPath: outputPath)
        }
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", expectedRoot + "/.termifier/run",
                        "--state-dir", expectedRoot + "/.termifier/state",
                        "attach", "01ARZ3NDEKTSV4RRFFQ69G5FAV", "--create", "--cwd", "/Users/dev/repo"],
                       "With no rootResolver override (real production use), the composed --runtime-dir/" +
                       "--state-dir paths must equal <real $HOME>/.termifier/{run,state} -- identical to the " +
                       "Rust CLI's own default_home_subdir fallback (termifier-session/crates/cli/src/commands/mod.rs:74), " +
