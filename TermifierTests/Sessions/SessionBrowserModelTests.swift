...
//  `termifier-session` process, matching this codebase's direct-query test
...
/// no real `termifier-session` binary involved.
...
final class SessionBrowserModelTests: XCTestCase {
    ...
    /// `termifier-session/crates/daemon/src/ledger.rs`'s retention GC), so a
