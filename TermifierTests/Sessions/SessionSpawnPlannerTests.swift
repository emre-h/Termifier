...
//  termifier-session-backed persistent shell (.persistent), gated by
...
//  for the real termifier-session binary (via an injected
...
final class SessionSpawnPlannerTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.termifier.tests.SessionSpawnPlannerTests"
    ...
    /// stub (standing in for the real termifier-session binary), runs the
    ...
    func test_plan_enabledTabOrigin_distinctCallsProduceDistinctSessionIDs() {
        ...
        // this test's outcome depend on whether a real termifier-session
        // binary happened to be bundled in the test environment, rather
        // than on the ULID-distinctness behavior it's meant to verify.
        let resolver = FakeBinaryResolver(path: "/dummy/termifier-session")
    ...
    // caller (TermifierWindowController.createManagedSurface); landing a new
