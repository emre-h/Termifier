...
//  must both consume, so they always agree on which termifier-session
...
//    .passthrough (not a hardcoded "termifier-session" literal) and the
...
final class SessionBinaryResolverTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.termifier.tests.SessionBinaryResolverTests"
    ...
    func test_resolverResolvesAPath_plannerAndClientAgreeOnSamePath() {
        let resolver = FakeBinaryResolver(path: "/opt/termifier-fixture/bin/termifier-session")

        let client = SessionDaemonClient(resolver: resolver)
        XCTAssertEqual(client.resolvedBinaryPath, "/opt/termifier-fixture/bin/termifier-session",
        ...
        XCTAssertTrue(
            command.contains(ShellEscape.escape("/opt/termifier-fixture/bin/termifier-session")),
    ...
    func test_resolverReturnsNil_plannerFallsBackToPassthrough_clientReturnsUnreachable() async {
        ...
        XCTAssertEqual(plan, .passthrough,
                       ...
                       "not synthesize a command around a hardcoded \"termifier-session\" literal that may not exist on PATH")
