...
//  termifier-session daemon can stall the reconnect decision indefinitely,
...
//  Process (SIGTERM), cancelling a slow `termifier-session kill` mid-IPC-write
...
//  in for a termifier-session subprocess that never exits, so a passing run
