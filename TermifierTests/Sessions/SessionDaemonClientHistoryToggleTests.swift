...
//  termifier-session/crates/cli/src/commands/history.rs -- the daemon
//  message pair predates the CLI subcommand that sends it).
//
//  Coverage:
//  - setHistoryEnabled(true) / setHistoryEnabled(false) each prepend
//    ["--runtime-dir", "<root>/.termifier/run"] before ["history", "on"/"off"],
...
final class SessionDaemonClientHistoryToggleTests: XCTestCase {
    ...
    func test_setHistoryEnabledTrue_prependsRuntimeDirBeforeHistoryOn() async {
        ...
        XCTAssertEqual(
            ...
            ["--runtime-dir", "/opt/termifier-fixture/custom-home/.termifier/run", "history", "on"],
            "setHistoryEnabled(true) must prepend --runtime-dir <root>/.termifier/run before its history on " +
    ...
    func test_setHistoryEnabledFalse_prependsRuntimeDirBeforeHistoryOff() async {
        ...
        XCTAssertEqual(
            recorded.first?.arguments,
            ["--runtime-dir", "/opt/termifier-fixture/custom-home/.termifier/run", "history", "off"],
            "setHistoryEnabled(false) must prepend --runtime-dir <root>/.termifier/run before its history off " +
            "subcommand arguments"
        )
    }
}
