    ...
    fn reap(&mut self, id: &str) -> Option<i32> {
        match self {
            SessionChild::Forked(child) => match child.wait() {
                ...
                Err(e) => {
                    eprintln!("termifier-sessiond: waitpid failed for {id}: {e}");
                    Some(-1)
                }
            },
...
pub(crate) fn spawn_session(
    ...
    cmd.env("TERMIFIER_SESSION_ID", &spec.id);
    // TERMIFIER_SURFACE_ID names a pane; the daemon only knows the stale
    ...
    cmd.env_remove("TERMIFIER_SURFACE_ID");
...
fn session_thread(ctx: SessionThread) {
    ...
    if !seed_replay.is_empty() {
        if let Err(e) = terminal.feed(&seed_replay) {
            eprintln!("termifier-sessiond: seeding handoff replay failed for {id}: {e}");
        }
    }
    drop(seed_replay);
    ...
    if registered && (history_enabled || history::has_persisted(&state_dir, &id)) {
            ...
            match history::read_persisted(&state_dir, &id) {
                Ok(Some(bytes)) => {
                    if let Err(e) = terminal.feed(&bytes) {
                        eprintln!("termifier-sessiond: seeding persisted history failed for {id}: {e}");
                    }
                    if let Err(e) = history::HistoryWriter::remove_all(&state_dir, &id) {
                        eprintln!(
                            "termifier-sessiond: resetting persisted history failed for {id}: {e}"
                        );
                    }
                }
                Ok(None) => {}
                Err(e) => {
                    eprintln!("termifier-sessiond: reading persisted history failed for {id}: {e}");
                }
            }
        }
        match history::HistoryWriter::open(&state_dir, &id, history::DEFAULT_CAP_BYTES) {
            ...
            Err(e) => {
                eprintln!(
                    "termifier-sessiond: opening history for {id} failed, running without \
                     history: {e}"
                );
            }
        }
    ...
    let responder_master: Option<Arc<OwnedFd>> = match master.try_clone() {
        ...
        Err(e) => {
            eprintln!("termifier-sessiond: dup pty master for responder failed for {id}: {e}");
            None
        }
    };
    ...
    loop {
        ...
        for request in requests {
            match request {
                ...
                SessionRequest::Resize { cols, rows } => {
                    ...
                    if rc < 0 {
                        eprintln!(
                            "termifier-sessiond: TIOCSWINSZ failed for {id}: {}",
                            std::io::Error::last_os_error()
                        );
                    }
                    if let Err(e) = terminal.resize(cols.max(1), rows.max(1)) {
                        eprintln!("termifier-sessiond: vt resize failed for {id}: {e}");
                    }
                    cur_cols = cols.max(1);
                    cur_rows = rows.max(1);
                }
        ...
        match poll(&mut poll_fds, PollTimeout::NONE) {
            ...
            Err(nix::errno::Errno::EINTR) => continue,
            Err(e) => {
                eprintln!("termifier-sessiond: poll failed for {id}: {e}");
                break;
            }
        }
            ...
            match nix::unistd::read(master.as_fd(), &mut buf) {
                ...
                Ok(n) => {
                    ...
                    if let Err(e) = terminal.feed(chunk) {
                        ...
                        eprintln!("termifier-sessiond: vt feed failed for {id}: {e}");
                    }
                    ...
                    if let Some(writer) = history.as_mut() {
                        if let Err(e) = writer.append(chunk) {
                            eprintln!(
                                "termifier-sessiond: history append failed for {id}, stopping \
    ...
    if mine {
        if let Some(exit_code) = reaped {
            ...
            // files.
            if let Err(e) = history::HistoryWriter::remove_all(&state_dir, &id) {
                eprintln!("termifier-sessiond: removing history for {id} failed: {e}");
            }
            let mut state = shared.lock_state();
...
fn push_replay(terminal: &mut vt::Terminal, queue: &Arc<OutQueue>, id: &str) {
    match terminal.render_replay() {
        ...
        Err(e) => {
            eprintln!("termifier-sessiond: render_replay failed for {id}: {e}");
