//! termifier-session daemon: a per-user background process that owns PTY
//! sessions and lets any number of `termifier-session attach` clients
...
//!   `TIOCSCTTY` + `execvp`), with `TERMIFIER_SESSION_ID=<id>` injected into
