...
//!
//! **Contract.** `termifier-session upgrade` spawns a new daemon process,
...
fn finalize_exited(shared: &Arc<Shared>, entry: SessionEntry) {
    ...
    if let Err(e) = history::HistoryWriter::remove_all(&shared.state_dir, &entry.id) {
        eprintln!(
            "termifier-sessiond: removing history for {} failed: {e}",
            entry.id
        );
    }
    let mut state = shared.lock_state();
...
pub(crate) fn host_handoff(
    ...
    match result {
        ...
        Err(e) => {
            eprintln!("termifier-sessiond: handoff failed, continuing to serve: {e}");
