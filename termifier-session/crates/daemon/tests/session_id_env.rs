//! Regression test (P2 review bug #9): `TERMIFIER_SESSION_ID` must always
//! reflect the real session id inside the spawned shell, even if
//! `SessionSpec.env` tries to set a different value. `spawn_session`
//! currently sets `TERMIFIER_SESSION_ID` first and then applies
...
#[test]
fn termifier_session_id_env_var_cannot_be_overridden_by_spec_env() {
    ...
    let s = SessionSpec {
        ...
        argv: Some(vec![
            "/bin/sh".to_string(),
            "-c".to_string(),
            "echo \"ID=$TERMIFIER_SESSION_ID\"".to_string(),
        ]),
        env: vec![("TERMIFIER_SESSION_ID".to_string(), "bogus".to_string())],
        cols: 80,
        rows: 24,
    };
    ...
    assert!(
        ...
        "TERMIFIER_SESSION_ID inside the session should be the real session id, got {output:?}"
    );
    assert!(
        !output.contains("ID=bogus"),
        "spec.env should not be able to override TERMIFIER_SESSION_ID, got {output:?}"
    );
}
