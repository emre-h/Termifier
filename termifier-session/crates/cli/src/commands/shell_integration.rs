...
//! `detectShell`). A persistent pane's command is `termifier-session attach
...
pub(crate) fn resolve_shell_integration_env(
    ...
    // the attach client's own env already has ZDOTDIR pointed at Termifier's
    // own installed zsh dir (TermifierShellIntegrationEnvironment.apply,
    ...
    // "ghostty統合が無効な環境ではzshがTermifierの.zshenvを直接読み、同じコード
    // が単独で成立する" design -- Termifier's own chain must stand alone
    ...
    // Termifier's own zsh command-log shell integration
    // (ShellIntegrationInstaller.swift / TermifierShellIntegrationEnvironment.swift)
    ...
    // Termifier.app's own process env, set by TermifierShellIntegrationEnvironment
    // .apply). Without this, a persistent-session daemon shell has nothing
    // to read TERMIFIER_ZSH_ZDOTDIR from once execution reaches Termifier's own
    ...
    // as `shell_integration_env`'s own gate) -- TERMIFIER_ZSH_ZDOTDIR is a
    // zsh-specific concept, a stray env var in a bash/fish shell.
    if zsh {
        if let Some(termifier_zsh_zdotdir) = get_env("TERMIFIER_ZSH_ZDOTDIR") {
            env.push(("TERMIFIER_ZSH_ZDOTDIR".to_string(), termifier_zsh_zdotdir));
        }
    }

    // XDG_DATA_DIRS: fish's own vendor_conf.d discovery mechanism
    // (TermifierShellIntegrationEnvironment.apply also appends Termifier's own
    ...
    // this, a persistent-session daemon's fish shell never sees Termifier's
...
mod tests {
    ...
    // ==================== P4: TERMIFIER_ZSH_ZDOTDIR forwarding ====================
    // Termifier's own zsh command-log shell integration installs its own
    // .zshenv-based restore chain (ShellIntegrationInstaller.swift /
    // TermifierShellIntegrationEnvironment.swift on the Swift side), entirely
    ...
    // forwards TERMIFIER_ZSH_ZDOTDIR into the daemon-hosted shell's own env,
    // so Termifier's own restore chain (see the P4 plan's zsh ZDOTDIR relay
    ...
    #[test]
    fn resolve_shell_integration_env_forwards_termifier_zsh_zdotdir_verbatim_when_present() {
        let fake_env: BTreeMap<&str, &str> = [
            ("GHOSTTY_RESOURCES_DIR", "/opt/ghostty-resources"),
            ("SHELL", "/bin/zsh"),
            ("ZDOTDIR", "/Users/alice/.config/zsh"),
            ("TERMIFIER_ZSH_ZDOTDIR", "/Users/alice/.config/termifier-zsh"),
        ]
        .into_iter()
        .collect();
        ...
        let expected: BTreeMap<String, String> = [
            ...
            (
                "TERMIFIER_ZSH_ZDOTDIR".to_string(),
                "/Users/alice/.config/termifier-zsh".to_string(),
            ),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            as_map(result),
            expected,
            "TERMIFIER_ZSH_ZDOTDIR must be forwarded verbatim alongside the existing ghostty zsh env, so \
             Termifier's own zsh integration restore chain has somewhere to read from in a persistent-session \
             daemon shell -- today it is dropped entirely"
        );
    }

    #[test]
    fn resolve_shell_integration_env_does_not_forward_termifier_zsh_zdotdir_for_non_zsh_shell() {
        let fake_env: BTreeMap<&str, &str> = [
            ("SHELL", "/usr/bin/fish"),
            ("TERMIFIER_ZSH_ZDOTDIR", "/Users/alice/.config/termifier-zsh"),
        ]
        ...
        assert!(
            !as_map(result).contains_key("TERMIFIER_ZSH_ZDOTDIR"),
            "TERMIFIER_ZSH_ZDOTDIR is a zsh-specific concept; a non-zsh $SHELL (fish here) must never \
    ...
    // attach client's own env already has ZDOTDIR pointed at Termifier's own
    // installed zsh dir (TermifierShellIntegrationEnvironment.apply, which
    // runs independently of ghostty), that value never reached the
    // persistent-session daemon's spawned shell. Termifier's own chain is
    ...
    fn resolve_shell_integration_env_relays_client_zdotdir_verbatim_when_no_resources_dir_for_zsh()
    {
        let fake_env: BTreeMap<&str, &str> = [
            ...
            (
                "ZDOTDIR",
                "/Users/alice/Library/Application Support/Termifier/shell-integration/zsh",
            ),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            ...
            Some("/Users/alice/Library/Application Support/Termifier/shell-integration/zsh"),
            "with no ghostty resources dir, Termifier's own chain must still stand alone: the client's \
             own ZDOTDIR (already pointed at Termifier's installed zsh dir) must relay verbatim into a \
    ...
    fn resolve_shell_integration_env_forwards_xdg_data_dirs_verbatim_alongside_zsh_env() {
        let fake_env: BTreeMap<&str, &str> = [
            ...
            (
                "XDG_DATA_DIRS",
                "/opt/termifier/shell-integration:/usr/local/share:/usr/share",
            ),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            ...
            Some("/opt/termifier/shell-integration:/usr/local/share:/usr/share"),
    ...
    fn resolve_shell_integration_env_forwards_xdg_data_dirs_for_fish_persistent_panes_without_resources_dir(
        ...
        // discover Termifier's vendor_conf.d entry -- was never forwarded,
        ...
        assert_eq!(
            ...
            "XDG_DATA_DIRS must forward for a fish persistent-session pane even with no ghostty \
             resources dir at all -- fixes fish persistent panes never picking up Termifier's own \
             vendor_conf.d entry"
        );
    }
}
