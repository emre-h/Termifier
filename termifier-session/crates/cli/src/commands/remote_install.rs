//! `termifier-session remote-install <host>`: deploys the session daemon
...
//!   `"$HOME/.termifier/bin/termifier-session"`) reassembles correctly on the
//!   wire and is parsed exactly once, remotely.
//!
//! - `$HOME`, NOT `~`, and always UNQUOTED in the remote command: a
//!   single-quoted `'$HOME/.termifier/bin'` would be WRONG, because single
...
//!   contract exactly: the `TERMIFIER_SSH_BIN` env override when set and
...
/// of the directory the termifier-session binary is installed into.
pub const REMOTE_BIN_DIR: &str = "$HOME/.termifier/bin";
/// Remote path of the installed termifier-session binary itself.
pub const REMOTE_BIN_PATH: &str = "$HOME/.termifier/bin/termifier-session";
...
pub const SSH_PROGRAM_ENV_VAR: &str = "TERMIFIER_SSH_BIN";
...
/// and restricted to the combinations termifier-session ships a payload
...
pub enum PayloadKind {
    LinuxX86_64,
    LinuxAarch64,
    /// Reuse the local host's own termifier-session binary as-is: a Darwin
...
pub enum ArchDetectError {
    ...
    /// OS termifier-session doesn't build for).
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ArchDetectError::Unsupported { os, machine } => {
                write!(
                    f,
                    "unsupported remote platform: {os} {machine} (no termifier-session payload is built for it)"
                )
            }
        }
    }
...
pub trait CommandRunner {
    ...
    /// payload file (the termifier-session binary, the terminfo entry) into
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RemoteInstallError::Transfer(stderr) => write!(
                f,
                "remote-install: failed to transfer the termifier-session binary: {stderr}"
            ),
        }
    }
}
...
/// `termifier-session remote-install <host>` entry point, called from
...
pub fn run(
    ...
    // The single place the TERMIFIER_SSH_BIN override is read; everything
    ...
    println!(
        "installed termifier-session ({}) at {REMOTE_BIN_PATH} on {}",
...
mod tests {
    ...
    fn cli_parses_remote_install_with_only_the_host_all_payload_flags_optional() {
        let cli = crate::cli::Cli::try_parse_from(["termifier-session", "remote-install", "myhost"])
            .expect(
    ...
    fn cli_remote_install_requires_the_host_positional() {
        let result = crate::cli::Cli::try_parse_from(["termifier-session", "remote-install"]);
        assert!(
            result.is_err(),
    ...
    fn transfer_and_chmod_command_writes_a_temp_path_then_renames_and_never_single_quotes_home() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "cat",
                ">",
                "$HOME/.termifier/bin/termifier-session.tmp",
                "&&",
                "chmod",
                "755",
                "$HOME/.termifier/bin/termifier-session.tmp",
                ...
                "-f",
                "$HOME/.termifier/bin/termifier-session.tmp",
                "$HOME/.termifier/bin/termifier-session",
            ]
        );
        for word in &argv {
            assert!(
    ...
    fn transfer_and_chmod_remote_command_word_expands_home_under_a_real_shell() {
        ...
        // separate `mkdir -p $HOME/.termifier/bin` ssh step always runs
        ...
        let installed = tempdir.path().join(".termifier/bin/termifier-session");
        ...
        let tmp = tempdir.path().join(".termifier/bin/termifier-session.tmp");
        assert!(
            !tmp.exists(),
