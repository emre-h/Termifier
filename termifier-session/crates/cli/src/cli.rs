//! Argument parsing for the `termifier-session` binary.
...
#[derive(Parser, Debug)]
#[command(name = "termifier-session", about = "Local PTY session daemon + client")]
pub struct Cli {
    /// Overrides the directory holding the daemon's Unix socket
    /// (default: `$HOME/.termifier/run`). Global so every subcommand, not
    /// just `daemon`, can be pointed at a scratch directory in tests —
    /// no subcommand may ever fall back to a real `~/.termifier` path
    /// implicitly during a test run.
    #[arg(long, global = true)]
    pub runtime_dir: Option<PathBuf>,
    /// Overrides the directory holding the session ledger (default:
    /// `$HOME/.termifier/state`). See `runtime_dir` for why this is global.
    #[arg(long, global = true)]
...
pub struct DaemonArgs {
    ...
    /// Spawned by `termifier-session upgrade`; not meant for direct use.
...
pub struct AttachArgs {
    ...
    /// Host-provided initial PTY columns. Internal Termifier launches use
...
pub struct UpgradeArgs {
    ...
    /// whatever `termifier-session` binary is currently installed).
    #[arg(long)]
...
mod tests {
    ...
    fn upgrade_parses_with_default_binary_none() {
        let cli = Cli::try_parse_from(["termifier-session", "upgrade"])
    ...
    fn upgrade_parses_an_explicit_binary_path() {
        let cli = Cli::try_parse_from([
            ...
            "upgrade",
            "--binary",
            "/tmp/new-termifier-session",
        ])
        .expect("upgrade --binary should parse");
        match cli.command {
            Command::Upgrade(args) => {
                assert_eq!(args.binary, Some(PathBuf::from("/tmp/new-termifier-session")));
            }
            other => panic!("expected Command::Upgrade, got {other:?}"),
        }
    }
    ...
    /// `termifier-session help`) is not surprised by its known limitations
